...
$b = rand()*256;
$c = rand()*256;
printf("#define NVRAM_GENERATED_MAC_ADDRESS        \"macaddr=00:A0:50:%.2x:%.2x:%.2x\"\n", $a, $b, $c);
printf("#define DCT_GENERATED_MAC_ADDRESS          \"\\x00\\xA0\\x50\\x%.2x\\x%.2x\\x%.2x\"\n", $c, $a, $b);
printf("#define DCT_GENERATED_ETHERNET_MAC_ADDRESS \"\\x00\\xA0\\x50\\x%.2x\\x%.2x\\x%.2x\"\n", $c, $a, ($b+1)%256);
