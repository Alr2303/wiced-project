    ...
    if ( duk_is_nan( ctx, -1 ))
    {
        if ( info.total_samples != 0 )
        {
            duration = (double)info.sample_rate / (double)info.total_samples ;
                 ...
                 ( info.bitrate != 0 ))
        {
            duration = (double)audio_client->ref->http_total_content_length /
...
static wiced_result_t audio_object_client_callback( audio_client_ref handle, void* userdata, AUDIO_CLIENT_EVENT_T event, void* arg )
{
    ...
    switch ( event )
    {
        case AUDIO_CLIENT_EVENT_ERROR:
        {
            ...
            callbacks_idx = duk_get_prop_string( ctx, -1,
                                                 INTERNAL_PROPERTY_CALLBACKS );
            /* -> [this callbacks] */
        ...
        case AUDIO_CLIENT_EVENT_PLAYBACK_STARTED:
        {
            ...
            /* Either started ore resumed playing */
    ...
    duk_remove( ctx, this_idx ); /* -> [<callbacks>] */

    while ( num_callbacks )
    {
        /* -> [<callbacks>] */

        duk_dup( ctx, (num_callbacks * -1 )); /* -> [<callbacks> callback] */
        duk_push_this( ctx ); /* -> [<callbacks> callback this] */
        ...
        /* -> [<callbacks> retval/err] */

        duk_pop( ctx ); /* -> [<callbacks>] */
        duk_remove( ctx, (num_callbacks * -1 )); /* -> [<callbacks>] */
