...
{
    WICED_LOG_LEVEL_T loglevel = (WICED_LOG_LEVEL_T)atoi(argv[ 1 ]);

    if (wiced_log_set_facility_level(WLF_TEST, loglevel) != WICED_SUCCESS)
    {
        return ERR_BAD_ARG;
    }
    return ERR_CMD_OK;
...
{
    printf("0x%08x\n", (unsigned int)wiced_log_get_facility_level(WLF_TEST));
    return ERR_CMD_OK;
}
