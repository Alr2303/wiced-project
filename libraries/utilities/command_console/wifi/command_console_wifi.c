...
static int print_nan_config_status (wwd_nan_config_status_t* nan_cfg_status);
...
int wl_nan (int argc, char* argv[] )
{
    ...
    wwd_nan_config_status_t nan_cfg_status;
...
static wwd_result_t wl_nan_iovar(wiced_bool_t set, uint16_t cmd_id, void *buffer, wiced_bool_t start )
{
    ...
    switch (cmd_id) {
        ...
        case WL_NAN_CMD_CFG_STATUS:
            result = wwd_nan_config_get_status( ( wwd_nan_config_status_t* ) buffer );
            print_nan_config_status(( wwd_nan_config_status_t* ) buffer );
            break;
...
int ds1_enter(int argc, char* argv[])
{
    ...
    {
        result = wiced_wifi_enter_ds1_debug( WICED_STA_INTERFACE, WICED_OFFLOAD_MAGIC, NULL, ulp_milliseconds, &overrides );
        goto exit;
    }
        ...
        strncmp(sub_cmd_name, "keep_alive", strlen(sub_cmd_name) ) == 0 )
    {
        ...
        result = wiced_wifi_enter_ds1_debug( WICED_STA_INTERFACE, WICED_OFFLOAD_KEEP_ALIVE, &offload, ulp_milliseconds, &overrides );
        free(buffer);
    }
        ...
        strncmp(sub_cmd_name, "arp_hostip", strlen(sub_cmd_name) ) == 0 )
    {
        ...
        {
            result = wiced_wifi_enter_ds1_debug( WICED_STA_INTERFACE, WICED_OFFLOAD_ARP_HOSTIP, &offload, ulp_milliseconds, &overrides );
        }
        else
        {
            err = ERR_BAD_ARG;
        ...
        strncmp(sub_cmd_name, "pattern", strlen(sub_cmd_name) ) == 0 )
    {
        ...
        result = wiced_wifi_enter_ds1_debug( WICED_STA_INTERFACE, WICED_OFFLOAD_PATTERN, &offload, ulp_milliseconds, &overrides );
...
int wlver(int argc, char* argv[] )
{
  ...
  {
    WPRINT_APP_INFO( (" Get WLAN Firmware Version failed\n"));
  }
  else
  {
      WPRINT_APP_INFO(("%s\n", version ));
...
static int print_nan_config_status (wwd_nan_config_status_t* nan_cfg_status)
{
    ...
    WPRINT_APP_INFO (("    NAN Enabled:                         %d\n", nan_cfg_status->enabled ));
    WPRINT_APP_INFO (("    NAN Inited:                          %d\n", nan_cfg_status->inited ));
    WPRINT_APP_INFO (("    NAN Joined:                          %d\n", nan_cfg_status->joined ));
    WPRINT_APP_INFO (("    NAN Merged:                          %d\n", nan_cfg_status->merged ));
    WPRINT_APP_INFO (("    NAN Role:                            %d\n", nan_cfg_status->role ));
    WPRINT_APP_INFO (("    NAN chanspec[0]:                     %lx\n", nan_cfg_status->chspec[0] ));
    WPRINT_APP_INFO (("    NAN chanspec[1]:                     %lx\n", nan_cfg_status->chspec[1] ));
    WPRINT_APP_INFO (("    NAN Master Rank:                     %02x:%02x:%02x:%02x:%02x:%02x\n",
            nan_cfg_status->mr[0], nan_cfg_status->mr[1],
            nan_cfg_status->mr[2], nan_cfg_status->mr[3],
            nan_cfg_status->mr[4], nan_cfg_status->mr[5] ));
    WPRINT_APP_INFO (("    NAN Anchor Master Rank:              %02x:%02x:%02x:%02x:%02x:%02x\n",
            nan_cfg_status->amr[0], nan_cfg_status->amr[1],
            nan_cfg_status->amr[2], nan_cfg_status->amr[3],
            nan_cfg_status->amr[4], nan_cfg_status->amr[5] ));

    WPRINT_APP_INFO (("    NAN Pending TxFrames:                %lu\n", nan_cfg_status->cnt_pend_txfrm ));
    WPRINT_APP_INFO (("    NAN TX Disc/Sync BeaconCount:        %lu\n", nan_cfg_status->cnt_bcn_tx ));
    WPRINT_APP_INFO (("    NAN RX Disc/Sync Beacon count:       %lu\n", nan_cfg_status->cnt_bcn_rx ));
    WPRINT_APP_INFO (("    NAN TX Service/Disc Frame count:     %lu\n", nan_cfg_status->cnt_svc_disc_tx ));
    WPRINT_APP_INFO (("    NAN RX Service/Disc Frame count:     %lu\n", nan_cfg_status->cnt_svc_disc_rx ));
    WPRINT_APP_INFO (("    NAN Anchor Master BeaconTarget Time: %lu\n", nan_cfg_status->ambtt ));
    WPRINT_APP_INFO (("    NAN ClusterID                        %02x:%02x:%02x:%02x:%02x:%02x\n",
            nan_cfg_status->cid.octet[0], nan_cfg_status->cid.octet[1],
            nan_cfg_status->cid.octet[2], nan_cfg_status->cid.octet[3],
            nan_cfg_status->cid.octet[4], nan_cfg_status->cid.octet[5] ));
    WPRINT_APP_INFO (("    NAN Hop Count:                       %d\n", nan_cfg_status->hop_count ));
    WPRINT_APP_INFO (("  ==============  \n" ));
