    ...
    wiced_bt_a2dp_sink_data_cb_t data_cb)
{
    ...
    wiced_hci_set_event_callback(AUDIO, wiced_hci_audio_cb);
...
wiced_result_t wiced_bt_a2dp_sink_connect(wiced_bt_device_address_t bd_address)
{
    ...
    wiced_hci_send(HCI_CONTROL_AUDIO_COMMAND_CONNECT, data, length);
    free(data);
    return WICED_SUCCESS;
}
...
wiced_result_t wiced_bt_a2dp_sink_disconnect(wiced_bt_device_address_t bd_address)
{
    ...
    wiced_hci_send(HCI_CONTROL_AUDIO_COMMAND_DISCONNECT, data, length);
    free(data);
    return WICED_SUCCESS;
}
...
wiced_result_t wiced_bt_a2dp_sink_start(wiced_bt_device_address_t bd_address)
{
    ...
    UINT16_TO_STREAM(p, wiced_bt_hci_a2dp_remote_dev[context_index].handle);
    wiced_hci_send(HCI_CONTROL_AUDIO_START, data, length);
    free(data);
    return WICED_SUCCESS;
}
...
wiced_result_t wiced_bt_a2dp_sink_start_rsp( wiced_bt_device_address_t bd_address, uint8_t response)
{
    ...
    wiced_hci_send(HCI_CONTROL_AUDIO_START_RSP, data, length);
    free(data);
    return WICED_SUCCESS;
}
...
wiced_result_t wiced_bt_a2dp_sink_suspend(wiced_bt_device_address_t bd_address)
{
    ...
    UINT16_TO_STREAM(p, wiced_bt_hci_a2dp_remote_dev[context_index].handle);
    wiced_hci_send(HCI_CONTROL_AUDIO_STOP, data, length);
    free(data);
    return WICED_SUCCESS;
}
...
static int get_a2d_context_index_by_addr(wiced_bt_device_address_t remote_addr)
{
    ...
    while(i < A2DP_CONN_MAX_LIMIT)
    {
        ...
        {
            WPRINT_APP_INFO(( "get_a2d_context_index_by_addr Already present i:%d\n",i ));
            return i;
        }
        i++;
    }
    WPRINT_APP_INFO(("get_a2d_context_index_by_addr Invalid!!\n" ));
...
static int get_a2d_context_index_by_handle(uint16_t    handle)
{
    ...
    while(i < A2DP_CONN_MAX_LIMIT)
    {
        ...
        {
            WPRINT_APP_INFO(( "get_a2d_context_index_by_handle Already present i:%d\n",i ));
            return i;
        }
        i++;
    }
    WPRINT_APP_INFO(("get_a2d_context_index_by_handle Invalid!!\n" ));
    return A2DP_CONN_MAX_LIMIT;
}
...
static int set_a2d_context_index_peer(wiced_bt_device_address_t remote_addr)
{
    ...
    WPRINT_APP_INFO(("set_a2d_context_index_peer Invalid!!\n" ));
    return A2DP_CONN_MAX_LIMIT;
}
...
static void wiced_hci_audio_cb(uint16_t event, uint8_t* payload, uint32_t len)
{
    ...
    WPRINT_APP_INFO(("%s event:%x\n", __func__, event));

    switch(event)
    {
        case HCI_CONTROL_AUDIO_EVENT_COMMAND_STATUS:
        {
            WPRINT_APP_INFO(("%s: command status:%d\n", __FUNCTION__, *p));
        }
        break;
        case HCI_CONTROL_AUDIO_EVENT_CONNECTED:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_CONNECTED\n", __FUNCTION__));
        ...
        case HCI_CONTROL_AUDIO_EVENT_CONNECTION_FAILED:
        {
            ...
            WPRINT_APP_INFO(("%s: Connection FAILED!!\n", __FUNCTION__));
        ...
        case HCI_CONTROL_AUDIO_EVENT_DISCONNECTED:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_DISCONNECTED\n", __FUNCTION__));
        ...
        case HCI_CONTROL_AUDIO_EVENT_STARTED:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_STARTED\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            ...
            (*wiced_bt_hci_a2dp_sink_cb.control_cb)(WICED_BT_A2DP_SINK_START_EVT,
                (wiced_bt_a2dp_sink_event_data_t *) &start);
        }
        break;
        case HCI_CONTROL_AUDIO_EVENT_START_IND:
        {
            wiced_bt_a2dp_sink_status_t  start;
            uint16_t handle;
            uint8_t  label;
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_START_IND handle:%x label:%x\n", __FUNCTION__, handle, label));
        ...
        case HCI_CONTROL_AUDIO_EVENT_STOPPED:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_STOPPED\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
        ...
        break;
        case HCI_CONTROL_AUDIO_EVENT_CODEC_CONFIGURED:
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AUDIO_EVENT_CODEC_CONFIGURED\n", __FUNCTION__));
        ...
        break;
        default:
            WPRINT_APP_INFO(("%s EVENT not handled\n", __FUNCTION__));
            break;
    }
}
