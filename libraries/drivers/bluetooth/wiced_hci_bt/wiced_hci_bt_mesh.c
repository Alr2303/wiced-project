...
static void wiced_hci_mesh_cb(uint16_t event, uint8_t* payload, uint32_t len)
{
    ...
    switch(event)
    {
        case HCI_CONTROL_MESH_EVENT_PROVISIONING_STATUS:
        {
            ...
            uint8_t result = *p;
            WPRINT_APP_INFO(("HCI_CONTROL_MESH_EVENT_PROVISIONING_STATUS : %02X \n",result));
            (*wh_bt_mesh_context.prov_end_cb)(conn_id, result);
        }
        break;
        case HCI_CONTROL_MESH_EVENT_PROXT_DATA:
        {
            uint32_t packet_len;
            uint8_t *packet;
        ...
        case HCI_CONTROL_EVENT_DEVICE_STARTED :
        {
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_EVENT_DEVICE_STARTED "));
        ...
        {
            WPRINT_APP_INFO(("recvd HCI_CONTROL_MESH_EVENT_NVRAM_WRITE command , nvm_idx : %d , len : %lu \n", *p, len));
            uint8_t nvram_id;
            uint8_t *packet;
