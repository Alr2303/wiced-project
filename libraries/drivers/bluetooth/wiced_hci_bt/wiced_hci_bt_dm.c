        ...
        const wiced_bt_cfg_buf_pool_t     wiced_bt_cfg_buf_pools[WICED_BT_CFG_NUM_BUF_POOLS] )
{
    ...
    WPRINT_APP_INFO(("[%s] init bus\n",__FUNCTION__));
    /* start wiced_hci only here. Once started, it stays up. */
    if ( (result = wiced_hci_up() ) != WICED_SUCCESS )
    {
        WPRINT_APP_INFO( ( "[%s] Failed to initialize wiced hci\n",__FUNCTION__) );
        return result;
    }
    WPRINT_APP_INFO(("[%s] wiced_hci up\n",__FUNCTION__));
...
static void wiced_hci_dm_cb(uint16_t command, uint8_t* payload, uint32_t len)
{
    ...
    switch(command)
    {
        ...
        case HCI_CONTROL_EVENT_NVRAM_DATA:
            {
                WPRINT_APP_INFO(("[%s] HCI_CONTROL_EVENT_NVRAM_DATA\n",__func__));

                STREAM_TO_UINT16(wh_bt_dm_context.nvram_id, p);
                WPRINT_APP_INFO(("NVRAM write:id:%x\n", wh_bt_dm_context.nvram_id));
        ...
        case HCI_CONTROL_EVENT_DEVICE_STARTED:
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_EVENT_DEVICE_STARTED\n",__func__));
            if ( payload )
            {
...
// TODO: to add all events corresponding to this.
        ...
        {
            evt = 0xff;
            WPRINT_APP_INFO(("HCI_CONTROL_LE_EVENT_COMMAND_STATUS\n"));
            break;
        }
        case HCI_CONTROL_LE_EVENT_SCAN_STATUS:
        {
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_LE_EVENT_SCAN_STATUS: %d\n", data.ble_scan_state_changed));
            break;
        }
        ...
        case HCI_CONTROL_LE_EVENT_ADVERTISEMENT_STATE:
        {
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_LE_EVENT_ADVERTISEMENT_STATE[%s]\n",__func__));
            break;
        }
        ...
        case HCI_CONTROL_LE_EVENT_CONNECTED:
        {
            ...
            WPRINT_APP_INFO(("[%d] LE Device Connected event\n",(int)len));
        ...
        {
            wiced_bt_gatt_event_data_t    event_data;
            WPRINT_APP_INFO(("HCI_CONTROL_LE_EVENT_DISCONNECTED:[%s]\n",__func__));
        ...
        case HCI_CONTROL_EVENT_COMMAND_STATUS:
        {
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_EVENT_COMMAND_STATUS: %d\n", control_event_status));
            break;
        }
        ...
        {
            //WPRINT_APP_INFO(("this is the HCI_CONTROL_EVENT_HCI_TRACE\n"));
            evt = 0xff;
            break;
        }
        case HCI_CONTROL_EVENT_PAIRING_COMPLETE:
        {
            WPRINT_APP_INFO(("This is the HCI_CONTROL_EVENT_PAIRING_COMPLETE\n"));
        ...
        case HCI_CONTROL_EVENT_CONNECTION_STATUS:
        {
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_EVENT_CONNECTION_STATUS is_connected:%d, reason:%d\n",is_connected, reason));
        ...
        break;
        default:
            WPRINT_APP_INFO((" [%s] this is the default case\n\n", __func__));
            evt = 0xff;
            break;
