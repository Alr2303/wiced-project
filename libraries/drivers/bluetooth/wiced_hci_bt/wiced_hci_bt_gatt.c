...
static void wiced_hci_gatt_cb(uint16_t command, uint8_t* payload, uint32_t len)
{
    ...
    switch(command)
    {
        case HCI_CONTROL_GATT_EVENT_COMMAND_STATUS:
            ...
            WPRINT_APP_INFO(("HCI_CONTROL_LE_EVENT_COMMAND_STATUS: %d\n", gatt_command_status));
            break;

        case HCI_CONTROL_GATT_EVENT_DISCOVERY_COMPLETE:
            STREAM_TO_UINT16(event_data.discovery_complete.conn_id, payload);
            break;

        case HCI_CONTROL_GATT_EVENT_SERVICE_DISCOVERED:
            WPRINT_APP_INFO(("HCI_CONTROL_GATT_EVENT_SERVICE_DISCOVERED\n"));
            if ( event_data.discovery_result.discovery_data.group_value.service_type.len == 2 )
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.group_value.service_type.uu.uuid16, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.group_value.s_handle, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.group_value.e_handle, payload );
            }
            else
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                for( i=0; i<16; i++ )
                {
                    STREAM_TO_UINT8( event_data.discovery_result.discovery_data.group_value.service_type.uu.uuid128[i], payload );
                }
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.group_value.s_handle, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.group_value.e_handle, payload );
            }
        ...
        case HCI_CONTROL_GATT_EVENT_CHARACTERISTIC_DISCOVERED:
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_CHARACTERISTIC_DISCOVERED\n",__func__));
            if ( event_data.discovery_result.discovery_data.characteristic_declaration.char_uuid.len == 2 )
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.characteristic_declaration.handle, payload );
                STREAM_TO_UINT8( event_data.discovery_result.discovery_data.characteristic_declaration.characteristic_properties, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.characteristic_declaration.val_handle, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.characteristic_declaration.char_uuid.uu.uuid16, payload );
            }
            else
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                ...
                for( i=0; i<16; i++ )
                {
                    STREAM_TO_UINT8( event_data.discovery_result.discovery_data.characteristic_declaration.char_uuid.uu.uuid128[i], payload );
                }
            }
            break;

        case HCI_CONTROL_GATT_EVENT_DESCRIPTOR_DISCOVERED:
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_DESCRIPTOR_DISCOVERED\n",__func__));
            if ( event_data.discovery_result.discovery_data.char_descr_info.type.len == 2 )
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.char_descr_info.handle, payload );
                STREAM_TO_UINT16( event_data.discovery_result.discovery_data.char_descr_info.type.uu.uuid16, payload );
            }
            else
            {
                STREAM_TO_UINT16( event_data.discovery_result.conn_id, payload );
                ...
                for( i=0; i<16; i++)
                {
                    STREAM_TO_UINT8( event_data.discovery_result.discovery_data.char_descr_info.type.uu.uuid128[i], payload );
                }
            }
            break;
        ...
        {
            uint8_t       data[4]; /* data provided in this event is only 4 bytes */
            ...
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_READ_REQUEST\n",__func__));
        ...
        case HCI_CONTROL_GATT_EVENT_READ_RESPONSE:
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_READ_RESPONSE\n",__func__));
            break;
        ...
        case HCI_CONTROL_GATT_EVENT_WRITE_REQUEST:
        {
            ...
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_WRITE_REQUEST\n",__func__));
            ...
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_COMMAND_WRITE_RESPONSE\n",__func__));
            break;
        }

        case HCI_CONTROL_GATT_EVENT_WRITE_RESPONSE:
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_WRITE_RESPONSE\n",__func__));
            break;
        ...
        case HCI_CONTROL_GATT_EVENT_INDICATION:
        {
            ...
            STREAM_TO_ARRAY(data, payload, len);
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_INDICATION\n",__func__));
            event_data.attribute_request.conn_id = (uint16_t)((data[0])|data[1] <<8);
        ...
        case HCI_CONTROL_GATT_EVENT_NOTIFICATION:
        {
            uint8_t data[4]; /* data provided in this event is only 4 bytes */
            STREAM_TO_ARRAY(data, payload, len);
            WPRINT_APP_INFO(("[%s] HCI_CONTROL_GATT_EVENT_NOTIFICATION\n",__func__));
            event_data.attribute_request.conn_id = (uint16_t)((data[0])|data[1] <<8);
            event_data.attribute_request.data.handle = (uint16_t)((data[2])|data[3]<<8);
        }
        break;
        ...
        default:
            WPRINT_APP_INFO(("[%s] Command Not Handled: %x\n",__func__,command));
            break;
    }
}
