...
*/
    ...
    WPRINT_APP_INFO((" %s: cmd:%x, state:%x \n", __func__, cmd, state));
...
static int get_rc_context_index_by_addr(wiced_bt_device_address_t remote_addr)
{
    ...
    WPRINT_APP_INFO(("get_rc_context_index_by_addr Invalid!!\n" ));
...
static int get_rc_context_index_by_handle(uint16_t    handle)
{
    ...
    WPRINT_APP_INFO(("get_rc_context_index_by_handle Invalid!!\n" ));
    return AVRCP_CONN_MAX_LIMIT;
}
    ...
    while(i < AVRCP_CONN_MAX_LIMIT)
    {
        if(wiced_bt_hci_avrcp_remote_dev[i].in_use == WICED_TRUE)
        {
            if(!wiced_bt_avrcp_sink_utils_bdcmp(wiced_bt_hci_avrcp_remote_dev[i].peer_bd_addr, remote_addr))
             {
                 WPRINT_APP_INFO(( "set_rc_context_index_peer Already present i:%d\n",i ));
                 return i;
             }
             else
             {
        ...
        else
        {
            ...
            WPRINT_APP_INFO( ( "set_rc_context_index_peer New entry i:%d\n",i ));
            return i;
        }
    }
    WPRINT_APP_INFO(("set_rc_context_index_peer Invalid!!\n" ));
    return AVRCP_CONN_MAX_LIMIT;
}
...
static void wiced_hci_avrc_controller_cb(uint16_t event, uint8_t* payload, uint32_t len)
{
    ...
    switch(event)
    {
        ...
        case HCI_CONTROL_AVRC_CONTROLLER_EVENT_CONNECTED:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_CONNECTED\n", __FUNCTION__));
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_DISCONNECTED\n", __FUNCTION__));
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_PLAY_STATUS_INFO\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        case HCI_CONTROL_AVRC_CONTROLLER_EVENT_CURRENT_TRACK_INFO:
        {
            ...
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_CURRENT_TRACK_INFO\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_PLAY_STATUS\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_TRACK_CHANGE\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_TRACK_END\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        {
            WPRINT_APP_INFO(("%s HCI_CONTROL_AVRC_CONTROLLER_EVENT_TRACK_START\n", __FUNCTION__));
            STREAM_TO_UINT16(handle, p);
            context_index = get_rc_context_index_by_handle(handle);
            if(context_index == AVRCP_CONN_MAX_LIMIT)
            {
        ...
        case HCI_CONTROL_AVRC_TARGET_EVENT_VOLUME_LEVEL:
        {
             ...
             STREAM_TO_UINT16(handle, p);
             WPRINT_APP_INFO(("%s Absolute volume changed on Remote-device handle: %d\n", __FUNCTION__, handle));
             context_index = get_rc_context_index_by_handle(handle);
             if(context_index == AVRCP_CONN_MAX_LIMIT)
             {
                 WPRINT_APP_INFO(("%s: Invalid Entry!!\n", __FUNCTION__));
