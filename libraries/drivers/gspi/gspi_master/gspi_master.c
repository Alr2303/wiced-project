...
wiced_result_t gspi_transfer_backplane_bytes( gspi_transfer_direction_t direction, uint32_t address, uint32_t size, uint8_t* data )
{
    ...
    {
        transfer_size = ( remaining_buf_size > GSPI_MAX_BACKPLANE_TRANSFER_SIZE ) ?
                              GSPI_MAX_BACKPLANE_TRANSFER_SIZE : remaining_buf_size;
...
static wiced_result_t gpsi_get_slave_response( gspi_master_t *gspi_master )
{
    ...
    switch ( status )
    {
        case GSPI_SLAVE_RESPONSE_GOOD:
            WPRINT_LIB_INFO( ("gspi slave response success\n") );
            break;
        case GSPI_SLAVE_RESPONSE_FAILED:
            WPRINT_LIB_INFO( ("gspi slave response error!\n") );
            break;
        default:
            WPRINT_LIB_INFO( ("Invalid gspi slave response\n") );
    }
    return result;
}
...
wiced_result_t gspi_write_command( gspi_master_t *gspi_master,  uint16_t address, uint16_t size, uint8_t* data_out )
{
    ...
    uint8_t data_payload[sizeof( *gspi_sw_tx_hdr ) + size];
    ...
    VERIFY_RESULT( gspi_transfer_backplane_bytes( GSPI_BUS_WRITE, gspi_master->master_to_slave_buffer_address, data_total_size, data_payload ) );
