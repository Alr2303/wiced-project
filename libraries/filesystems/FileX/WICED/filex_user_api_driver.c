...
static wiced_result_t filex_shim_dir_delete       ( wiced_filesystem_t* fs_handle, const char* directory_name );
...
wiced_filesystem_driver_t wiced_filesystem_driver_filex =
{
    ...
    .dir_delete       = filex_shim_dir_delete      ,
...
static wiced_result_t filex_shim_format           ( wiced_block_device_t* device )
{
    ...
    fx_media_format_oem_name_set( (const UCHAR*) "Cypress" );
...
static wiced_result_t filex_shim_mount            ( wiced_block_device_t* device, wiced_filesystem_t* fs_handle_out )
{
    ...
    /* Initialize FX_MEDIA, or it will have non NULL callback pointer for FileX 5.4
       e.g) fx_media_open_notify_set, fx_media_close_notify_set */
    memset( media, 0, sizeof(FX_MEDIA) );
...
static wiced_result_t filex_shim_file_open        ( wiced_filesystem_t* fs_handle, wiced_file_t* file_handle_out, const char* filename, wiced_filesystem_open_mode_t mode )
{
    ...
    /* Initialize FX_FILE, or it will have non NULL callback pointer for FileX 5.4
       e.g) fx_file_write_notify */
    memset( filex_handle, 0, sizeof(FX_FILE) );
...
static wiced_result_t filex_shim_file_read        ( wiced_file_t* file_handle, void* data, uint64_t bytes_to_read, uint64_t* returned_bytes_count )
{
    ...
    *returned_bytes_count = bytes_read;
    ...
    if ( filex_result != FX_SUCCESS )
    {
        /* other than FX_SUCCESS is not always error */
        return filex_result;
    }
...
/* Delete a directory within a FileX filesystem */
static wiced_result_t filex_shim_dir_delete       ( wiced_filesystem_t* fs_handle, const char* directory_name )
{
    FX_MEDIA* media  = &fs_handle->data.filex.handle;
    UINT      filex_result;

    filex_result = fx_directory_delete( media, directory_name );
    if ( filex_result != FX_SUCCESS )
    {
        return WICED_FILESYSTEM_ERROR;
    }
