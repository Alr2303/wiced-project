...
tsp_loop_return adts_dsp_loop( tsp_loop_param data )
{
    ...
    if ( NO_ERR == myerr )
    {
        ...
        *ctrl = ( *ctrl ) | ADTSPARSE_CTRL_DSP_STATUS;
        ...
        {
            LOG_MSG( adtsp->log_lvl, LOG_LOW, "[ADTSPARSE][%s] dsp_loop (STARTPING) \n", adtsp->label );
            do
            {
                /** DSP thread
                ...
                if ( ( TRUE == fdk_f ) && ( NO_ERR == myerr ) )
                {
                    ...
                    adtsp->fdk_err = aacDecoder_SetParam( adtsp->fdk_handle,
                                                          AAC_PCM_OUTPUT_INTERLEAVED,
                                                          1 /* interleaved */ );

                    if ( adtsp->fdk_err != AAC_DEC_OK )
                    {
                        LOG_ERR( 0, LOG_ALWAYS, "[ADTSPARSE][%s] [ERROR] dsp_loop: unable to set FDK optional paramters.\n",
                                 adtsp->label );
                    ...
                    if ( adtsp->pcm_info_tgt_chmap == ( AACDEC_CHANNEL_MAP_FL | AACDEC_CHANNEL_MAP_FR ) )
                    {
                        ...
                        if ( adtsp->fdk_err != AAC_DEC_OK )
                        {
                            LOG_ERR( 0, LOG_ALWAYS, "[ADTSPARSE][%s] [ERROR] dsp_loop: unable to set FDK min_out_chnum paramter.\n",
                                     adtsp->label );
                        ...
                        if ( adtsp->fdk_err != AAC_DEC_OK )
                        {
                            LOG_ERR( 0, LOG_ALWAYS, "[ADTSPARSE][%s] [ERROR] dsp_loop: unable to set FDK max_out_chnum paramter.\n",
                                     adtsp->label );
                ...
                if ( NO_ERR == myerr )
                {
                            ...
                            ( adtsp->data_fifo[ adtsp->data_fifo_r_idx ].status != 0 ) )
                    {
                        ...
                        uint8_t* data_ptr  = &adtsp->data_raw[ adtsp->data_fifo[ adtsp->data_fifo_r_idx ].raw_idx + local_offs ];
                      ...
                      void*   adtsparse )
{
    ...
    else
    {
        if ( NULL == adtsp )
        {
            LOG_ERR( 0, LOG_ALWAYS, "[ADTSPARSE][%s] [ERROR] adtsparse_flish adtsparse is NULL\n", "-" );
        }
        else
        {
            LOG_ERR( 0, LOG_ALWAYS,
                                   ...
                                   uint8_t  buf_type )
{
     ...
     * DSP must be alive
     */
    if ( !( adtsp->ctrl & ADTSPARSE_CTRL_DSP_STATUS ) )
    {
        LOG_ERR( 0, LOG_ALWAYS,
