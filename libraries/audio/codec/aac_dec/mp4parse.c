...
static inline uint8_t mp4_parse_box( int8_t* err, mp4parse_t* mp4parse,
                                     uint32_t* idx, char* box_id, uint32_t set_flag, uint8_t skip_flag );

static inline uint8_t mp4_parse_box_discard( int8_t* err, mp4parse_t* mp4parse,
...
static inline int8_t mp4_get_sample_limits( int8_t* err, mp4parse_t* mp4parse,
...
#if 0
            // quick print to verify the table
            {
                uint32_t  k;
                uint32_t* tv = (uint32_t*) mp4p->mp4_sample_size_table.byte_data;

                printf( "STSZ table (%" PRIu32 "), ", mp4p->mp4_sample_size_table.sample_count );
                for ( k = 0; k < mp4p->mp4_sample_size_table.sample_count; k++ )
                {
...
#if 0
            // HACK to verify the table
            {
                uint32_t  k;
                uint32_t* tv32 = (uint32_t*) mp4p->mp4_chunk_offs_table.byte_data;
                uint32_t* tv64 = (uint32_t*) mp4p->mp4_chunk_offs_table.byte_data;

                printf( "STCO table (%" PRIu32 "), ", mp4p->mp4_chunk_offs_table.entry_count );
                for ( k = 0; k < mp4p->mp4_chunk_offs_table.entry_count; k++ )
                {
                                               ...
                                               uint8_t** audio_specific_info, uint32_t* audio_specific_info_size )
{
     ...
     */
    while ( ( i < ( mp4p->data_raw_w_idx - 4 ) ) &&
            ( 0 == myerr ) &&
            ( ( 0 == mp4p->mp4_audio_specific_info_size ) ||
              ( !( mp4p->search_status & SEARCH_STSIZE_COMPLETE ) ) ||
              ( !( mp4p->search_status & SEARCH_COFFSET_COMPLETE ) ) ) )
    {
        id4 = (char*) ( mp4p->data_raw + i );
...
tsp_loop_return mp4parse_dsp_loop( tsp_loop_param data )
{
    ...
    if ( NO_ERR == myerr )
    {
        ...
        *ctrl = ( *ctrl ) | MP4PARSE_CTRL_DSP_STATUS;
        ...
        {
            LOG_MSG( mp4p->log_lvl, LOG_LOW, "[MP4PARSE][%s] mp4parse_dsp_loop (STARTPING) \n", mp4p->label );
            do
            {
                /** DSP thread
                 ...
                 */
                uint8_t search_complete_f = ( ( 0 != mp4p->mp4_audio_specific_info_size ) &&
                                              ( mp4p->search_status & SEARCH_STSIZE_COMPLETE ) &&
                                              ( mp4p->search_status & SEARCH_COFFSET_COMPLETE ) &&
                                              ( mp4p->search_status & SEARCH_STSC_COMPLETE ) );
                          ...
                          ( FALSE == search_complete_f ) ) )
                {
                    ...
                    search_complete_f = ( ( 0 != mp4p->mp4_audio_specific_info_size ) &&
                                          ( mp4p->search_status & SEARCH_STSIZE_COMPLETE ) &&
                                          ( mp4p->search_status & SEARCH_COFFSET_COMPLETE ) &&
                                          ( mp4p->search_status & SEARCH_STSC_COMPLETE ) );
                }
                                  ...
                                  uint8_t  buf_type )
{
     ...
     * DSP must be alive
     */
    if ( !( mp4p->ctrl & MP4PARSE_CTRL_DSP_STATUS ) )
    {
        LOG_ERR( 0, LOG_ALWAYS,
