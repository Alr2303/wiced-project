...
wiced_result_t bt_buffer_pool_init( bt_buffer_pool_handle_t* pool_handle, uint16_t buffer_count, uint16_t data_size )
{
    ...
    wiced_log_printf("bt_buffer_pool_init: buffer size (Header+Data+padding) %d + %d + %d, Buffer Count %u, \n", sizeof(bt_buffer_t), data_size, (int) padding, (unsigned int)buffer_count);
    wiced_log_printf("bt_buffer_pool_init: Pool size (Pool Header + total buffers size) %d + %u \n", sizeof(bt_buffer_pool_t), (unsigned int)pool_size);
#endif
    pool = malloc( pool_size + sizeof(bt_buffer_pool_t) );
    if ( pool == NULL )
    {
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR,"bt_buffer_pool_init: Pool allocation failed, out of heap space \n");
   ...
   pool->min_free_count = pool->free_count;
...
#ifdef MEM_POOL_DEBUG
        wiced_log_printf("bt_buffer_pool_init: Buffer #%u, buffer ptr %p \n", (unsigned int)a, packet->data);
#endif
    ...
    else
    {
        *pool_handle = (bt_buffer_pool_handle_t)pool;
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO,"bt_buffer_pool_init: Pool creation successful, pool handle %p \n", *pool_handle);
    }
...
wiced_result_t bt_buffer_pool_deinit( bt_buffer_pool_handle_t pool_handle )
{
    ...
    wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO,"bt_buffer_pool_deinit: destroying pool %p \n", pool_handle);
...
#endif
{
    ...
    if ( result == WICED_BT_SUCCESS )
    {
        ...
        // wiced_log_printf("bt_buffer_pool_allocate_buffer: allocated by %s    pool handle 0x%x, buffer ptr %p free remaining %d\n", packet->alloc_string, (int)pool_handle, buffer, pool->free_count);
#endif
    }
    else
    {
#ifdef MEM_POOL_DEBUG
        wiced_log_printf("bt_buffer_pool_allocate_buffer: %s failed to get a buffer. pool handle 0x%x, free_count %d\n", alloc_string, (int) pool_handle, pool->free_count);
#else
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR,"bt_buffer_pool_allocate_buffer: failed. pool handle 0x%x, free_count %d\n", (int) pool_handle, pool->free_count);
#endif
    }
...
#endif
{
    ...
    //wiced_log_printf("bt_buffer_pool_free_buffer: alloced by: %s, freed by: %s, pool handle %p, buffer ptr %p free count after free %d\n", packet->alloc_string, packet->free_string, packet->pool, buffer, packet->pool->free_count);
#endif
    wiced_rtos_unlock_mutex( &packet->pool->mutex );
    //wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR,"bt_buffer_pool_free_buffer: pool handle %p, buffer ptr %p free count after free %d\n", packet->pool, buffer, packet->pool->free_count);
