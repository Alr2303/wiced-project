...
static wiced_bt_dev_status_t bluetooth_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    wiced_log_msg( WLF_AUDIO, WICED_LOG_INFO,"bluetooth_management_callback: %x\r\n", event );
    ...
    switch( event )
    {
        ...
        case BTM_LPM_STATE_LOW_POWER:
            /* Event when Host has told the Bluetooth controller to go to sleep */
            break;
        ...
        default:
            ...
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO,"bluetooth_management_callback: Unhandled Bluetooth Management Event: 0x%x\n", event);
