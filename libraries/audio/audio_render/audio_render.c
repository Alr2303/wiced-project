...
#define AUDIO_RENDER_PERIOD_SIZE                    (256)
#define AUDIO_RENDER_PERIODS_TO_START               (4)
...
static inline wiced_result_t get_media_time(audio_render_t* audio, int64_t* media_time, wiced_bool_t* clock_set)
{
    ...
    if (audio->time_get_cb == NULL)
    {
        ...
        if (result == WICED_SUCCESS)
        {
            *media_time = ((int64_t)master_secs * NUM_NSECONDS_IN_SECOND) + (int64_t)master_nsecs;
            if (clock_set)
            {
                /*
                ...
                if (master_secs != local_secs || master_nsecs != local_nsecs)
                {
...
static inline void release_head_buffer(audio_render_t* audio)
{
    buf_list_t* buf_ptr;
...
static int process_buffer_with_clock(audio_render_t* audio)
{
    ...
    atime   = (int64_t)((double)(NUM_NSECONDS_IN_SECOND * (double)(audio->latency / (double)audio->config.frame_size)) / (double)audio->config.sample_rate);
    ...
    if (diff > 0)
    {
        bytes_to_drop = ((uint32_t)(((double)diff * (double)audio->config.sample_rate) / (double)NUM_NSECONDS_IN_SECOND)) * audio->config.frame_size;
        if (bytes_to_drop > buffer_bytes_avail)
        {
            ...
            bytes_to_drop = (buffer_bytes_avail / audio->config.frame_size) * audio->config.frame_size;
        }

        audio->audio_frames_dropped += bytes_to_drop / audio->config.frame_size;
        ...
        while (bytes_to_drop > 0 && buf_ptr != NULL)
        {
            ...
            else
            {
                ...
                buf_ptr->buf.pts += (int64_t)((double)(NUM_NSECONDS_IN_SECOND * (double)(bytes_to_drop / (double)audio->config.frame_size)) /
                                              (double)audio->config.sample_rate);
            }
        }
    ...
    else if (diff < 0)
    {
        ...
        bytes_to_add = ((uint32_t)(((double)(-diff) * (double)audio->config.sample_rate) / (double)NUM_NSECONDS_IN_SECOND)) * audio->config.frame_size;

        audio->audio_frames_inserted += bytes_to_add / audio->config.frame_size;
    ...
    while (size < audio_buf_size && audio->buffer_head != NULL)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                buf_ptr->buf.pts += (int64_t)((double)(NUM_NSECONDS_IN_SECOND * (double)(bytes / (double)audio->config.frame_size)) /
                                              (double)audio->config.sample_rate);
            }
        }
...
static void process_audio_buffers(audio_render_t* audio)
{
    ...
    if (audio->signal_eos && !audio->quit)
    {
        ...
        if (bytes_available < audio->period_size)
        {
            ...
            wait_time_min = (uint32_t)((double)((NUM_USECONDS_IN_SECOND / (double)(audio->config.sample_rate * audio->config.frame_size)) * (double)audio->period_size) / (double)NUM_MSECONDS_IN_SECOND);
...
wiced_result_t audio_render_configure(audio_render_ref audio, wiced_audio_config_t* config)
{
    ...
    if (audio->audio_configured)
    {
        ...
        /*
         * We need to reinitialize the audio device to change the audio format.
         * Since we need to deinit the audio driver before reconfiguring, we have
         * to make sure that the main audio render thread isn't actively trying to
         * output data to the audio driver. Start by sending a flush event to the main thread.
         */

        wiced_rtos_set_event_flags(&audio->events, AUDIO_EVENT_FLUSH);

        /*
         * And give the thread time to process the flush. Note that if the main thread
         * was waiting on an audio buffer, we may have to wait for the duration of a
         * period buffer for the thread to be able to respond to the flush event.
         */

        wait_time = (uint32_t)((double)((NUM_USECONDS_IN_SECOND / (double)audio->config.sample_rate) * (double)audio->period_size) / (double)NUM_MSECONDS_IN_SECOND);
        attempts = 0;
        while (audio->buffer_head != NULL && attempts++ < MAX_CONFIG_FLUSH_ATTEMPTS)
        {
            wiced_rtos_delay_milliseconds(wait_time);
        }

        if (audio->buffer_head != NULL)
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Error flushing buffer list for reconfigure\n");
            return WICED_ERROR;
        }

        /*
         * Should be safe to shut it down now.
         */

        if (perform_device_release(audio) != WICED_SUCCESS)
        {
            return WICED_ERROR;
        }
    }
    ...
    wiced_log_msg(WLF_AUDIO, WICED_LOG_NOTICE, "Audio config is %d channels, %lu kHz, %d bps, frame size %u\n",
...
wiced_result_t audio_render_push(audio_render_ref audio, audio_render_buf_t *buf)
{
    ...
    else if (audio->buffer_free_list != NULL)
    {
        ...
        if (audio->media_time_set == WICED_FALSE && audio->clock_enable == WICED_TRUE && buf != NULL)
        {
            ...
            {
                wiced_log_msg(WLF_AUDIO, WICED_LOG_NOTICE, "media time - net is %d:%09d, PTS %d:%09d (clock_set: %d)\n",
                              PRINT_TIME(mtime), PRINT_TIME(buf->pts), audio->clock_set);
            }
            else
            {
