...
static void reset_initial_http_variables(audio_client_t* client, wiced_bool_t seek_request)
{
    ...
    client->http_icy_header                = WICED_FALSE;
    client->decoder_first_kick             = WICED_TRUE;
    client->http_transfer_encoding_chunked = WICED_FALSE;
    client->http_redirect                  = WICED_FALSE;
    ...
    client->http_chunk_size                = 0;
    client->http_chunk_bytes               = 0;
    client->http_chunk_skip                = 0;
...
static wiced_result_t audio_client_connect(audio_client_t* client)
{
    ...
    result = audio_client_uri_split(client->stream_uri, client->hostname, AUDIO_CLIENT_HOSTNAME_LEN, client->path, AUDIO_CLIENT_PATH_LEN, &client->port, &tls);
    ...
    if (str_to_ip(client->hostname, &ip_address) != 0)
    {
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "Attemping DNS lookup for %s\n", client->hostname);
        result = dns_client_hostname_lookup(client->hostname, &ip_address, AUDIO_CLIENT_DNS_TIMEOUT_MS, WICED_STA_INTERFACE);
        if (result != WICED_SUCCESS)
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "DNS lookup failed (%d)\n", result);
            return result;
        }
    }
...
static wiced_result_t audio_client_connect_and_send_request(audio_client_t* client)
{
    wiced_result_t result;
...
static wiced_result_t audio_client_parse_http_header(audio_client_t* client)
{
                                    ...
                                    http_code == HTTP_STATUS_PERMANENT_REDIRECT))
    {
        ...
        wiced_tcp_disconnect_with_timeout(client->socket_ptr, WICED_NO_WAIT);
        wiced_tcp_delete_socket(client->socket_ptr);
        client->socket_ptr    = NULL;
        client->http_redirect = WICED_TRUE;
    }
    ...
    while (ptr != NULL && ptr < &client->http_buf[client->http_buf_idx])
    {
        ...
        else if (strncasecmp((const char *)ptr, HTTP_HEADER_CONTENT_TYPE, sizeof(HTTP_HEADER_CONTENT_TYPE) - 1) == 0)
        {
            ...
            semi = strchr(client->http_content_type, ';');
            if (semi != NULL)
            {
                *semi = '\0';
            }
        ...
        else if (strncasecmp((const char *)ptr, HTTP_HEADER_TRANSFER_ENCODING, sizeof(HTTP_HEADER_TRANSFER_ENCODING) - 1) == 0)
        {
            ...
            if (strncasecmp((const char *)ptr, HTTP_HEADER_CHUNKED, sizeof(HTTP_HEADER_CHUNKED) - 1) == 0)
            {
                ...
                client->http_transfer_encoding_chunked = WICED_TRUE;
            }
            else
            {
        ...
        else if (strncasecmp((const char *)ptr, HTTP_HEADER_LOCATION, sizeof(HTTP_HEADER_LOCATION) - 1) == 0)
        {
            if (client->http_redirect)
            {
                ...
                else
                {
                    ...
                    client->stream_uri    = play_node->data;
                    strcpy(client->stream_uri, ptr);
                }
            }
        }
    ...
    if (client->http_redirect)
    {
        client->params.event_cb(client, client->params.userdata, AUDIO_CLIENT_EVENT_HTTP_REDIRECT, (void*)client->stream_uri);

        /*
         * Send the main loop an event to start the connection process again.
         */

        wiced_rtos_set_event_flags(&client->http_events, HTTP_EVENT_CONNECT);
        return WICED_SUCCESS;
    }

    if (client->http_total_content_length == 0)
    {
    ...
    for (i = 0; audio_client_playlist_mime_types[i].mime_type != NULL && client->params.load_playlist_files; i++)
    {
        if (strncasecmp(audio_client_playlist_mime_types[i].mime_type, client->http_content_type, strlen(audio_client_playlist_mime_types[i].mime_type)) == 0)
        {
            ...
            result = client->params.event_cb(client, client->params.userdata, AUDIO_CLIENT_EVENT_PLAYLIST_MIME_TYPE, (void*)&audio_client_playlist_mime_types[i]);
            if (result == WICED_SUCCESS)
            {
                client->http_load_file = WICED_TRUE;
            }
            break;
        }
    }
    ...
    if (client->http_load_file)
    {
        ...
        if (client->http_total_content_length == 0)
        {
            client->http_content_length = AUDIO_CLIENT_DEFAULT_LOAD_FILE_SIZE;
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "No content length specified for load file. Using default size of %d\n", client->http_content_length);
        }
    ...
    if (!client->http_in_header)
    {
        ...
        while (idx < avail_data_length)
        {
            ...
            if ((data[idx] == audio_client_HTTP_response[client->http_header_idx] ||
                (audio_client_HTTP_response[client->http_header_idx] == '?' && isdigit(data[idx]))) ||
                (data[idx] == audio_client_ICY_response[client->http_header_idx]))
            {
                if (client->http_header_idx == 0 && data[idx] == audio_client_ICY_response[client->http_header_idx])
                {
                    client->http_icy_header = WICED_TRUE;
                }

                client->http_buf[client->http_buf_idx++] = data[idx];
                client->http_header_idx++;

                if ((!client->http_icy_header && audio_client_HTTP_response[client->http_header_idx] == '\0') ||
                    (client->http_icy_header  && audio_client_ICY_response[client->http_header_idx] == '\0'))
                {
                    /*
...
static void send_empty_buffer_to_decoder(audio_client_t* client)
{
    ...
    while (client->http_done == WICED_TRUE && !client->stop_in_progress)
    {
...
void write_data_buf(audio_client_t* client, data_buf_t* dbuf, uint8_t* data, uint16_t data_length)
{
    ...
    wiced_log_msg(WLF_AUDIO, WICED_LOG_DEBUG2, "write decoder buf: %u bytes, ridx %u, widx %u\n",
    ...
    if (client->params.data_buffer_preroll == 0 || client->initial_buffer_count > client->params.data_buffer_preroll ||
        (client->http_total_content_length == 0 && client->params.no_length_disable_preroll))
    {
        wiced_rtos_set_event_flags(&client->decoder_events, DECODER_EVENT_AUDIO_DATA);
...
static wiced_result_t process_chunked_data(audio_client_t* client, uint8_t* data, uint16_t data_length)
{
    data_buf_t* dbuf;
    char* ptr;
    int32_t chunk_size;
    ...
    while (idx < data_length && !client->http_done)
    {
        while (idx < data_length && client->http_chunk_skip > 0)
        {
            idx++;
            client->http_chunk_skip--;
        }

        if (client->http_chunk_size == 0)
        {
            /*
            ...
            while (idx < data_length && client->http_buf_idx < AUDIO_CLIENT_HTTP_BUF_SIZE - 1)
            {
                client->http_buf[client->http_buf_idx++] = data[idx];
                if (data[idx] == audio_client_HTTP_chunk_separator[client->http_body_idx])
                {
                    if (audio_client_HTTP_chunk_separator[++client->http_body_idx] == '\0')
                    {
                        end_of_chunk_size = WICED_TRUE;
                        idx++;
                        client->http_buf[client->http_buf_idx - (sizeof(audio_client_HTTP_chunk_separator) - 1)] = '\0';
                        break;
                    }
                }
                else
            ...
            ptr = strchr( (const char*)client->http_buf, ';');
            if (ptr != NULL)
            {
                *ptr = 0;
            }

            if (string_to_signed((const char*)client->http_buf, strlen(client->http_buf), &chunk_size, 1) == 0)
            {
                wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Error converting chunk size\n");
                return WICED_ERROR;
            }

            client->http_chunk_size = chunk_size;
            client->http_buf_idx    = 0;
            client->http_body_idx   = 0;
        }
        ...
        while (client->http_done == WICED_FALSE && idx < data_length)
        {
            size = MIN(data_length - idx, client->http_chunk_size - client->http_chunk_bytes);
            if (client->http_load_file)
            {
                len = size;
                if (client->http_file_idx + len > client->http_content_length)
                {
                    len = client->http_content_length - client->http_file_idx;
                }

                if (len > 0)
                {
                    memcpy(&client->http_file_data[client->http_file_idx], &data[idx], len);
                    client->http_file_idx += len;
                }
            }
            else
            {
                ...
                write_data_buf(client, dbuf, &data[idx], size);
            }
            idx += size;
            client->http_chunk_bytes += size;

            if (client->http_chunk_bytes == client->http_chunk_size)
            {
                /*
...
static wiced_result_t process_data(audio_client_t* client, uint8_t* data, uint16_t data_length)
{
    ...
    if (client->http_transfer_encoding_chunked)
    {
        /*
         * Chunked data needs to be parsed rather than blindly copied.
         */

        return process_chunked_data(client, data, data_length);
    }
    ...
    if (client->http_load_file)
    {
        ...
        if (len > 0)
        {
            memcpy(&client->http_file_data[client->http_file_idx], data, len);
            client->http_file_idx += len;
        }
    ...
    while (client->http_done == WICED_FALSE)
    {
        ...
        write_data_buf(client, dbuf, data, data_length);
        break;
    }
...
static void audio_client_http_process_socket(audio_client_t* client)
{
    ...
    while (!client->http_done)
    {
        ...
        if (result != WICED_SUCCESS)
        {
            ...
            if (client->http_load_file && client->http_total_content_length == 0 && client->http_file_idx > 0)
            {
                client->http_done = WICED_TRUE;
                break;
            }
        ...
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "All done reading HTTP data\n");
            client->http_done = WICED_TRUE;
        }
    }
...
static void audio_client_http_thread(uint32_t arg)
{
    ...
    wiced_rtos_wait_for_event_flags(&client->http_events, HTTP_ALL_EVENTS, &events, WICED_TRUE, WAIT_FOR_ANY_EVENT, WICED_NO_WAIT);
    wiced_rtos_set_event_flags(&client->http_events, HTTP_EVENT_CONNECT);
    ...
    while (!client->http_done && !client->http_error)
    {
        ...
        {
            if (audio_client_connect_and_send_request(client) != WICED_SUCCESS)
            {
                break;
            }
        }
