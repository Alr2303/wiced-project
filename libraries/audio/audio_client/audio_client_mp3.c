    ...
    if ( buf_id == NULL )
    {
        if ( dec->pcm_first_buffer )
        {
            ...
            if (dec->audio_client->start_offset_ms > 0)
            {
                ...
                audio_client_check_start_offset_seek(dec->audio_client);
            }
            else
            {
        ...
        if ( !(dec->sync_f) )
        {
            ...
            if (errCode != BRCM_MP3DEC_BITSTREAM_DECODE_ERROR)
            {
                if (errCode == BRCM_MP3DEC_NO_ERROR)
                {
                    ...
                    wiced_log_msg( WLF_AUDIO, WICED_LOG_DEBUG0, "audio_client_mp3: process_mp3: sync FOUND. r_idx=%d, w_idx=%d\r\n", dec->cdb_r_idx, dec->cdb_w_idx );
            ...
            {
                wiced_log_msg( WLF_AUDIO, WICED_LOG_ERR, "audio_client_mp3: process_mp3: sync search bitstream error\r\n" );
...
wiced_result_t audio_client_mp3_decoder_start( audio_client_t* client )
{
    ...
    wiced_log_msg( WLF_AUDIO, WICED_LOG_INFO, "audio_client_mp3: LIB_MP3 version %ld \n", brcm_mp3dec_getversion());
...
wiced_result_t audio_client_mp3_decoder_ioctl(struct audio_client_s* client, DECODER_IOCTL_T ioctl, void* arg)
{
    ...
    switch (ioctl)
    {
        ...
        case DECODER_IOCTL_GET_SEEK_POSITION:
            ...
            if ( position > client->http_total_content_length )
            {
                position = client->http_total_content_length;
            }
