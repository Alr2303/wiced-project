...
#define UPNP_TRANSPORT_THREAD_PRIORITY   WICED_APPLICATION_PRIORITY
#define UPNP_TRANSPORT_THREAD_STACK_SIZE (4096)
...
static transport_state_t  transport_state_ = TRANSPORT_STOPPED;
...
static wiced_thread_t  wiced_thread;
static int             thread_exit = 0;
static char* upnp_duration = NULL;
...
static void change_transport_state(transport_state_t new_state) {
...
static int set_avtransport_uri(struct action_event *event)
{
	...
	/* Get duration upnp controller provides */
    if(upnp_duration != NULL)
    {
        free(upnp_duration);
        upnp_duration = NULL;
    }

	upnp_duration = SongMetaData_get_duration(meta);
	if (upnp_duration != NULL)
	{
	    replace_var(TRANSPORT_VAR_CUR_TRACK_DUR, upnp_duration);
	}
...
static void *thread_update_track_time(void *userdata) {
	...
	while (thread_exit == 0) {
		...
		if (pos_result == 0) {
			if (upnp_duration == NULL && duration != last_duration) {
...
struct service *upnp_transport_get_service(void) {
    ithread_mutex_init(&transport_mutex, NULL);
...
void upnp_transport_init(struct upnp_device *device) {
    if (transport_service_.last_change == NULL) {
	...
	thread_exit = 0;
	upnp_duration = NULL;
	wiced_rtos_create_thread(&wiced_thread, UPNP_TRANSPORT_THREAD_PRIORITY, "upnp_transport", (wiced_thread_function_t)thread_update_track_time, UPNP_TRANSPORT_THREAD_STACK_SIZE, NULL);
}

void upnp_transport_deinit(void) {
    thread_exit = 1;
    wiced_rtos_thread_force_awake(&wiced_thread);
    wiced_rtos_thread_join(&wiced_thread);
    wiced_rtos_delete_thread(&wiced_thread);

    if (transport_service_.last_change != NULL) {
        UPnPLastChangeCollector_delete(transport_service_.last_change);
        transport_service_.last_change = NULL;
    }
    if (transport_service_.variable_container != NULL) {
        VariableContainer_delete(transport_service_.variable_container);
        transport_service_.variable_container = NULL;
        state_variables_ = NULL;
    }
    ithread_mutex_destroy(&transport_mutex);
