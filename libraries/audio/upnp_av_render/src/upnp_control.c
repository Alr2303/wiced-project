...
static int set_volume_db(struct action_event *event) {
	...
	output_set_volume(exp(decibel / 20 * log(10)), volume_decibel_to_level(raw_decibel_in));
	service_unlock();
...
static int get_volume_dbrange(struct action_event *event) {
	...
	snprintf(minval, sizeof(minval), /* "%lld" */ "%ld", (long)volume_db_range.min);
...
struct service *upnp_control_get_service(void) {
    ithread_mutex_init(&control_mutex, NULL);
...
void upnp_control_deinit(void) {
    if (control_service_.last_change != NULL) {
        UPnPLastChangeCollector_delete(control_service_.last_change);
        control_service_.last_change = NULL;
    }
    if (control_service_.variable_container != NULL) {
        VariableContainer_delete(control_service_.variable_container);
        control_service_.variable_container = NULL;
        state_variables_ = NULL;
    }
    ithread_mutex_destroy(&control_mutex);
