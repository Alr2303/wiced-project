...
#else
#define kAirTunesBufferNodeCountUDP			256		// 256 nodes * 352 samples per node = ~4 seconds.
#endif
...
#else
#define kAirTunesRetransmitCount			256		// Max number of outstanding retransmits.
#endif
...
static OSStatus
	_LegacyAudioSetup(
		...
		CFMutableDictionaryRef		inResponseParams );
static void		_LegacyAudioTearDown( AirPlayReceiverSessionRef inSession );
static void *	_LegacyAudioThread( void *inArg );
static OSStatus	_LegacyAudioReceiveRTCP( AirPlayReceiverSessionRef inSession, SocketRef inSock, RTCPType inExpectedType );
static OSStatus	_LegacyAudioReceiveRTP( AirPlayReceiverSessionRef inSession, RTPPacket *inPkt, size_t inSize );
static OSStatus
	_LegacyAudioProcessPacket( 
		AirPlayReceiverSessionRef	inSession, 
		AirTunesBufferNode *		inNode, 
		size_t 						inSize, 
		Boolean						inIsRetransmit );
static OSStatus
	_LegacyAudioDecodePacket( 
		AirPlayReceiverSessionRef	inSession, 
		uint8_t *					inSrc, 
		size_t						inSrcSize, 
		uint8_t *					inDst, 
		size_t						inDstMaxSize, 
		size_t *					outSize );
static void		_LegacyAudioAdjustForSkew( AirPlayReceiverSessionRef inSession, AirTunesSource *inSource, AirTunesBufferNode *inNode );
static void		_LegacyAudioRender( AirPlayReceiverSessionRef inSession, uint32_t inRTPTime, void *inBuffer, size_t inSize );
static Boolean	_LegacyAudioTrackDups( AirPlayReceiverSessionRef inSession, uint16_t inSeq );
static void		_LegacyAudioTrackLosses( AirPlayReceiverSessionRef inSession, AirTunesBufferNode *inNode );
static void		_LegacyAudioUpdateLatency( AirPlayReceiverSessionRef inSession );
...
static OSStatus
	_MainAltAudioSetup(
		...
		CFMutableDictionaryRef		inResponseParams );
static void		_MainAltAudioTearDown( AirPlayAudioStreamContext * const ctx );
static OSStatus	_MainAltAudioStart( AirPlayAudioStreamContext * const ctx );
...
#endif
static void		_AudioDecoderFinalize( AirPlayReceiverSessionRef inSession );
static OSStatus
...
// Debugging
	...
	int						gAirTunesDebugLogAllSkew						= 1;
		...
		const AirPlayReceiverSessionCreateParams *	inParams )
{
	...
	me->rtpAudioSock			= kInvalidSocketRef;
	me->rtpAudioCmdSock			= kInvalidSocketRef;
	...
	ats->maxIdleTicks				= me->server->timeoutDataSecs * ticksPerSec;
	...
	ats->idleLogIntervalTicks		= 1 * ticksPerSec;
		...
		void *										inContext )
{
	...
	if( inCompletion )
	{
		...
		msg->userContext2 = (void *) inCompletion;
...
static void	_AirPlayReceiverSessionSendCommandCompletion( HTTPMessageRef inMsg )
{
	...
	AirPlayReceiverSessionCommandCompletionFunc const		completion	= (AirPlayReceiverSessionCommandCompletionFunc) inMsg->userContext2;
	OSStatus												err;
		...
		CFDictionaryRef *			outResponseParams )
{
	...
	{
		requestStreamDesc = (CFDictionaryRef) CFArrayGetValueAtIndex( requestStreams, streamIndex );
		require_action( CFIsType( requestStreamDesc, CFDictionary ), exit, err = kTypeErr );
		...
		switch( type )
		{
			case kAirPlayStreamType_LegacyAudio:
				err = _LegacyAudioSetup( me, requestStreamDesc, responseParams );
				require_noerr( err, exit );
	...
	err = AirPlayReceiverSessionControl( me, kCFObjectFlagDirect, CFSTR( kAirPlayCommand_SetUpStreams ), NULL, 
		inRequestParams, NULL );
	require_noerr( err, exit );
	
	if( legacy )
	{
		_LegacyAudioUpdateLatency( me );
	...
	atr_ulog( kLogLevelTrace, "Session set up for ID %llu with %##a\n%?@", 
		me->clientSessionID, &me->peerAddr, log_category_enabled( atr_ucat(), kLogLevelVerbose ), responseParams );
...
exit:
	...
	if( err ) AirPlayReceiverSessionTearDown( me, err );
	return( err );
}
...
void	AirPlayReceiverSessionTearDown( AirPlayReceiverSessionRef inSession, OSStatus inReason )
{
	...
	AirPlayReceiverSessionPlatformFinalize( inSession );
	_MainAltAudioTearDown( &inSession->mainAudioCtx );
	_MainAltAudioTearDown( &inSession->altAudioCtx );
	_LegacyAudioTearDown( inSession );
	_ControlTearDown( inSession );
	_TimingFinalize( inSession );
	AirTunesClock_Finalize();
	_AudioDecoderFinalize( inSession );
	...
	atr_ulog( kLogLevelTrace, "Torn down\n" );
}
...
OSStatus	AirPlayReceiverSessionStart( AirPlayReceiverSessionRef inSession, AirPlayReceiverSessionStartInfo *inInfo )
{
	...
	if( IsValidSocket( inSession->rtpAudioSock ) )
	{
		err = pthread_create( &inSession->rtpAudioThread, NULL, _LegacyAudioThread, inSession );
		require_noerr( err, exit );
		inSession->rtpAudioThreadPtr = &inSession->rtpAudioThread;
	}
	
	err = _MainAltAudioStart( &inSession->mainAudioCtx );
	require_noerr( err, exit );
	
	err = _MainAltAudioStart( &inSession->altAudioCtx );
	require_noerr( err, exit );
	
	err = _TimingNegotiate( inSession );
	require_noerr( err, exit );
	
	intervalNanos = 250 * kNanosecondsPerMillisecond;
	...
	inSession->periodicTimer = source = dispatch_source_create( DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue() );
	...
	dispatch_source_set_timer( source, dispatch_time( DISPATCH_TIME_NOW, intervalNanos ), intervalNanos, intervalNanos );
		...
		uint32_t *					outLastTS )
{
...
#endif
	ats->rtcpTIAnnounceCount		= 0; // Reset so we don't try to play samples again until we get a new timeline.
	...
	for( curr = stop->next; curr != stop; curr = next )
	{
		...
		{
			inSession->busyByteCount -= (uint32_t) curr->size;
	...
	switch( inType )
	{
		case kAirPlayStreamType_LegacyAudio:
			...
			if( inSession->source.timeAnnouncePending )
			{
				...
				AirTunesClock_GetSynchronizedTimeNearUpTicks( &ntpTime, inHostTime );
				_TimingProcessTimeAnnounce( inSession, &ntpTime, inSampleTime );
				inSession->source.timeAnnouncePending = false;
			}
			_LegacyAudioRender( inSession, inSampleTime, inBuffer, inLen );
			_SessionUnlock( inSession );
			err = kNoErr;
			break;
...
static void	_Finalize( CFTypeRef inCF )
{
	AirPlayReceiverSessionRef const		me = (AirPlayReceiverSessionRef) inCF;
	
	AES_CBCFrame_Final( &me->decryptorStorage );
	pthread_mutex_forget( &me->mutexPtr );
}
...
static void	_PerformPeriodTasks( void *inContext )
{
	...
	ticks = UpTicks();
	audioOnly = !( me->screen || me->mainAudioCtx.session );
	if( audioOnly )
	{
		if( ats->receiveCount == ats->lastReceiveCount )
		{
			...
			if( idleTicks > ats->maxIdleTicks )
			{
				...
				AirPlayReceiverServerControl( me->server, kCFObjectFlagDirect, CFSTR( kAirPlayCommand_SessionDied ), me, 
					NULL, NULL );
				goto exit;
			}
		}
		...
		CFMutableDictionaryRef		inResponseParams )
{
	...
	AirPlayAudioFormat						audioFormat;
	AudioStreamBasicDescription				asbd;
	sockaddr_ip								sip;
	size_t									i, n;
	uint8_t *								ptr;
	
	require_action( !IsValidSocket( me->rtpAudioSock ), exit2, err = kAlreadyInitializedErr );
	...
	ctx->type  = kAirPlayStreamType_LegacyAudio;
	ctx->label = "Legacy";
	
	me->compressionType = (AirPlayCompressionType) CFDictionaryGetInt64( inStreamDesc, CFSTR( kAirPlayKey_CompressionType ), NULL );
	audioFormat = (AirPlayAudioFormat) CFDictionaryGetInt64( inStreamDesc, CFSTR( kAirPlayKey_AudioFormat ), NULL );
	if( audioFormat == kAirPlayAudioFormat_Invalid )
	{
		if(      me->compressionType == kAirPlayCompressionType_PCM )		audioFormat = kAirPlayAudioFormat_PCM_44KHz_16Bit_Stereo;
		else if( me->compressionType == kAirPlayCompressionType_ALAC )		audioFormat = kAirPlayAudioFormat_ALAC_44KHz_16Bit_Stereo;
		else { dlogassert( "Bad compression type: 0x%X", me->compressionType ); err = kUnsupportedErr; goto exit; }
	}
	err = AirPlayAudioFormatToASBD( audioFormat, &asbd, &ctx->bitsPerSample );
	require_noerr( err, exit );
	...
	me->totalBufferFrameCount	= me->samplesPerFrame * me->nodeCount;
	
	// Set up RTP.
	
	err = ServerSocketOpen( me->peerAddr.sa.sa_family, SOCK_DGRAM, IPPROTO_UDP, -kAirPlayPort_RTPAudio, 
		&me->rtpAudioPort, -kAirTunesRTPSocketBufferSize, &me->rtpAudioSock );
	require_noerr( err, exit );
	
	if( !me->server->qosDisabled ) SocketSetQoS( me->rtpAudioSock, me->audioQoS );
	
	err = OpenSelfConnectedLoopbackSocket( &me->rtpAudioCmdSock );
	require_noerr( err, exit );
	...
	CFDictionarySetInt64( responseStreamDesc, CFSTR( kAirPlayKey_Type ), kAirPlayStreamType_LegacyAudio );
	...
	atr_ulog( kLogLevelTrace, "RTP audio set up on port %d, RTCP on port %d\n", me->rtpAudioPort, me->rtcpPortLocal );
	
exit:
	CFReleaseNullSafe( responseStreamDesc );
	if( err ) _LegacyAudioTearDown( me );
	
exit2:
	if( err ) atr_ulog( kLogLevelWarning, "### RTP audio setup failed: %#m\n", err );
	return( err );
}

//===========================================================================================================================
//	_LegacyAudioTearDown
//===========================================================================================================================
...
static void	_LegacyAudioTearDown( AirPlayReceiverSessionRef inSession )
{
	...
	if( inSession->rtpAudioThreadPtr )
	{
		err = SendSelfConnectedLoopbackMessage( inSession->rtpAudioCmdSock, "q", 1 );
		check_noerr( err );
		
		err = pthread_join( inSession->rtpAudioThread, NULL );
		check_noerr( err );
		
		inSession->rtpAudioThreadPtr = NULL;
	}
	ForgetSocket( &inSession->rtpAudioCmdSock );
	ForgetSocket( &inSession->rtpAudioSock );
	...
	if( ctx->type != kAirPlayStreamType_Invalid )
	{
		...
		atr_ulog( kLogLevelTrace, "RTP/RTCP torn down\n" );
	}
}
...
static void *	_LegacyAudioThread( void *inArg )
{
	...
	SocketRef const						rtpSock  = session->rtpAudioSock;
	SocketRef const						rtcpSock = session->rtcpSock;
	SocketRef const						cmdSock  = session->rtpAudioCmdSock;
	fd_set								readSet;
	int									maxFd;
	int									n;
	OSStatus							err;
	...
	SetThreadPriority( kAirPlayThreadPriority_AudioReceiver );
	
	FD_ZERO( &readSet );
	maxFd = -1;
	if( rtpSock  > maxFd ) maxFd = rtpSock;
	if( rtcpSock > maxFd ) maxFd = rtcpSock;
	...
	for( ;; )
	{
		...
		if( FD_ISSET( rtpSock,  &readSet ) ) _LegacyAudioReceiveRTP( session, NULL, 0 );
		if( FD_ISSET( rtcpSock, &readSet ) ) _LegacyAudioReceiveRTCP( session, rtcpSock, kRTCPTypeAny );
		if( FD_ISSET( cmdSock,  &readSet ) ) break; // The only event is quit so break if anything is pending.
	}
	atr_ulog( kLogLevelTrace, "RTP/RTCP audio thread exit\n" );
	return( NULL );
}

//===========================================================================================================================
//	_LegacyAudioReceiveRTCP
//===========================================================================================================================
...
static OSStatus	_LegacyAudioReceiveRTCP( AirPlayReceiverSessionRef inSession, SocketRef inSock, RTCPType inExpectedType )
{
	...
	switch( pkt.header.pt )
	{
		case kRTCPTypeTimeSyncResponse:
			...
			AirTunesClock_GetSynchronizedTimeNearUpTicks( &recvTime, ticks );
...
static OSStatus	_LegacyAudioReceiveRTP( AirPlayReceiverSessionRef inSession, RTPPacket *inPkt, size_t inSize )
{
	...
	{
			err = SocketRecvFrom( inSession->rtpAudioSock, node->data, inSession->nodeBufferSize, &len, NULL, 0, NULL, 
				NULL, NULL, NULL );
		if( err == EWOULDBLOCK ) goto exit;
		require_noerr( err, exit );
		debug_add( gAirTunesDebugRecvByteCount, len );
	...
	err = _LegacyAudioProcessPacket( inSession, node, len, inPkt != NULL );
	require_noerr_quiet( err, exit );
	node = NULL;
		...
		Boolean						inIsRetransmit )
{
	...
	if( _LegacyAudioTrackDups( inSession, pktSeq ) )	{ err = kDuplicateErr; goto exit; }
	if( !inIsRetransmit )								_LegacyAudioTrackLosses( inSession, inNode );
...
static void	_LegacyAudioRender( AirPlayReceiverSessionRef inSession, uint32_t inRTPTime, void *inBuffer, size_t inSize )
{
	...
	if( src->rtcpTIAnnounceCount == 0 ) goto exit;
	...
	for( curr = stop->next; curr != stop; curr = next )
	{
		...
		{
			err = _LegacyAudioDecodePacket( inSession, curr->ptr, curr->size, curr->ptr, 
		...
		{
			_LegacyAudioAdjustForSkew( inSession, src, curr );
			endTS = srcTS + (uint32_t)( curr->size / bytesPerUnit );
		}
...
static OSStatus	_RetransmitsSendRequest( AirPlayReceiverSessionRef inSession, uint16_t inSeqStart, uint16_t inSeqCount )
{
	...
	if( debug_false_conditional( gAirTunesDebugRetransmitTiming ) )
	{
		...
		AirTunesClock_GetSynchronizedTime( &nowAT );
...
static OSStatus	_RetransmitsProcessResponse( AirPlayReceiverSessionRef inSession, RTCPRetransmitResponsePacket *inPkt, size_t inSize )
{
	...
	err = _LegacyAudioReceiveRTP( inSession, &inPkt->payload.rtp, 
		...
		CFMutableDictionaryRef		inResponseParams )
{
	...
	audioFormat = (AirPlayAudioFormat) CFDictionaryGetInt64( inRequestStreamDesc, CFSTR( kAirPlayKey_AudioFormat ), NULL );
	if( audioFormat == kAirPlayAudioFormat_Invalid ) audioFormat = kAirPlayAudioFormat_PCM_44KHz_16Bit_Stereo;
	err = AirPlayAudioFormatToASBD( audioFormat, &asbd, &ctx->bitsPerSample );
	require_noerr( err, exit );
	ctx->sampleRate		= (uint32_t) asbd.mSampleRate;
	ctx->channels		= asbd.mChannelsPerFrame;
	...
	atr_ulog( kLogLevelTrace, "%s audio set up on receive port %d, send port %d\n", ctx->label, receivePort, sendPort );
	
exit:
	CFReleaseNullSafe( responseStreamDesc );
	if( err && ctx ) _MainAltAudioTearDown( ctx );
...
static OSStatus	_TimingNegotiate( AirPlayReceiverSessionRef inSession )
{
	...
	for( ;; )
	{
		...
		for( ;; )
		{
			...
			err = _LegacyAudioReceiveRTCP( inSession, timingSock, kRTCPTypeTimeSyncResponse );
			if( err )
			{
				++nRecvErrors;
				++nFailure;
...
static void *	_TimingThread( void *inArg )
{
	...
	SetThreadPriority( kAirPlayThreadPriority_TimeSyncClient );
	...
	for( ;; )
	{
		...
		if( err )							{ dlogassert( "select() error: %#m", err ); usleep( 100000 ); continue; }
		if( FD_ISSET( sock,    &readSet ) ) _LegacyAudioReceiveRTCP( session, sock, kRTCPTypeTimeSyncResponse );
		if( FD_ISSET( cmdSock, &readSet ) ) break; // The only event is quit so break if anything is pending.
	}
...
static OSStatus	_TimingSendRequest( AirPlayReceiverSessionRef inSession )
{
	...
	AirTunesClock_GetSynchronizedTime( &now );
...
static OSStatus	_TimingProcessResponse( AirPlayReceiverSessionRef inSession, RTCPTimeSyncPacket *inPkt, const AirTunesTime *inTime )
{
	...
	src->rtcpTIResetStats = AirTunesClock_Adjust( (int64_t)( median * 1E9 ), forceStep );
...
static void	_TimingProcessTimeAnnounce( AirPlayReceiverSessionRef inSession, const AirTunesTime *inGlobalTime, uint32_t inRTPTime )
{
	...
	oldAnnounceCount = src->rtcpTIAnnounceCount;
	...
	increment_wrap( src->rtcpTIAnnounceCount, 1 );
...
#endif // AIRPLAY_DECODER

//===========================================================================================================================
//	_AudioDecoderFinalize
//===========================================================================================================================
...
static void	_LogStarted( AirPlayReceiverSessionRef inSession, AirPlayReceiverSessionStartInfo *inInfo, OSStatus inStatus )
{
	...
	atr_ulog( kLogLevelNotice, 
		"AirPlay audio started: From=%s D=0x%012llx A=%##a "
...
static void	_LogEnded( AirPlayReceiverSessionRef inSession, OSStatus inReason )
{
	...
	DataBuffer_AppendF( &db, "AirPlay audio ended: Dur=%{dur} Reason=%#m\n", durationSecs, inReason );
...
static void	_UpdateEstimatedRate( AirPlayAudioStreamContext *ctx, uint32_t inSampleTime, uint64_t inHostTime )
{
	...
	if( inHostTime >= ctx->rateUpdateNextTicks )
	{
		...
		newCount = oldCount + 1;
		AirTunesClock_GetSynchronizedTimeNearUpTicks( &atTime, inHostTime );
		...
		void *										inContext )
{
	...
	params = AirPlayReceiverSessionCreateModesDictionary( inChanges, inReason, &err );
	require_noerr( err, exit );
...
OSStatus	AirTunesDebugControl( const char *inCmd, CFStringRef *outOutput )
{
	...
	if( strncmpx( key, keyLen, "aro" ) == 0 )
	{
		...
		if( gAirTunes ) _LegacyAudioUpdateLatency( gAirTunes );
...
void	AirTunesDebugControl_Tweak( DataBuffer *inDB, const char *inCmd )
{
	...
	if( strncmpx( name, nameLen, "gAirTunesDebugLogAllSkew" ) == 0 )
	{
		if( ( stricmp( value, "yes" ) == 0 ) || ( stricmp( value, "1" ) == 0 ) )	gAirTunesDebugLogAllSkew = 1;
		else																		gAirTunesDebugLogAllSkew = 1;
	...
	if( !inCmd || ( *inCmd == '\0' ) || ( strcmp( inCmd, "globals" ) == 0 ) )
	{
		...
		if( gAirTunes )
		{
			...
			DataBuffer_AppendF( inDB, "    rtp                      = sock %d, local port %d\n", 
				gAirTunes->rtpAudioSock, gAirTunes->rtpAudioPort );
			...
			DataBuffer_AppendF( inDB, "    rtcpTISend, Resp, Ann    = %u, %u, %u\n", 
				src->rtcpTISendCount, src->rtcpTIResponseCount, src->rtcpTIAnnounceCount );
...
OSStatus	AirTunesDebugPerf( int inPollIntervalSeconds, AirPlayReceiverSessionRef inSession )
{
	...
	if( gAirTunesDebugPerfMode == 0 ) // Packet Info
	{
		...
		dlog( kLogLevelMax, "S %3llu bytes, R %6llu bytes, R %4u pkts, E %4hu pkts, L %2u pkts, L %5.2f%%, G %3u, D %.0f secs, T %.0f secs, %3u%% cmp\n", 
			sentDiff, recvDiff, (int) recvCountDiff, seqDiff, (int) lossDiff, lossPercent, (int) glitchDiff, deltaTime, nowFP, 
			inSession->compressionPercentAvg / 100 );
	}
