...
// AIRPLAY_ALAC: 1=Support Apple Lossless compression (ALAC). 0=Don't compile in any Apple Lossless code.

// AIRPLAY_CONFIG_FILES: 1=Include support for using fixed config files. 0=Strip out config file code.

#if( !defined( AIRPLAY_CONFIG_FILES ) )
		#define AIRPLAY_CONFIG_FILES		1
#endif
#if( AIRPLAY_CONFIG_FILES && !defined( AIRPLAY_CONFIG_FILE_PATH ) )
	#define AIRPLAY_CONFIG_FILE_PATH		"/etc/airplay.plist"
#endif

// AIRPLAY_DACP: 1=Enable accessory-side DACP support. 0=Strip out DACP code.

#if( !defined( AIRPLAY_DACP ) )
	#define AIRPLAY_DACP		1
#endif

// AIRPLAY_META_DATA: 1=Enable music meta data support (e.g. album artwork, song titles, etc. 0=Strip out meta data code.

#if( !defined( AIRPLAY_META_DATA ) )
	#define AIRPLAY_META_DATA		1
#endif

// AIRPLAY_METRICS: 1=Enable metrics. 0=Strip out metrics code.

#if( !defined( AIRPLAY_METRICS ) )
	#define AIRPLAY_METRICS		1
#endif

// AIRPLAY_MFI: 1=Use Made for iPod (MFi)-based authentication and encryption. 0=Don't include MFi code.

#if( !defined( AIRPLAY_MFI ) )
	#define AIRPLAY_MFI		0
#endif

// AIRPLAY_PASSWORDS: 1=Include support for password/PIN authorization in receiver. 0=Strip out password/PIN code.

#if( !defined( AIRPLAY_PASSWORDS ) )
	#define AIRPLAY_PASSWORDS		1
#endif

// AIRPLAY_RAOP_SERVER: 1=Enable _raop._tcp Bonjour service for compatibility. 0=Strip out RAOP server code.

#if( !defined( AIRPLAY_RAOP_SERVER ) )
	#define AIRPLAY_RAOP_SERVER			1 // Enable by default for compatibility with sender devices that expect it.
#endif

// AIRPLAY_SDP_SETUP: 1=Include support for ANNOUNCE/SETUP requests using SDP. 0=Strip out SDP-based code.

#if( !defined( AIRPLAY_SDP_SETUP ) )
...
// AIRPLAY_VOLUME_CONTROL: 1=Enable volume control code. 0=Strip out volume control code.

#if( !defined( AIRPLAY_VOLUME_CONTROL ) )
...
#define kAirPlayAudioBufferMainAltWiFiMs	  50 // 50 ms over WiFi.
#define kAirPlayAudioLatencyOther		   88200 // 2 seconds @ 44100 Hz.
#define kAirPlayAudioLatencyScreen		    4410 // 100 ms @ 44100 Hz.
...
#define kAirPlayScreenLatencyWiFiMs		      90 // 90 ms over WiFi.
...
#define kAirPlayStreamType_LegacyAudio		 96	// RTP payload type for high latency (2 seconds), legacy audio output. UDP.
...
#define AirPlayStreamTypeToString( X ) \
	( ( (X) == kAirPlayStreamType_LegacyAudio )		? "LegacyAudio"	: \
...
#define kAirPlayStatusFlag_CloudConnectivity		( 1 << 6 ) // 0x0040: Cloud connectivity. May be stale.
#define kAirPlayStatusFlag_PasswordRequired			( 1 << 7 ) // 0x0080: Password required to use AirPlay.
#define kAirPlayStatusFlag_ConferenceRoomMode		( 1 << 8 ) // 0x0100: Conference room mode is on.
...
#define kAirPlayAudioFormat_All							0x03FFFFFF
...
#define kAirPlayTXTKey_Password						"pw"		// [String] 1=Password required. missing/0=Password not required.
#define kAirPlayTXTKey_PIN							"pin"		// [String] 1=PIN required. missing/0=PIN not required.
...
#if( AIRPLAY_RAOP_CLIENT || AIRPLAY_RAOP_SERVER )
// _raop._tcp -- DEPRECATED: Use _airplay._tcp for all new development. _raop._tcp will go away eventually.
...
/*---------------------------------------------------------------------------------------------------------------------------
	UpdateBonjour: Command to update Bonjour on the AirPlay receiver.
...
/*---------------------------------------------------------------------------------------------------------------------------
	PerformDiagnostic: Performs a diagnostic command.
	
	Request keys
	------------
	kAirPlayKey_Command		-- Command for the accessory to perform (e.g. kAirPlayCommand_ChangeModes).
	[command-specific keys]
	
	Response keys
	-------------
	[Request-specific response keys]
*/
#define kAirPlayCommand_PerformDiagnostic	"performDiagnostic"
...
/*---------------------------------------------------------------------------------------------------------------------------
	...
	kAirPlayKey_StreamID		-- Stream to duck (e.g. main audio). See AirPlayStreamID_* constants.
...
/*---------------------------------------------------------------------------------------------------------------------------
	...
	kAirPlayKey_StreamID	-- Stream to unduck (e.g. main audio). See AirPlayStreamID_* constants.
...
#if( AIRPLAY_DACP )
/*---------------------------------------------------------------------------------------------------------------------------
	SendDACP: Sends a DACP command from the accessory to the controller.
	
	Request keys
	------------
	kAirPlayKey_DACPCommand -- DACP command to send (e.g. "nextitem"). See DACPCommon.h for DACP constants.
	
	No response keys
*/
#define kAirPlayCommand_SendDACP			"sendDACP"

/*---------------------------------------------------------------------------------------------------------------------------
	DACPStatus: Updates the platform when a DACP command completes.
	
	Request keys
	------------
	kAirPlayKey_Status -- Status of the command.
	
	No response keys
*/
#define kAirPlayEvent_DACPStatus			"dacpStatus"
#endif
...
/*---------------------------------------------------------------------------------------------------------------------------
	...
	Response keys
	-------------
	kAirPlayProperty_Modes	-- Latest modes property. See the ModesChanged command for keys, etc.
...
/*---------------------------------------------------------------------------------------------------------------------------
	UpdatePIN: Updates to a new, random PIN required to access AirPlay servers.
	
	No request keys.
	No response keys.
*/
#define kAirPlayCommand_UpdatePIN			"updatePIN"
...
#define kAirPlayFeature_Audio							( 1   <<  9 ) // 0x000000200: Audio.
#define kAirPlayFeature_RedundantAudio					( 1   << 11 ) // 0x000000800: Redundant audio packets to handle loss.
#if( AIRPLAY_META_DATA )
	#define kAirPlayFeature_AudioMetaDataArtwork		( 1   << 15 ) // 0x000008000: Artwork meta data for audio streams.
	#define kAirPlayFeature_AudioMetaDataProgress		( 1   << 16 ) // 0x000010000: Progress meta data for audio streams.
	#define kAirPlayFeature_AudioMetaDataText			( 1   << 17 ) // 0x000020000: Textual meta data for audio streams.
#endif
#define kAirPlayFeature_AudioPCM						( 1   << 18 ) // 0x000040000: Uncompressed PCM audio data.
	#define kAirPlayFeature_AudioAppleLossless			( 1   << 19 ) // 0x000080000: Apple Lossless audio compression.
#define kAirPlayFeature_AudioUnencrypted				( 1   << 22 ) // 0x000400000: Unencrypted audio.
#define kAirPlayFeature_AudioAES_128_MFi_SAPv1			( 1   << 26 ) // 0x004000000: Audio: 128-bit AES key encrypted with MFi-SAPv1.
#if( AIRPLAY_META_DATA )
	#define kAirPlayFeature_UnifiedMetaData				( 1   << 29 ) // 0x020000000: Unified meta data in a single plist request.
#endif
#define kAirPlayFeature_UnifiedBonjour					( 1   << 30 ) // 0x040000000: _airplay._tcp and _raop._tcp are unified (_raop._tcp only legacy).
#define kAirPlayFeature_Reserved						( 1   << 31 ) // 0x080000000: Reserved to avoid client bugs treating this as a sign bit.
...
// [Number] Number of audio bits per sample (e.g. 16 for a 16-bit audio sample).
#define kAirPlayKey_BitDepth			"bits"
...
// [String] Command to perform when embedded within another command (e.g. kAirPlayCommand_PerformDiagnostic).
...
#if( AIRPLAY_DACP )
// [String] DACP command to send (e.g. "nextitem"). See DACPCommon.h for DACP constants.
#define kAirPlayKey_DACPCommand			"dacpCommand"
#endif
...
// [Data] USB-style HID country code.
...
#define kAirPlayKey_PlayPassword		"playPassword"
...
// [Number] Number of audio samples in a frame. Note: if there are 352 stereo samples per frame, this would be 352, not 704.
#define kAirPlayKey_SamplesPerFrame		"spf"
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@group		AirPlay Notifications
	@abstract	Notifications posted by AirPlay.
*/

#define kAirPlayNotification_Active					"com.apple.airplay.active"				// [uint64_t] Value=1 means active. Value=0 means inactive.
#define kAirPlayNotification_Authenticate			"com.apple.airplay.authenticate"		// [no data]  Posting triggers authentication of the current route.
#define kAirPlayNotification_CloudConnectivity		"com.apple.airplay.cloudConnectivity"	// [uint64_t] Value=1 means cloud is reachable. 0=not reachable or unknown.
#define kAirPlayNotification_ConferenceRoomMode		"com.apple.airplay.conferenceRoomMode"	// [uint64_t] Value=1 means conference room mode is on. 0=off.
#define kAirPlayNotification_DeviceAvailability		"com.apple.airplay.deviceAvailability"	// [uint64_t] Value=0 means none. 1=Audio devices. 3=Audio+Screen devices.
	#define kAirPlayDeviceAvailability_Audio			( 1 << 0 ) // At least one audio-capable AirPlay receiver is available.
	#define kAirPlayDeviceAvailability_Screen			( 1 << 1 ) // At least one screen/mirroring-capable AirPlay receiver is available.
#define kAirPlayNotification_EndpointsChanged		"com.apple.airplay.endpointsChanged"	// [no data]  Posted when endpoints are added, removed, or update.
#define kAirPlayNotification_EndpointDeselected		"com.apple.airplay.endpointDeselected"	// [no data]  Posted when an active endpoint is deselected.
#define kAirPlayNotification_Error					"com.apple.airplay.error"				// Posted if an async error occurs. Use client's StartError keys.
#define kAirPlayNotification_FakeForceRefresh		"com.apple.airplay.fakeForceRefresh"	// Fakes one force refresh for AirPlay Screen.
#define kAirPlayNotification_Quiesce				"com.apple.airplay.quiece"				// Posted on errors, etc. to pull route away from AirPlay.
#define kAirPlayNotification_PasswordRequired		"com.apple.airplay.passwordRequired"	// Posted when a password is needed. Data is 32-bit DataSource ID.
#define kAirPlayNotification_PlaybackAllow			"com.apple.airplay.playbackAllow"		// Tell sender playback is not allowed. See dmcp.device-prevent-playback for details.
#define kAirPlayNotification_PlaybackPrevent		"com.apple.airplay.playbackPrevent"		// Tell sender playback is allowed. See dmcp.device-prevent-playback for details.
#define kAirPlayNotification_ProblemDetected		"com.apple.airplay.problemDetected"		// Posted when AirPlay detects a problem with the network, etc.
#define kAirPlayNotification_Route					"com.apple.airplay.route"				// Posted on route changes. State is scaler MAC address.
#define kAirPlayNotification_StartStatus			"com.apple.airplay.startStatus"			// Posted on success or failure. Use client's StartError keys.
#define kAirPlayNotification_State					"com.apple.airplay.state"  				// Posted on changes to route state/type. State contains flags below.
	#define kAirPlayStateFlag_Audio						( 1 << 0 ) // AirPlay audio is active.
	#define kAirPlayStateFlag_Screen					( 1 << 1 ) // AirPlay Screen/Mirroring is active.
#define kAirPlayNotification_Stop					"com.apple.airplay.stop"  				// Posted when AirPlay is stopped by the user.
#define kAirPlayNotification_Suspended				"com.apple.airplay.suspended"  			// Posted when AirPlay suspended or resumed. 1=Suspended. 0=Not suspended.
#define kAirPlayNotification_Unauthorized			"com.apple.airplay.unauthorized"		// Posted if a valid password isn't provided for an AirPlay device.
#define kAirPlayNotification_Unconfigured			"com.apple.airplay.unconfigured"		// [uint64_t] Value=1 means device is unconfigured.
#define kAirPlayNotification_ConnectionState		"com.apple.airplay.connectionState"		// Posted on changes to the connection state for AirPlay Screen.
	#define kAirPlayConnectionState_Disconnected		 0 // Disconnected
	#define kAirPlayConnectionState_QueryingSRV			10 // Querying for SRV record
	#define kAirPlayConnectionState_ResolvingDNS		20 // Resolving DNS
	#define kAirPlayConnectionState_Connecting			30 // Connecting
	#define kAirPlayConnectionState_Connected			40 // Connected
...
// [Number:AirPlayAudioFormat] Bit mask of supported formats. See kAirPlayAudioFormat_*.
#define kAirPlayProperty_AudioInputFormats			"audioInputFormats"
#define kAirPlayProperty_AudioOutputFormats			"audioOutputFormats"

// [Boolean] Indicates to the accessory if audio input should be enabled. Only used for accessory with audio input support.
#define kAirPlayProperty_AudioInputEnabled			"audioInputEnabled"
...
// [Boolean] Tells the accessory to loop the data it receives for output back in as input data to send to the client.
#define kAirPlayProperty_AudioLoopbackEnabled		"audioLoopbackEnabled"
...
// [Boolean] Controls whether diag mode is enabled or not.
#define kAirPlayProperty_DiagMode					"diagMode"
...
// [Data] PNG data for 82x56 pixel icon representing the accessory's logo.
#define kAirPlayProperty_DockIcon					"dockIcon"
#define kAirPlayProperty_DockIconPath				"dockIconPath"
...
#if( AIRPLAY_METRICS )
// [Dictionary] Collection of keys for performance, error, etc. information. See AirPlayMetrics.h.
...
#define kAirPlayProperty_ScreenFPSMax				"screenFPSMax"

// [Data] PNG data for 44x30 pixel icon representing the accessory's logo.
#define kAirPlayProperty_SettingsIcon				"settingsIcon"
#define kAirPlayProperty_SettingsIconPath			"settingsIconPath"

// [Integer] Sets the current amount of skew detected to allow the platform to compensate for it.
#define kAirPlayProperty_Skew						"skew"

// [Boolean] Returns true if the platform supports its own fine-grained skew compensation.
