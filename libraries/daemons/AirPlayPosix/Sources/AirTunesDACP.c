...
// AIRTUNES_DACP_LOG_DMAP -- 1=Log all DMAP content block responses.

#if( !AIRTUNES_DACP_LOG_DMAP )
	#define AIRTUNES_DACP_LOG_DMAP			1
#endif
...
void	AirTunesDACPClient_Delete( AirTunesDACPClientRef inClient )
{
	...
	{
		dlog( kDebugLevelWarning, "### deleting request %p (%s) before running it\n", request, request->command );
		...
		Boolean					inInternal )
{
	...
	{
		dlog( kDebugLevelWarning, "### too many outstanding DACP requests (%d)...rejecting command \"%s\" @ %N\n", 
	...
	{
		dlog( kDebugLevelWarning, "### no DACP controller yet...rejecting command \"%s\"  @ %N\n", inCommand );
	...
	*requestEnd = newRequest;
	...
	dlog( kDebugLevelVerbose, "DACP command \"%s\" scheduled @ %N %s\n", inCommand, inInternal ? "(internal)" : "" );
...
DEBUG_STATIC void *	_AirTunesDACPClient_Thread( void *inArg )
{
	...
	for( ;; )
	{
		...
		if( request->internal )
		{
			if( strcmp( request->command, "stopSession" ) == 0 )
			{
				...
				dlog( kDebugLevelVerbose, "DACP command \"%s\" completed @ %N (internal)\n", request->command );
			}
			else
			{
	...
	dlog( kDebugLevelTrace, "exiting DACP thread\n" );
	return( NULL );
}
...
DEBUG_STATIC OSStatus	_AirTunesDACPClient_SendCommand( AirTunesDACPClientRef inClient, const char *inCommand )
{
	...
	if( inClient->connected )
	{
		...
		{
			dlog( kDebugLevelNotice, "Cleaning up after peer disconnected to a force new connection\n" );
	...
	if( !inClient->connected )
	{
		...
		if( err )
		{
			dlog( kDebugLevelNotice, "### Resolve DACP ID 0x%016llX failed: %#m\n", inClient->dacpID, err );
			goto exit;
		}
		...
		// If we don't have Bonjour support built into getaddrinfo then use Bonjour to get the address list.
			...
			if( err )
			{
				dlog( kDebugLevelNotice, "### failed to resolve addresses for %s: %#m\n", 
		...
		dlog( kDebugLevelNotice, "DACP connecting to %s\n", host );
		
		err = NetSocket_TCPConnect( inClient->netSock, host, inClient->controllerTCPPort, 5 );
		if( err == kCanceledErr ) goto exit;
		if( err )
		{
			dlog( kDebugLevelNotice, "### failed to connect to controller %s:%u: %#m\n", 
		...
		dlog( kDebugLevelTrace, "    DACP connected     to  %s:%u %s\n", inClient->controllerDNSName, 
...
#if( AIRTUNES_DACP_LOG_MESSAGES )
	dlog( kDebugLevelNotice, "********** DACP REQUEST **********\n" );
	dlog( kDebugLevelNotice, "%s", buf );
#endif
...
#if( AIRTUNES_DACP_LOG_MESSAGES )
	dlog( kDebugLevelNotice, "********** DACP RESPONSE **********\n" );
	dlog( kDebugLevelNotice, "%s", header->buf );
#endif
...
exit:
	...
	{
		NetSocket_Disconnect( inClient->netSock, 2 );
		dlog( kDebugLevelTrace, "    DACP disconnected from %s:%u transiently\n", 
			inClient->controllerDNSName, inClient->controllerTCPPort );
	}
	else if( err && inClient->connected )
	{
		dlog( kDebugLevelNotice, "Cleaning up connection after error: %#m\n", err );
		NetSocket_Disconnect( inClient->netSock, 0 );
		inClient->connected = false;
	}
	if( !err )	dlog( kDebugLevelVerbose, "DACP command \"%s\" completed @ %N\n", inCommand );
	else		dlog( kDebugLevelVerbose, "DACP command \"%s\" failed    @ %N: %#m\n", inCommand, err );
	return( err );
}
	...
	if( IsValidSocket( resolverSock ) )
	{
		for( ;; )
		{
			...
			if( err )
			{
				...
				{
					dlog( kDebugLevelNotice, "### DACP resolve addresses timed out, but got some: %s\n", inClient->addrsStr );
