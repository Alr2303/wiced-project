...
// TARGET_HAS_COMMON_CRYPTO: 1=Use CommonCrypto. 0=Use OpenSSL or some other crypto library.
		...
		#define TARGET_HAS_COMMON_CRYPTO_DIGEST		0
#endif
...
// TARGET_RT_LITTLE_ENDIAN
		 ...
		 ( __ARMCC_VERSION && !defined( __BIG_ENDIAN ) )													|| \
		 IL_LITTLE_ENDIAN || defined( __LITTLE_ENDIAN__ ) || defined( EFI_NET_LITTLE_ENDIAN )				|| \
...
// TARGET_RT_BIG_ENDIAN
		 ...
		 ( __ARMCC_VERSION && defined( __BIG_ENDIAN ) )															|| \
		 IL_BIG_ENDIAN || defined( __BIG_ENDIAN__ ) || defined( EFI_NET_BIG_ENDIAN )							|| \
...
//
// - Darwin when not building with BSD headers
// - Windows
...
#else
	#define MD5_HEADER		"CommonServices.h" // Can't think of a better way without messy #if's at the call site.
#endif
...
#else
	#define SHA_HEADER		"CommonServices.h" // Can't think of a better way without messy #if's at the call site.
#endif
...
// Macros to work with Q32.32 numbers. <http://en.wikipedia.org/wiki/Q_(number_format)>
//
...
//===========================================================================================================================
//	Modular Math (AKA Serial Number Arithmetic per RFC 1982)
//===========================================================================================================================
...
/*!	@group		GThreads
	...
	Examples:
		...
		int	main( int argc, const char **argv )
		{
			...
			GTHREAD_START( &thread, MyThread, NULL );
			while( MyThread( &thread, NULL ) != kGState_Done ) {} // Bad to spin wait like this...only here to so how it works.
			return( 0 );
		}
		
		GState	MyThread( GThread *inThread, void *inArg )
		{
			(void) inArg;
			
			GTHREAD_BEGIN( inThread );
	...
	Implementation Details:
		...
		* GTHREAD_END() begins with "do {} while( 0 );" to allow it to be placed immediately after an exit label.
		  Otherwise, some compilers generate warnings about not having a statement after a label.
		
		* GTHREAD_YIELD() has an extra "do {} while( 0 );" to avoid warnings about a label without a statement after it.
*/
...
typedef struct GTopThread	GTopThread;

typedef GState ( *GThreadFunc )( GThread *inThread, void *inArg );
struct GThread
{
	int					pc;
	GTopThread *		topThread;
};
...
#define GTHREAD_START( GTT, FUNC, ARG )		do { (GTT)->gt.pc = 0; (GTT)->gt.topThread = (GTT); (GTT)->func = (FUNC); (FUNC)( &(GTT)->gt, (ARG) ); } while( 0 )
#define GTHREAD_ENTER( GT )					{ GThread * const __GThread = (GT); switch( (GT)->pc ) { case 0:
#define GTHREAD_EXIT()						do {} while( 0 ); } return( kGState_Done ); } do {} while( 0 )
#define GTHREAD_YIELD()						do { __GThread->pc = __LINE__; return( kGState_Waiting ); case __LINE__: do {} while( 0 ); } while( 0 )
#define GTHREAD_CALL( GT, FUNC, ARG )		do { (GT)->pc = 0; (GT)->topThread = __GThread->topThread; GTHREAD_WAIT_UNTIL( (FUNC)( (GT), (ARG) ) == kGState_Done ); } while( 0 )
#define GTHREAD_WAIT_UNTIL( X )				do { __GThread->pc = __LINE__; case __LINE__: if( !(X) ) return( kGState_Waiting ); } while( 0 )
