...
static OSStatus	_NTPClockClientSendRequest( NTPClockRef inClock )
{
	...
	*inClock->transmitLoPtr	= htonl( transmitLo );
	...
	n = send( inClock->clientSock, &inClock->transmitPkt, inClock->transmitLen, 0 );
...
static void *	_NTPClockClientThread( void *inArg )
{
	...
	if( ntpClock->hasThreadPriority ) SetThreadPriority( ntpClock->threadPriority );
...
//===========================================================================================================================
//	NTPClockGetUpTicksNearSynchronizedNTPTimeMid32
//===========================================================================================================================

uint64_t	NTPClockGetUpTicksNearSynchronizedNTPTimeMid32( NTPClockRef inClock, uint32_t inNTPMid32 )
{
	uint64_t now  = NTPClockGetSynchronizedNTPTime( inClock );
	uint64_t then = ( now & UINT64_C( 0xFFFF000000000000 ) ) | ( ( (uint64_t) inNTPMid32 ) << 16 );
	return( ( now > then ) ? ( UpTicks() - NTPtoUpTicks( then - now ) ) : ( UpTicks() + NTPtoUpTicks( then - now ) ) );
}
...
static void *	_NTPClockServerThread( void *inArg )
{
	...
	maxFd = -1;
	if( cmdSock > maxFd ) maxFd = cmdSock;
	if( sockV4  > maxFd ) maxFd = sockV4;
	if( sockV6  > maxFd ) maxFd = sockV6;
	maxFd += 1;
	for( ;; )
	{
		FD_SET( cmdSock, &readSet );
...
static void	_NTPClockServerProcessPacket( NTPClockRef inClock, SocketRef inSock )
{
	...
	*transmitTimeLoPtr	= htonl( lo );
	...
	n = sendto( inSock, &pkt, len, 0, &sip.sa, (socklen_t) sipLen );
