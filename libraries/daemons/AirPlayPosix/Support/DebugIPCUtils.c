...
DEBUG_STATIC void	_IPCAgent_Delete( void *inContext )
{
	...
	dispatch_release( agent->internalQueue );
}
...
DEBUG_STATIC void	_IPCAgent_Start( void *inContext )
{
	...
	memset( &agent->groupAddr.sa4, 0, sizeof( agent->groupAddr.sa4 ) );
	SIN_LEN_SET( &agent->groupAddr.sa4 );
	agent->groupAddr.sa4.sin_family			= AF_INET;
	agent->groupAddr.sa4.sin_port			= htons( kIPCPort );
	agent->groupAddr.sa4.sin_addr.s_addr	= htonl( kIPCAddr );
	...
	source = dispatch_source_create( DISPATCH_SOURCE_TYPE_READ, sock, 0, agent->internalQueue );
...
DEBUG_STATIC void	_IPCAgent_ReadHandler( void *inArg )
{
	...
	for( ;; )
	{
		...
		{
			dlog( kDebugLevelNotice, "[DebugIPC] Socket became defunct\n" );
			dispatch_socket_forget( agent->source, &agent->sock, false );
			err = kNoErr;
			goto exit;
		}
...
DEBUG_STATIC void	_IPCAgent_SendMessage( void *inContext )
{
	...
	do
	{
		...
		n = sendto( agent->sock, &pkt, len, 0, &agent->groupAddr.sa, (socklen_t) sizeof( agent->groupAddr.sa4 ) );
...
exit:
	...
	if( context.lock )		dispatch_release( context.lock );
	if( context.msgSem )	dispatch_release( context.msgSem );
