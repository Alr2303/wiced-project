		...
		va_list 		inArgs )
{
	...
	if( inParent )
	{
		...
		else
		{
			...
			dlog( kDebugLevelError, "%s: parent must be an array, dictionary, or null (typeID=%d)\n", __ROUTINE__, (int) typeID );
			err = kTypeErr;
			goto exit;
		}
	}
	...
	for( c = *fmt; c != '\0'; c = *( ++fmt ) )
	{
		...
		if( c == 'k' )
		{
			...
			switch( c )
			{
				...
				default:
					dlog( kDebugLevelError, "%s: unknown key specifier: %s\n", __ROUTINE__, fmt );
					err = kFormatErr;
					goto exit;
		...
		switch( c )
		{
			...
			default:
				dlog( kDebugLevelError, "%s: unknown format char: %s\n", __ROUTINE__, fmt );
				err = kFormatErr;
				goto exit;
		}
	}
...
OSStatus	CFPropertyListExtractFormattedVAList( CFPropertyListRef inObj, void *outResult, const char *inFormat, va_list inArgs )
{
		...
		if( p[ 0 ] == '%' )			// %k specifiers
		{
			if( p[ 1 ] == 'k' )
			{
				switch( p[ 2 ] )
				{
					...
					default:
						dlog( kDebugLevelError, "%s: unknown %%k specifier: %.*s\n", __ROUTINE__, (int)( q - p ), p );
						err = kFormatErr;
						goto exit;
				}
				p += 3;
			...
			{
				dlog( kDebugLevelError, "%s: unknown specifier: %.*s\n", __ROUTINE__, (int)( q - p ), p );
				err = kFormatErr;
				goto exit;
			}
		}
	...
	if( type )
	{
		...
		else if( strcmp( type, "utf8" ) == 0 )	// utf8: CFString to malloc'd UTF-8
		{
			...
			{
				free( utf8 );
				dlog( kDebugLevelError, "%s: cannot convert to UTF-8: %@\n", __ROUTINE__, value );
				err = kUnexpectedErr;
				goto exit;
			}
		}
		...
		else
		{
			...
			if( *p != '\0' )
			{
				dlog( kDebugLevelError, "%s: unknown conversion type: %s\n", __ROUTINE__, type );
				err = kUnexpectedErr;
				goto exit;
			}
			require_action( i >= 0, exit, err = kRangeErr );
...
OSStatus	CFPropertyListCreateFromANSIFile( FILE *inFile, CFOptionFlags inOptions, CFPropertyListRef *outPlist )
{
	...
	plist = NULL;
	CFBinaryPlistCreateFromData( NULL, CFDataGetBytePtr( data ), (size_t) CFDataGetLength( data ), (void *) &plist );
	if( !plist )
	{
		#if( CFL_XML_PARSING )
			plist = CFPropertyListCreateFromXMLData( NULL, data, inOptions, NULL );
		#else
			(void) inOptions; // Unused
		#endif
		require_action( plist, exit, err = kFormatErr );
	}
...
//===========================================================================================================================
//	CFPropertyListCreateFromData
//===========================================================================================================================

OSStatus	CFPropertyListCreateFromData( const void *inData, size_t inSize, CFOptionFlags inOptions, CFPropertyListRef *outPlist )
{
	OSStatus				err;
	CFPropertyListRef		plist;
	
	plist = NULL;
	CFBinaryPlistCreateFromData( NULL, inData, inSize, (void *) &plist );
	if( !plist )
	{
		#if( CFL_XML_PARSING )
			CFDataRef		data;
			
				data = CFDataCreate( NULL, (const uint8_t *) inData, (CFIndex) inSize );
				require_action( data, exit, err = kNoMemoryErr );
		...
		FILE *				inFile )
{
	...
	{
		data = CFPropertyListCreateXMLData( kCFAllocatorDefault, inPlist );
		require_action( data, exit, err = kNoMemoryErr );
	}
#endif
...
OSStatus	CFCreateObjectFromStringEx( const char *inStr, const char *inType, CFTypeRef *outObj )
{
	...
	{
		err = HexTextToData( inStr, kSizeCString, 0, NULL, &len );
		require_noerr_quiet( err, exit );
		...
		err = HexTextToData( inStr, kSizeCString, len, CFDataGetMutableBytePtr( data ), &len );
		require_noerr( err, exit );
...
char *	CFGetCString( CFTypeRef inObj, char *inBuf, size_t inMaxLen )
{
	...
	if( !inObj ) { strlcpy( inBuf, "", inMaxLen ); return( inBuf ); }
	...
	else if( typeID == CFDateGetTypeID() )
	{
		...
		const char *		amPMStr;
			...
			CFLDateGetDate( (CFDateRef) inObj, &components );
...
uint8_t *	CFGetData( CFTypeRef inObj, void *inBuf, size_t inMaxLen, size_t *outLen, OSStatus *outErr )
{
	...
	else if( typeID == CFStringGetTypeID() )
	{
		...
		err = CFStringGetOrCopyCStringUTF8( (CFStringRef) inObj, &utf8Ptr, &utf8Storage );
		require_noerr( err, exit );
		
		HexToData( utf8Ptr, strlen( utf8Ptr ), kHexToData_IgnoreWhitespace | kHexToData_IgnoreDelimiters, 
...
OSStatus	CFDictionaryCreateFromNameTypeValueArgList( CFDictionaryRef *outDict, int inArgI, int inArgC, const char *inArgV[] )
{
	...
	while( inArgI < inArgC )
	{
		...
		else if( strncmpx( type, (size_t)( typeEnd - type ), "h" ) == 0 )	// Hex string
		{
			...
			err = HexTextToData( value, kSizeCString, sizeof( buf ), buf, &len );
			require_noerr_quiet( err, exit );
...
#define	kCFZLibHeaderSize			( (size_t) 8 )
#define	kCFZLibSignature			"cflz"
#define	kCFZLibSignatureSize		sizeof_string( kCFZLibSignature )

OSStatus	CFZLibCompress( const void *inData, size_t inSize, void *outCompressedData, size_t *outCompressedSize )
{
	OSStatus		err;
	uint8_t *		dst;
	uint8_t *		p;
	...
	// Allocate header and compression buffer. ZLib requires the buffer to be at least 0.1% larger than the source + 12 bytes.
	
	compressedSize = (uLongf)( inSize + ( ( inSize / 10 ) + 12 ) );
	dst = (uint8_t *) malloc( kCFZLibHeaderSize + compressedSize );
	require_action( dst, exit, err = kNoMemoryErr );
	p = dst;
	
	// Write the header (4 byte signature and 4 byte expanded size in network byte order).
	
	*p++ = (uint8_t)( kCFZLibSignature[ 0 ] );
	*p++ = (uint8_t)( kCFZLibSignature[ 1 ] );
	*p++ = (uint8_t)( kCFZLibSignature[ 2 ] );
	*p++ = (uint8_t)( kCFZLibSignature[ 3 ] );
	
	*p++ = (uint8_t)( ( inSize >> 24 ) & 0xFF );
	*p++ = (uint8_t)( ( inSize >> 16 ) & 0xFF );
	*p++ = (uint8_t)( ( inSize >>  8 ) & 0xFF );
	*p++ = (uint8_t)(   inSize         & 0xFF );
	
	// Compress the data.
	
	err = compress2( (Bytef *) p, &compressedSize, (Bytef *) inData, (uLongf) inSize, Z_BEST_COMPRESSION );
	require_noerr( err, exit );
	compressedSize += kCFZLibHeaderSize;
	
	// Compression succeeded. Shrink the buffer to free unused space.
	
	if( outCompressedData )
	{
		uint8_t *		tmp;
		
		tmp = (uint8_t *) realloc( dst, compressedSize );
		require_action( tmp, exit, err = kSizeErr );
		dst = tmp;
		
		*( (uint8_t **) outCompressedData ) = dst;
		dst = NULL;
	}
	if( outCompressedSize ) *outCompressedSize = compressedSize;
...
#if( CF_ZLIB )
OSStatus	CFZLibExpand( const void *inData, size_t inSize, void *outExpandedData, size_t *outExpandedSize )
{
	OSStatus			err;
	const uint8_t *		src;
	uLongf				size;
	...
	// Check the header and extract the expanded size (in network byte order) from it.
	
	src = (const uint8_t *) inData;
	require_action_quiet( inSize >= kCFZLibHeaderSize, exit, err = kMalformedErr );
	require_action_quiet( ( src[ 0 ] == kCFZLibSignature[ 0 ] ) ||
						  ( src[ 1 ] == kCFZLibSignature[ 1 ] ) ||
						  ( src[ 2 ] == kCFZLibSignature[ 2 ] ) ||
						  ( src[ 3 ] == kCFZLibSignature[ 3 ] ), 
						  exit, err = kMismatchErr );
	size = (uLongf)( ( src[ 4 ] << 24 ) | 
					 ( src[ 5 ] << 16 ) | 
					 ( src[ 6 ] <<  8 ) | 
					 ( src[ 7 ] <<  0 ) );
	
	// Allocate the expansion buffer and expand the data into it.
	...
	if( size > 0 )
	{
		...
		err = uncompress( (Bytef *) dst, &size, src + kCFZLibHeaderSize, (uLongf)( inSize - kCFZLibHeaderSize ) );
		require_noerr( err, exit );
	}
	else
	{
		// Size is zero so just return a 1 byte buffer since it's undefined if malloc( 0 ) returns a valid ptr.
	...
	if( outExpandedData )
	{
		*( (void **) outExpandedData ) = dst;
		dst = NULL;
	}
	if( outExpandedSize ) *outExpandedSize = size;
	err = kNoErr;
