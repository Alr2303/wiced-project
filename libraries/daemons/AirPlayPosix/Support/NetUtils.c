...
OSStatus	DrainUDPSocket( SocketRef inSock, int inTimeoutMs, int *outDrainedPackets )
{
	...
	deadlineTicks = UpTicks() + MillisecondsToUpTicks( inTimeoutMs );
...
OSStatus	OpenSelfConnectedLoopbackSocket( SocketRef *outSock )
{
	...
	memset( &sip.sa4, 0, sizeof( sip.sa4 ) );
	sip.sa4.sin_family		= AF_INET;
	sip.sa4.sin_port		= 0;
	sip.sa4.sin_addr.s_addr	= htonl( INADDR_LOOPBACK );
	err = bind( sock, &sip.sa, sizeof( sip.sa4 ) );
	err = map_socket_noerr_errno( sock, err );
	require_noerr( err, exit );
...
OSStatus	SendSelfConnectedLoopbackMessage( SocketRef inSock, const void *inMsg, size_t inLen )
{
	...
	n = sendto( inSock, (char *) inMsg, inLen, 0, &sip.sa, (socklen_t) sizeof( sip.sa4 ) );
...
OSStatus	SendUDPQuitPacket( int inPort, SocketRef inSockV4, SocketRef inSockV6 )
{
	...
	if( IsValidSocket( inSockV4 ) )
	{
		...
		SIN_LEN_SET( &sip.sa4 );
		sip.sa4.sin_family		= AF_INET;
		sip.sa4.sin_port		= htons( (uint16_t) inPort );
		sip.sa4.sin_addr.s_addr	= htonl( INADDR_LOOPBACK );
		
		n = sendto( inSockV4, "", 0, 0, &sip.sa, (socklen_t) sizeof( sip.sa4 ) );
	...
	{
		memset( &sip.sa6, 0, sizeof( sip.sa6 ) );
		SIN6_LEN_SET( &sip.sa6 );
		sip.sa6.sin6_family	= AF_INET6;
		sip.sa6.sin6_port	= htons( (uint16_t) inPort );
		sip.sa6.sin6_addr	= in6addr_loopback;
		
		n = sendto( inSockV6, "", 0, 0, &sip.sa, (socklen_t) sizeof( sip.sa6 ) );
		...
		SocketRef *	outSock )
{
	...
	if( inFamily == AF_INET )
	{
		...
		SIN_LEN_SET( &sip.sa4 );
		sip.sa4.sin_family		= AF_INET;
		sip.sa4.sin_port		= htons( (uint16_t) port );
		sip.sa4.sin_addr.s_addr	= htonl( INADDR_ANY );
		err = bind( sock, &sip.sa, (socklen_t) sizeof( sip.sa4 ) );
		err = map_socket_noerr_errno( sock, err );
		if( err && ( inPort < 0 ) )
		{
			sip.sa4.sin_port = 0;
	...
	else if( inFamily == AF_INET6 )
	{
		...
		SIN6_LEN_SET( &sip.sa6 );
		sip.sa6.sin6_family	= AF_INET6;
		sip.sa6.sin6_port	= htons( (uint16_t) port );
		sip.sa6.sin6_addr	= in6addr_any;	
		err = bind( sock, &sip.sa, (socklen_t) sizeof( sip.sa6 ) );
		err = map_socket_noerr_errno( sock, err );
		if( err && ( inPort < 0 ) )
		{
			sip.sa6.sin6_port = 0;
		...
		SocketRef *		outSock )
{
	...
	if( inListenPort != -1 )
	{
		...
		if( inFamily == AF_INET )
		{
			...
			SIN_LEN_SET( &sip.sa4 );
			sip.sa4.sin_family		= AF_INET;
			sip.sa4.sin_port		= htons( (uint16_t) inListenPort );
			sip.sa4.sin_addr.s_addr	= htonl( INADDR_ANY );
			len = sizeof( sip.sa4 );
		}
		#if( defined( AF_INET6 ) )
		else if( inFamily == AF_INET6 )
		{
			...
			SIN6_LEN_SET( &sip.sa6 );
			sip.sa6.sin6_family		= AF_INET6;
			sip.sa6.sin6_port		= htons( (uint16_t) inListenPort );
			sip.sa6.sin6_flowinfo	= 0;
			sip.sa6.sin6_addr		= in6addr_any;
			sip.sa6.sin6_scope_id	= 0;
			len = sizeof( sip.sa6 );
		}
		#endif
		else
		...
		void *						inContext )
{
	...
	if( inErrorCode == kDNSServiceErr_NoError )
	{
		...
		#if( defined( AF_INET6 ) )
		{
			...
			if( ( newSIP->sa.sa_family == AF_INET6 ) && IN6_IS_ADDR_LINKLOCAL( &newSIP->sa6.sin6_addr ) )
			{
				newSIP->sa6.sin6_scope_id = inIFI;
			}
		}
		#endif
		...
		if( ( sip->sa.sa_family == AF_INET ) && !SockAddrIsLinkLocal( sip ) )
		{
			for( next = &context->results; ( curr = *next ) != NULL; next = &curr->ai_next )
			{
				...
				#if( defined( AF_INET6 ) )
					if( !( ( tempSIP->sa.sa_family == AF_INET6 ) && IN6_IS_ADDR_LINKLOCAL( &tempSIP->sa6.sin6_addr ) ) &&
		...
		#if( defined( AF_INET6 ) )
		else if( ( sip->sa.sa_family == AF_INET6 ) && IN6_IS_ADDR_LINKLOCAL( &sip->sa6.sin6_addr ) )
		{
		...
		void *							inContext )
{
	...
	for( ptr = inHostList; *ptr != '\0'; ptr = end )
	{
		...
		if( err == kNoErr )
		{
			...
			err = __NetSocket_Connect( inSock, nativeSock, &sip, inTimeoutSecs );
			if( err == kNoErr ) break;
		...
		#if( NETUTILS_HAVE_GETADDRINFO )
		{
			...
			if( port > 0 )
			{
				...
				for( ai = aiList; ai; ai = ai->ai_next )
				{
					...
					err = __NetSocket_Connect( inSock, nativeSock, ai->ai_addr, inTimeoutSecs );
					if( err == kNoErr ) break;
		...
		int32_t			inTimeoutSecs )
{
	...
	tempInt = 1;
	setsockopt( inNativeSock, IPPROTO_TCP, TCP_NODELAY, &tempInt, (socklen_t) sizeof( tempInt ) );
		...
		int32_t			inTimeoutSecs )
{
	...
	while( remaining > 0 )
	{
		n = recv( inSock->nativeSock, bufferPtr, (int) remaining, inFlags );
		if( n > 0 )
		{
			bufferPtr += n;
			remaining -= n;
...
OSStatus	SocketAccept( SocketRef inSock, int inSeconds, SocketRef *outSock, sockaddr_ip *outAddr )
{
	...
	deadlineTicks = ( inSeconds >= 0 ) ? ( UpTicks() + SecondsToUpTicks( inSeconds ) ) : kUpTicksForever;
	len = 0;
	FD_ZERO( &readSet );
	for( ;; )
	{
		...
		if( !err )
		{
			...
			option = 1;
			setsockopt( newSock, IPPROTO_TCP, TCP_NODELAY, &option, (socklen_t) sizeof( option ) );
			break;
		}
...
OSStatus	SocketConnect( SocketRef inSock, const void *inSockAddr, int inSeconds )
{
	...
	tempInt = 1;
	setsockopt( inSock, IPPROTO_TCP, TCP_NODELAY, &tempInt, (socklen_t) sizeof( tempInt ) );
...
OSStatus	SocketReadAll( SocketRef inSock, void *inData, size_t inSize )
{
	...
	while( dst < lim )
	{
		n = recv( inSock, (char *) dst, (int)( lim - dst ), 0 );
		if( n > 0 )
		{
			dst += n;
		}
...
OSStatus	SocketWriteAll( SocketRef inSock, const void *inData, size_t inSize, int32_t inTimeoutSecs )
{
	...
	while( src < end )
	{
		...
		n = send( inSock, (char *) src, (int)( end - src ), 0 );
		err = map_socket_value_errno( inSock, n >= 0, n );
		if( err == EINTR ) continue;
		require_noerr( err, exit );
...
OSStatus	SocketReadData( SocketRef inSock, void *inBuffer, size_t inSize, size_t *ioOffset )
{
	...
	do
	{
		n = read_compat( inSock, (char *) dst, (int) remaining );
		err = map_socket_value_errno( inSock, n >= 0, n );
...
DEBUG_STATIC void	SocketSendFile_UpdateIOVec( iovec_t **ioArray, int *ioCount, off_t *ioAmount );
...
int	SocketGetBufferSize( SocketRef inSock, int inWhich, OSStatus *outErr )
{
	...
	err = getsockopt( inSock, SOL_SOCKET, inWhich, &bufferSize, &len );
	err = map_socket_noerr_errno( inSock, err );
	require_noerr( err, exit );
		...
		NetTransportType *	outTransportType )
{
	...
	if( !inIfName )
	{
		...
		if( outMACAddress )
		{
				err = GetInterfaceMACAddress( inIfName, outMACAddress );
				require_noerr( err, exit );
		}
	}
	...
	if( outMedia || outTransportType )
	{
			...
			err = map_socket_noerr_errno( inSock, err );
			ifmedia = !err ? ifmr.ifm_current : 0;
...
#if( defined( IP_RECVIF ) )
	err = setsockopt( inSock, IPPROTO_IP, IP_RECVIF, &inEnable, (socklen_t) sizeof( inEnable ) );
	err = map_socket_noerr_errno( inSock, err );
	require_noerr( err, exit );
#else
...
uint64_t	SocketGetPacketUpTicks( struct msghdr *inPacket )
{
	...
	for( cmPtr = CMSG_FIRSTHDR( inPacket ); cmPtr; cmPtr = CMSG_NXTHDR( inPacket, cmPtr ) )
	{
			...
			if( ( cmPtr->cmsg_level == SOL_SOCKET ) && ( cmPtr->cmsg_type == SCM_TIMESTAMP ) )
			{
				...
				trueTicks  = UpTicks();
				fakeTicks  = SecondsToUpTicks( tv.tv_sec ) + MicrosecondsToUpTicks( tv.tv_usec );
				deltaTicks = (int64_t)( fakeTicks - trueTicks );
				
				memcpy( &tv, CMSG_DATA( cmPtr ), sizeof( tv ) );
				ticks = SecondsToUpTicks( tv.tv_sec ) + MicrosecondsToUpTicks( tv.tv_usec );
				ticks -= deltaTicks;
				return( ticks );
			}
		#endif
...
OSStatus	SocketSetQoS( SocketRef inSock, int inQoS )
{
	...
	{
		#if( defined( IP_TOS ) )
			err = setsockopt( inSock, IPPROTO_IP, IP_TOS, &diffServ, (socklen_t) sizeof( diffServ ) );
			err = map_socket_noerr_errno( inSock, err );
	...
	else if( family == AF_INET6 )
	{
			...
			err = setsockopt( inSock, IPPROTO_IPV6, IPV6_TCLASS, &diffServ, (socklen_t) sizeof( diffServ ) );
			err = map_socket_noerr_errno( inSock, err );
		...
		Boolean			inJoin )
{
	...
	if( sip->sa.sa_family == AF_INET )
	{
		...
		mreq.imr_multiaddr = sip->sa4.sin_addr;
		if( inIfName )
		{
			memset( &ifr, 0, sizeof( ifr ) );
			strlcpy( ifr.ifr_name, inIfName, sizeof( ifr.ifr_name ) );
		...
		if( inJoin )
		{
			setsockopt( inSock, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq, (socklen_t) sizeof( mreq ) );
			err = setsockopt( inSock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, (socklen_t) sizeof( mreq ) );
			err = map_socket_noerr_errno( inSock, err );
			require_noerr( err, exit );
		}
		else
		{
			err = setsockopt( inSock, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq, (socklen_t) sizeof( mreq ) );
			err = map_socket_noerr_errno( inSock, err );
			require_noerr( err, exit );
		}
	}
	...
	else if( sip->sa.sa_family == AF_INET6 )
	{
		...
		mreq.ipv6mr_multiaddr = sip->sa6.sin6_addr;
		...
		if( inJoin )
		{
			setsockopt( inSock, IPPROTO_IPV6, IPV6_LEAVE_GROUP, &mreq, (socklen_t) sizeof( mreq ) );
			err = setsockopt( inSock, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, (socklen_t) sizeof( mreq ) );
			err = map_socket_noerr_errno( inSock, err );
			require_noerr( err, exit );
		}
		else
		{
			err = setsockopt( inSock, IPPROTO_IPV6, IPV6_LEAVE_GROUP, &mreq, (socklen_t) sizeof( mreq ) );
			err = map_socket_noerr_errno( inSock, err );
			require_noerr( err, exit );
		}
	}
...
OSStatus	SocketSetMulticastInterface( SocketRef inSock, const char *inIfName, uint32_t inIfIndex )
{
	...
	if( family == AF_INET )
	{
		...
		err = setsockopt( inSock, IPPROTO_IP, IP_MULTICAST_IF, addr, (socklen_t) sizeof( *addr ) );
		err = map_socket_noerr_errno( inSock, err );
		require_noerr( err, exit );
	}
#if( defined( AF_INET6 ) )
	...
	{		
		...
		err = setsockopt( inSock, IPPROTO_IPV6, IPV6_MULTICAST_IF, &inIfIndex, (socklen_t) sizeof( inIfIndex ) );
		err = map_socket_noerr_errno( inSock, err );
		require_noerr( err, exit );
	}
#endif
...
OSStatus	SocketSetMulticastLoop( SocketRef inSock, Boolean inEnableLoopback )
{
	...
	if( family == AF_INET )
	{
		...
		err = setsockopt( inSock, IPPROTO_IP, IP_MULTICAST_LOOP, &option, (socklen_t) sizeof( option ) );
		err = map_socket_noerr_errno( inSock, err );
		require_noerr( err, exit );
	}
#if( defined( AF_INET6 ) )
	else if( family == AF_INET6 )
	{
		...
		option = inEnableLoopback;
		err = setsockopt( inSock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &option, (socklen_t) sizeof( option ) );
		err = map_socket_noerr_errno( inSock, err );
		require_noerr( err, exit );
	}
#endif
...
OSStatus	SockAddrSimplify( const void *inSrc, void *outDst )
{
	...
	else if( family == AF_INET6 )
	{
		...
		else if( IN6_IS_ADDR_LINKLOCAL( &sa6->sin6_addr ) )
		{
			...
			{
				sa6Copy.sin6_scope_id = ( sa6Copy.sin6_addr.s6_addr[ 2 ] << 8 ) | sa6Copy.sin6_addr.s6_addr[ 3 ];
			}
...
uint64_t	SockAddrToDeviceID( const void *inSockAddr )
{
	...
	if( sip.sa.sa_family == AF_INET )
	{
		...
		deviceID |= ntohl( sip.sa4.sin_addr.s_addr );
	}
#if( defined( AF_INET6 ) )
	else if( sip.sa.sa_family == AF_INET6 )
	{
		deviceID = UINT64_C( 0x0600000000000000 ); // 6 = IPv6 address type.
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[  9 ] ) ) << 48;
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[ 10 ] ) ) << 40;
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[ 11 ] ) ) << 32;
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[ 12 ] ) ) << 24;
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[ 13 ] ) ) << 16;
		deviceID |= ( (uint64_t)( sip.sa6.sin6_addr.s6_addr[ 14 ] ) ) <<  8;
		deviceID |=               sip.sa6.sin6_addr.s6_addr[ 15 ];
	}
#endif
	else
	{
...
DEBUG_STATIC Boolean	__EqualBits( const void *inA, const void *inB, uint8_t inBitCount );
...
Boolean	IsIPv4MartianAddress( uint32_t inAddr )
{
	...
	for( ; src < end; ++src )
	{
		if( __EqualBits( &inAddr, src->addr, src->prefix ) )
			return( true );
	}
	return( false );
}
...
Boolean	IsIPv6MartianAddressEx( const void *inAddr, uint32_t inFlags )
{
	...
	for( i = 0; i < countof( kMartianRules ); ++i )
	{
		...
		if( __EqualBits( inAddr, rule->addr, rule->prefix ) )
		{
