...
DEBUG_STATIC OSStatus	__CFRunLoopRunOne( CFRunLoopRef inRL, struct timeval *inTimeout, Boolean inReturnAfterSourceHandled )
{
	...
	if( inTimeout )
	{
		...
		timeoutTicks +=   ( inTimeout->tv_sec  * ticksPerSec );
		timeoutTicks += ( ( inTimeout->tv_usec * kMicrosecondsPerSecond ) / ticksPerSec );
	}
	else
	{
...
DEBUG_STATIC void	__CFRunLoopCommandSocketHandler( CFRunLoopRef inRL )
{
	...
	{
		dlog( kDebugLevelError, "%s: ### unknown control cmd '%c'\n", __ROUTINE__, cmd );
		goto exit;
	}
		...
		const CFFileDescriptorContext *		inContext )
{
	...
	else						flags &= ~kCFSocketCloseOnInvalidate;
