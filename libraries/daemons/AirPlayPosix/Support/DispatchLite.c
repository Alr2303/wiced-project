...
DEBUG_STATIC OSStatus	__LibDispatch_KQueueDispatchAsync( dispatch_function_t inFunction, void *inContext )
{
	...
	err = map_errno( n == (ssize_t) sizeof( itemData ) );
	require_noerr( err, exit );
		...
		switch( ke.filter )
		{
			case EVFILT_READ:
				if( ke.ident == (uintptr_t) gDispatchKQueue_PipeRecv )
				{
					...
					err = map_errno( n == (ssize_t) sizeof( dispatchItemData ) );
					check_noerr( err );
					if( err ) break;
...
DEBUG_STATIC void	__LibDispatch_SelectDrain( void *inArg )
{
	...
	for( ;; )
	{
		...
		for( curr = gDispatchSelect_ReadWriteList; curr; curr = curr->armedNext )
		{
			...
			if(      curr->type == DISPATCH_SOURCE_TYPE_READ )  FD_SET( curr->u.rw.fd, &gDispatchSelect_ReadSet );
			else if( curr->type == DISPATCH_SOURCE_TYPE_WRITE ) FD_SET( curr->u.rw.fd, &gDispatchSelect_WriteSet );
		}
