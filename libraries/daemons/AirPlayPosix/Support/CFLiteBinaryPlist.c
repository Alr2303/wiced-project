...
static OSStatus	_WriteObject( CFBinaryPlistContext *inContext, CFTypeRef inObj )
{
	...
	else if( type == gCFDateType )			// Date
	{
		...
		CFDateGetComponents( (CFDateRef) inObj, &year, &month, &day, &hour, &minute, &second );
...
OSStatus	CFBinaryPlistCreateFromData( CFAllocatorRef inAllocator, const void *inData, size_t inSize, void *outCFObj )
{
	...
	*( (const void **) outCFObj ) = obj; obj = NULL;
		...
		OSStatus *				outErr )
{
	...
	switch( marker & 0xF0 )
	{
		...
		case kCFLBinaryPlistMarkerInt:
			count = 1 << ( marker & 0x0F );
			require_action_quiet( count <= 8, exit, err = kSizeErr );
			require_action_quiet( count <= ( (size_t)( inEnd - ptr ) ), exit, err = kSizeErr );
			v64.u64 = 0;
			while( count-- > 0 ) v64.u64 = ( v64.u64 << 8 ) | *ptr++;
