 ...
 * @param[in] saved_params - pointer to saved parameters
 *
 * @return - WICED_SUCCESS
 *           WICED_ERROR
 */
static wiced_result_t wiced_ota2_network_settings_restore( wiced_interface_t interface, wiced_ota2_saved_network_params_t* saved_params )
{
    ...
    {
        /* no changes to wifi needed for ethernet interface */
        return WICED_SUCCESS;
    }
    ...
    /* Restore APSTA */
    retval = wwd_wifi_set_iovar_value(IOVAR_STR_APSTA, saved_params->APSTA, interface);
    ...
    /* Restore AP */
    CHECK_RETURN(wwd_wifi_set_ioctl_value(WLC_SET_AP, saved_params->AP, interface));


    /* Get Minimum Power Consumption */
    CHECK_RETURN(wwd_wifi_set_iovar_value(IOVAR_STR_MPC, saved_params->MPC, interface));
    ...
    /* Restore rmc_ackreq */
    retval = wwd_wifi_set_iovar_value(IOVAR_STR_RMC_ACKREQ, saved_params->rmc_ackreq, interface);
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
...
wiced_result_t wiced_ota2_network_down( wiced_interface_t interface, wiced_ota2_saved_network_params_t* saved_params )
{
    ...
    return wiced_ota2_network_settings_restore(interface, saved_params);
}
 ...
 *
 * @param[in]  interface    - wiced_interface_t type (WICED_STA_INTERFACE or WICED_ETHERNET_INTERFACE)
 * @param[out] saved_params - pointer to location to save wifi parameters
 * @param[in]  channel      - channel we expect AP to be on
 *
 * @return - WICED_SUCCESS
 *           WICED_ERROR
 */
static wiced_result_t wiced_ota2_network_settings_for_ota2(wiced_interface_t interface, wiced_ota2_saved_network_params_t* saved_params, uint32_t channel)
{
    wiced_result_t  result = WICED_SUCCESS;
    wwd_result_t    retval;
    ...
    retval = wwd_wifi_get_iovar_value(IOVAR_STR_APSTA, &saved_params->APSTA, interface );
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
    ...
    /* Get current AP */
    saved_params->AP = 0;
    retval = wwd_wifi_get_ioctl_value(WLC_GET_AP, &saved_params->AP, interface);
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
    ...
    /* Get Minimum Power Consumption */
    saved_params->MPC = 0;
    retval = wwd_wifi_get_iovar_value(IOVAR_STR_MPC, &saved_params->MPC, interface );
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
    ...
    /* Set MPC to 0 */
    CHECK_RETURN(wwd_wifi_set_iovar_value(IOVAR_STR_MPC, 0, interface));
    ...
    if (channel != 0)
    {
        CHECK_RETURN(wwd_wifi_set_ioctl_value(WLC_SET_CHANNEL, channel, interface));
    ...
    retval = wwd_wifi_get_iovar_value(IOVAR_STR_RMC_ACKREQ, &saved_params->rmc_ackreq, interface );
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
    ...
    retval = wwd_wifi_set_iovar_value(IOVAR_STR_RMC_ACKREQ, 0, interface);
    if ((retval != WWD_SUCCESS) && (retval != WWD_UNSUPPORTED))
    {
...
wiced_result_t wiced_ota2_network_up( wiced_interface_t interface, wiced_config_ap_entry_t* ap_info, wiced_ota2_saved_network_params_t* saved_params )
{
    wiced_result_t  result = WICED_SUCCESS;
    int             tries = 0;
    ...
    result = wiced_ota2_network_settings_for_ota2(interface, saved_params, (uint32_t)ap_info->details.channel);
    if (result != WICED_SUCCESS)
    {
        OTA2_LIB_PRINTF(("wiced_ota2_network_up() failed! %d\r\n", result));
        wiced_ota2_network_settings_restore( interface, saved_params );
        return result;
    }
