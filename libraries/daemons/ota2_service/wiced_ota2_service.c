...
#define WAIT_FOR_APP_TO_SHUTDOWN_NETWORK_MS        (1000)   /* After sending a OTA2_SERVICE_STATE_TIME_TO_CHECK_FOR_UPDATES callback notification,
...
typedef struct wiced_ota2_service_session_s
{
        ...
        int                             default_ap_failures;
            ...
            /* save the current network parameters so we can restore them when we are done */
        wiced_ota2_saved_network_params_t   saved_network_parameters;
...
static wiced_result_t ota2_service_start_timer(wiced_ota2_service_session_t* session, wiced_time_t time)
{
    ...
    {
        OTA2_LIB_PRINTF(("START timer wrong thread\r\n"));
    }
...
static void ota2_service_stop_timer(wiced_ota2_service_session_t* session)
{
    ...
    if (wiced_rtos_is_current_thread( &session->ota2_worker_thread ) != WICED_SUCCESS)
    {
        OTA2_LIB_PRINTF(("STOP timer wrong thread\r\n"));
    }
...
static void ota2_service_stop_retry_timer(wiced_ota2_service_session_t* session)
{
    ...
    if (wiced_rtos_is_current_thread( &session->ota2_worker_thread ) != WICED_SUCCESS)
    {
        OTA2_LIB_PRINTF(("STOP retry timer wrong thread\r\n"));
    }
...
static wiced_result_t ota2_service_start_download_now_timer(wiced_ota2_service_session_t* session, wiced_time_t time)
{
    ...
    if (wiced_rtos_is_current_thread( &session->ota2_worker_thread ) != WICED_SUCCESS)
    {
        OTA2_LIB_PRINTF(("START download now timer wrong thread\r\n"));
    }
...
static void ota2_service_stop_download_now_timer(wiced_ota2_service_session_t* session)
{
    ...
    if (wiced_rtos_is_current_thread( &session->ota2_worker_thread ) != WICED_SUCCESS)
    {
        OTA2_LIB_PRINTF(("STOP download now timer wrong thread\r\n"));
    }
...
static wiced_result_t wiced_ota2_service_network_switch_to_ota2_ap(wiced_ota2_service_session_t* session )
{
    ...
    {
        OTA2_LIB_PRINT(session, OTA2_LOG_WARNING, ("wiced_ota2_service_network_switch_to_ota2_ap() No network to switch to.\r\n"));
        return WICED_SUCCESS;
    }
    ...
    if (session->curr_network != NULL)
    {
        ...
        result = wiced_ota2_network_down(session->ota2_interface, &session->saved_network_parameters) ;
    ...
    result = wiced_ota2_network_up(session->ota2_interface, session->ota2_ap_info, &session->saved_network_parameters) ;
...
static wiced_result_t wiced_ota2_service_network_switch_to_default_ap(wiced_ota2_service_session_t* session )
{
    ...
    {
        OTA2_LIB_PRINT(session, OTA2_LOG_WARNING, ("wiced_ota2_service_network_switch_to_default_ap() NO network to switch to.\r\n"));
    ...
    result = wiced_ota2_network_up(session->ota2_interface, session->default_ap_info, &session->saved_network_parameters ) ;
    if (result == WICED_SUCCESS)
    {
...
static wiced_result_t ota2_service_connect(wiced_ota2_service_session_t* session)
{
    ...
    {
        result =  dns_client_hostname_lookup( host_name, &ip_address, 10000, WICED_STA_INTERFACE );
        if (result!= WICED_SUCCESS)
        {
        ...
        else if (reply_packet != NULL)
        {
                ...
                !((offset == 0) && (strncasecmp( (char*)body, "HTTP", 4) == 0)) )
            {
                ...
                if (req_buffer == NULL)
                {
                   ...
                   OTA2_LIB_PRINT(session, OTA2_LOG_WARNING, ("%s (%ld%%)\r", bar_graph, percent_done));
                }
#endif
...
wiced_result_t wiced_ota2_service_get_the_update(wiced_ota2_service_session_t* session)
{
    ...
    {
        result = wiced_ota2_network_up(session->ota2_interface, session->ota2_ap_info, &session->saved_network_parameters) ;
        OTA2_LIB_PRINT(session, OTA2_LOG_INFO, ("wiced_ota2_network_up( ethernet ) result %d!\r\n", result));
        if (result != WICED_SUCCESS)
        {
    ...
    if (session->ota2_ap_list != NULL)
    {
        ...
        OTA2_LIB_PRINT(session, OTA2_LOG_INFO, ("wiced_ota2_service_get_the_update() Use the ota2 list count:%d!\r\n", session->ota2_ap_list_count ));
        ...
        if (result != WICED_SUCCESS)
        {
            if (ap_failures > 0)
            {
                OTA2_LIB_PRINT(session, OTA2_LOG_ERROR, ("wiced_ota2_service_get_the_update() wiced_ota2_service_network_switch_to_ota2_ap() failed (list)!\r\n"));
            ...
            {
                OTA2_LIB_PRINT(session, OTA2_LOG_ERROR, ("wiced_ota2_service_get_the_update() ota2_service_connect() failed (list)!\r\n"));
                wiced_ota2_service_make_error_callback(session, OTA2_SERVICE_SERVER_CONNECT_ERROR );
                session->tcp_failures++;
            }
            goto _check_update_done;
    ...
    {
        OTA2_LIB_PRINT(session, OTA2_LOG_INFO, ("wiced_ota2_service_get_the_update() Use the ota2 info!\r\n"));

        /* connect to alternate AP, if provided */
        if (wiced_ota2_service_network_switch_to_ota2_ap( session ) != WICED_SUCCESS)
        {
            OTA2_LIB_PRINT(session, OTA2_LOG_ERROR, ("wiced_ota2_service_get_the_update() wiced_ota2_service_network_switch_to_ota2_ap() failed!\r\n"));
            wiced_ota2_service_make_error_callback(session, OTA2_SERVICE_AP_CONNECT_ERROR);
            session->ota2_ap_failures++;
            goto _check_update_done;
        }
    ...
    while(1)
    {
        ...
        if (events & OTA2_EVENT_WORKER_START_INITIAL_TIMER)
        {
            ...
            if (session->initial_check_interval > 0)
            {
                ...
                session->session_next_time += (session->initial_check_interval * MILLISECONDS_PER_SECOND);    /* for info */
            }
            else
            {
        ...
        if (events & OTA2_EVENT_WORKER_CHECK_FOR_UPDATES)
        {
            ...
            {
                wiced_time_get_time( &curr_time );
                OTA2_LIB_PRINT(session, OTA2_LOG_ERROR, ("OTA2: session %p kick off OTA2_EVENT_WORKER_START_DOWNLOAD %ld!\r\n", session, curr_time));
                ...
                OTA2_LIB_PRINT(session, OTA2_LOG_ERROR, ("OTA2: ota2_service_start_download_now_timer() returned\r\n"));
            }
            else
            {
...
void*  wiced_ota2_service_init(wiced_ota2_backround_service_params_t *params, void* opaque)
{
    ...
    if (params->retry_check_interval < WICED_OTA2_MIN_RETRY_INTERVAL_TIME)
    {
...
wiced_result_t wiced_ota2_service_app_network_is_down(void* session_id)
{
    ...
    switch (state)
    {
        ...
        case OTA2_SERVICE_STATE_PAUSED:
...
wiced_result_t wiced_ota2_service_status(void* session_id)
{
    ...
    OTA2_LIB_PRINT(session, OTA2_LOG_NOTIFY, ("               interface: %s\r\n", (session->ota2_interface == WICED_STA_INTERFACE) ? "STA" :
                                                                                   (session->ota2_interface == WICED_AP_INTERFACE) ? "AP" :
                                                                                    (session->ota2_interface == WICED_P2P_INTERFACE) ? "P2P" : "Ether" ));
