 ...
 *
 * $Id: wlu.c,v 1.1084.2.43 2011-02-10 22:46:22 ichu Exp $
 */
...
cmd_func_t wl_int;
extern cmd_func_t wl_cmd_exec;
extern cmd_func_t wl_wwd_ip_state;
...
static cmd_func_t wlu_ratedump;
...
static cmd_func_t wl_phy_txiqcc, wl_phy_txlocc, wl_phytable, wl_phy_pavars, wl_phy_pavars2;
static cmd_func_t wl_phy_povars;
static cmd_func_t wl_phy_fem, wl_phy_maxpower, wl_antgain, wl_phy_txpwrindex, wl_phy_oclscd;
...
cmd_t wl_cmds[] = {
	...
	"\t0: clock off\n"
	"\t1: clock on" },
	{ "spect", wl_int, WLC_GET_SPECT_MANAGMENT, WLC_SET_SPECT_MANAGMENT,
	...
	{ "ba_dump", wl_print_deprecate, -1, -1, "Deprecated. Folded under 'wl dump ba'" },
	{ "ba_clear_counters", wl_var_void, -1, WLC_SET_VAR,
	"clear BA counters"},
	...
	{ "pciereg", wlu_reg2args, WLC_GET_VAR, WLC_SET_VAR, "g/set pcie core registers"},
	...
	{ "phy_oclscdenable", wl_phy_oclscd, WLC_GET_VAR, WLC_SET_VAR,
	"Set ocl and scd for rx\n"
	"\t0 - disable ocl and scd \n"
	"\t1 - force ocl \n"
	"\t2 - disable ocl and scd \n"
	"\t3 - enable ocl and scd"
	},
	...
	"[bestn (2)|[1-3]] [mscan (0)|[0-90]] [bdscan (0)|1] [adapt (off)|[smart, strict]]"
	"[rssi_delta xxxx(30 dBm)] [sort (listorder)|rssi] [bkgscan (0)|1]"
...
#endif /* SERDOWNLOAD */
	{ "iperf_start", wl_cmd_exec, -1, -1,
      "Spawns a thread doing iperf with given params.\n"},
    { "wwd_join", wl_cmd_exec, -1, -1,
      "Joins a network (open mode only) and brings up ip stack.\n"},
    { "wwd_ip_state", wl_wwd_ip_state, -1, -1,
      "Display the ip state of the STA interface's network stack.\n"},
#ifdef BCMCCX
	{ "cckm_rn", wl_varint, WLC_GET_VAR, -1,
	"\tGet CCKM rekey refresh number.\n"},
#endif /* BCMCCX */
...
wlu_cisdump(void *wl, cmd_t *cmd, char **argv)
{
	...
	defined(__IOPOS__) || defined(__NetBSD__) || defined( TARGET_wiced )
...
wl_tssical_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	cmd_t pkteng_cmd;
	char (*pkteng_start_arg_ptrs[7]);
	char pkteng_start_args[6][24] = {"pkteng_start",
									 "00:11:22:33:44:55",
									 "tx", "100", "1000", "0"};
	char (*pkteng_stop_arg_ptrs[3]);
...
wl_parse_assoc_params(char **argv, wl_assoc_params_t *params)
{
	...
	while ((p = *argv) != NULL) {
		...
		if (opt == 'b' || !stricmp(key, "bssid")) {
			...
			params->chanspec_num = wl_parse_chanspec_list(valstr, params->chanspec_list,
				WL_NUMCHANNELS);
...
wl_dump_networks(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (iscan) {
		int buflen = (WL_DUMP_BUF_LEN < 1920) ? WL_DUMP_BUF_LEN : 1920; /* usually fits about 10 BSS infos */
...
wl_get_current_power(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (phytype != WLC_PHY_TYPE_HT) {   /* HTPHY will use new format */
		...
		wl_txpwr_array_print(power.reg_limit, CHSPEC_IS2G(chanspec), mimo, 1);
		printf("\n");

		printf("Board Limits:\n");
		wl_txpwr_array_print(power.board_limit, CHSPEC_IS2G(chanspec), mimo, 0);
		printf("\n");

		printf("Power Target:\n");
		wl_txpwr_array_print(power.target, CHSPEC_IS2G(chanspec), mimo, 0);
		printf("\n");
	}
...
static void
wl_txpwr_array_print(uint8 *pwr, int cck, int mimo, int reg_power)
{
	int newline = TRUE;
	int start = WL_TX_POWER_MCS20_SISO_FIRST_SSN;
	...
	if (!mimo) {
		...
		if (reg_power) /* Regulatory Power */
			start = WL_TX_POWER_MCS20_SISO_FIRST;

		/* MCS 20MHz SISO rates */
		wl_txpwr_range_print(pwr, start,
...
wl_HCI_ACL_data(void *wl, cmd_t *cmd, char **argv)
{
	 ...
	 * from the stack.
	 */
	static g_wlu_hci_dbuf_t g_wlu_hci_dbuf;
...
static int
wl_phy_oclscd(void *wl, cmd_t *cmd, char **argv)
{
	int err;
	int32 oclscd;
	int32 *ptroclscd;
	...
	if (!*++argv) {
		if ((err = wlu_iovar_get(wl, cmd->name, ptroclscd, 2*sizeof(int32))) < 0)
			return err;
		printf("%d\n", oclscd);
	...
	else
	{
		...
		if ((err = wlu_var_setbuf(wl, cmd->name, ptroclscd, 2*sizeof(int32))) < 0)
			return err;
	}
	...
	if (argc != 0) {
		...
		while ((opt_err = miniopt(&to, argv)) != -1) {
			...
			if (to.opt == 's') {
				...
				if (to.val < 10 || to.val > 15) {
                    fprintf(stderr, "%s: sample count invalid %d;"
                        "acceptable values: (10 <= x <= 15)\n", fn_name, to.val);
					err = -1;
					goto exit;
				}
...
wl_p2p_scan(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*(argv + 1) != NULL) {
		...
		switch (toupper((unsigned char)**(argv + 1))) {
		case 'S':
...
/* direct backplane access via a mfgtest USB/HSIC fw */
static int
...
static int ReadFiles(char *fname, char *vname, unsigned char ** buffer)
{
	...
	if ((actual_len = check_file(buf)) <= 0) {
		printf("ReadFiles: Failed input file check!\n", fname);
		ret = -1;
		goto exit;
	}
