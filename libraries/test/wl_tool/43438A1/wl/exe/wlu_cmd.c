/*
 * $Id: wlu_cmd.c 452237 2014-01-29 13:28:11Z ssingla $
 *
...
cmd2cat_t cmd2cat[] = {
	...
	{"iov", CMD_ADMIN},
	{"iperf_start", CMD_AP + CMD_STA + CMD_DEV},
	{"wwd_join", CMD_AP + CMD_STA + CMD_DEV},
	{"wwd_ip_state", CMD_STA},
	...
	{"probresp_mac_filter", CMD_SEC + CMD_MGMT + CMD_AP},
...
struct iov_control_info {
	...
	bool one_col;	/* Print output in single column */
...
wl_iov_usage(FILE *fid, char *cmd)
{
	...
	fprintf(fid, "    -1 Display names in a single column\n");
...
wl_iov_names(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argv[1] && strcmp(argv[1], "-h") == 0) { /* Print usage */
