 ...
 *
 * $Id: wlu_server_shared.c 445283 2013-12-25 08:12:37Z blitvin $
 */
...
#else
#if !defined(TARGETOS_nucleus) && !defined(MACOSX) && !defined(vxworks) && !defined(__IAR_SYSTEMS_ICC__)
#include <malloc.h>
#endif
...
#include <netdb.h>
#include <signal.h>
#endif /* TARGETOS_nucleus && TARGET_wiced */
#endif /* WIN32 */
...
#endif /* if !defined( TARGETOS_symbian ) && !defined(TARGET_wiced) */
...
remote_server_exec(int argc, char **argv, void *wl)
{
    ...
    char *fn = "/etc/jffs2/rtecdc.bin";
    char *fw = "/etc/jffs2/nvram.txt";
#else
