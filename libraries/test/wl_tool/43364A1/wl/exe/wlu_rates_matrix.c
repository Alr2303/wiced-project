...
int vht_ss1_reg_rate_map[VHT_RATE_INDEX_MAX][VHT_SS1_MODE_ID_MAX] = {
	...
	{MCS87, MCS87_CDD1, MCS87_STBC, MCS87_CDD2, MCS87_STBC_SPEXP1,MCS87_TXBF1, MCS87_TXBF2},
	{MCS88, MCS88_CDD1, MCS88_STBC, MCS88_CDD2, MCS88_STBC_SPEXP1,MCS88_TXBF1, MCS88_TXBF2},
};
...
int vht_ss2_reg_rate_map[VHT_RATE_INDEX_MAX][VHT_SS2_MODE_ID_MAX] = {
	...
	{VHT9SS2, VHT9SS2_SPEXP1,NO_RATE,        VHT9SS2_TXBF1},
	{MCS99, MCS99_SPEXP1,    NO_RATE,	 MCS99_TXBF1},
	{MCS100, MCS100_SPEXP1,  NO_RATE,	 MCS100_TXBF1},
};
...
get_legacy_reg_rate_index(int rate_hmhz, int tx_expansion, exp_mode_t expmode)
{
	...
	{
		index = (reg_rate_index_t)legacy_reg_rate_map[rate_id][get_legacy_mode_identifier(tx_expansion, expmode)];
	}
	return index;
}
...
{
	reg_rate_index_t rate_index = NO_RATE;
	int vht_mcs = VHT_RATE_INDEX_MAX;
	int nss = 0;
	...
	{
		/* Set index and stream number for proprietary rates mcs87_88
		 * mcs99_100, mcs101_102
		 */
		if (IS_PROPRIETARY_11N_MCS(mcs)) {
			switch (mcs) {
			case 87:
				nss = 1;
				vht_mcs = HT_RATE_PROP_1;
				break;
			case 99:
				nss = 2;
				vht_mcs = HT_RATE_PROP_1;
...
get_vht_reg_rate_index(int mcs, int nss, int tx_expansion, exp_mode_t expmode)
{
	...
	if (nss == 1)
	{
		rate_index = (reg_rate_index_t)vht_ss1_reg_rate_map[rate_id][get_vht_ss1_mode_identifier(tx_expansion, expmode)];
	}
	else if (nss == 2)
	{
		rate_index = (reg_rate_index_t)vht_ss2_reg_rate_map[rate_id][get_vht_ss2_mode_identifier(tx_expansion, expmode)];
	}
	else if (nss == 3)
	{
		rate_index = (reg_rate_index_t)vht_ss3_reg_rate_map[rate_id][get_vht_ss3_mode_identifier(tx_expansion, expmode)];
	}
