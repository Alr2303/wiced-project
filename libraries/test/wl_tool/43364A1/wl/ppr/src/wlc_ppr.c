...
static uint ppr_serialize_data(const ppr_t *pprptr, uint8* buf, uint32 serflag)
{
	...
	bands = ppr_bands_by_bw((wl_tx_bw_t)header->bw);
...
static pprpbw_t* ppr_get_bw_powers_20(ppr_t* p, wl_tx_bw_t bw)
{
	...
	/* XXX wlc_phy_txpower_limit_set() asks for 40MHz powers irrespective of the channel bw */
	/* else */
	/*   ASSERT(0); */
	return pwrs;
}
...
static int8* ppr_get_dsss_group(pprpbw_t* bw_pwrs, wl_tx_chains_t tx_chains)
{
	...
	default:
#ifdef BCMQT
		/* XXX JIRA:SWMUMIMO-37 Pending on WL_TX_CHAINS_4 support. */
...
#ifdef WL_BEAMFORMING
			if (PPR_TXBF_ENAB()) {
				offset = mcs_groups_nss2_txbf[(int)tx_chains - (int)Nss][mode];
			} else
#endif /* WL_BEAMFORMING */
			{
				offset = mcs_groups_nss2[(int)tx_chains - (int)Nss][mode];
			}
			if (offset != OFFSNONE) {
				group_pwrs = (int8*)bw_pwrs + offset;
			}
...
#ifdef WL_BEAMFORMING
			if (PPR_TXBF_ENAB()) {
				offset = mcs_groups_nss1_txbf[(int)tx_chains - (int)Nss][mode];
			} else
#endif /* WL_BEAMFORMING */
			{
				offset = mcs_groups_nss1[(int)tx_chains - (int)Nss][mode];
			}
			if (offset != OFFSNONE) {
				group_pwrs = (int8*)bw_pwrs + offset;
			}
 ...
 * rates need to be disabled that cannot be controlled with the accuracy needed to avoid
...
#if (PPR_MAX_TX_CHAINS > 3)
	/* XXX JIRA:SWMUMIMO-37 Pending on WL_TX_CHAINS_4 support. */
	printf("%s: %d: pending on WL_TX_CHAINS_4 supported!!\n", __FUNCTION__, __LINE__);
...
ppr_deserialize_create(osl_t *osh, const uint8* buf, uint buflen, ppr_t** pprptr)
{
	...
	if ((buflen > SER_HDR_LEN) && (bptr != NULL) && (*bptr == PPR_SERIALIZATION_VER)) {
		...
		wl_tx_bw_t ch_bw = (wl_tx_bw_t)ser_head->bw;
...
/* XXX This needs to be stored per interface */
...
/** XXX: Accessor function forced into RAM to keep tx_pwr_cache out of shdat */
