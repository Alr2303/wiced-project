 ...
 *
 * $Id: wlu_linux.c 435186 2013-11-08 21:02:09Z arunroy $
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
static int (*g_driver_io)(void *wl, wl_ioctl_t *ioc);
...
#ifdef NL80211
static int __wl_driver_nl80211(void *wl, wl_ioctl_t *ioc)
{
...
static int __wl_driver_ioctl(void *wl, wl_ioctl_t *ioc)
{
	struct ifreq *ifr = (struct ifreq *) wl;
	int ret;
	int s;
...
static void wl_select_nl80211(int select)
{
#ifdef NL80211
	if (select)
		g_driver_io = __wl_driver_nl80211;
	else
		g_driver_io = __wl_driver_ioctl;
#else
...
#ifdef NL80211
	/* Linux client using nl80211 netlink control interface */
	if (*argv && strncmp(*argv, "--nl80211", strlen(*argv)) == 0) {
		wl_select_nl80211(TRUE);
		(void)*argv++;
	}
#endif
