 ...
 *
 * $Id: wlu_server_shared.c 428562 2013-10-09 09:33:20Z richarch $
 */
...
#include <errno.h>
#ifndef WIN32
#if !defined(TARGETOS_nucleus) && !defined(TARGET_wiced)
#include <sys/types.h>
#include <sys/socket.h>
#if  !defined(vxworks) && !defined(TARGETOS_symbian)
#include <arpa/inet.h>
#include <net/if.h>
#endif
#include <unistd.h>
...
extern int start_iperf(char *buffer, int len);

extern int control_audio_loopback(char *cmd);
extern int wiced_wlu_wireless_network_start( char *buf_ptr, int buf_len );
extern int wiced_wlu_ip_details( char *buf_ptr, int buf_len );
...
#if defined (RWL_DONGLE) || defined (RWL_WIFI) || defined (RWL_SERIAL) || defined (RWL_SOCKET)
...
#if !defined( TARGETOS_symbian ) && !defined(TARGET_wiced)
 ...
 */
			...
			if (g_rem_ptr->msg.flags & REMOTE_SET_IOCTL) {
				if (g_rem_ptr->msg.cmd == WLC_SET_VAR && buf_ptr &&
				   ...
				   !strncmp((const char *)buf_ptr,
                    ...
                    !strncmp((const char *)buf_ptr, "wwd_join", strlen("wwd_join"))) {

                    err = wiced_wlu_wireless_network_start( (char*)buf_ptr, g_rem_ptr->msg.len );

                } else if (g_rem_ptr->msg.cmd == WLC_SET_VAR && buf_ptr &&
                    !strncmp((const char *)buf_ptr, "iperf_start",
                    strlen("iperf_start"))) {

                    err = start_iperf( (char*)buf_ptr, g_rem_ptr->msg.len );
                } else if (g_rem_ptr->msg.cmd == WLC_SET_VAR && buf_ptr &&
                    !strncmp((const char *)buf_ptr, "audio_loopback",
                    strlen("audio_loopback"))) {

                    err = control_audio_loopback( (char*)buf_ptr );
                } else {
		...
		if (g_rem_ptr->msg.flags & REMOTE_GET_IOCTL ||
            ...
            set_interface(wl, g_rem_ptr->intf_name);
#endif
        if ( buf_ptr && strncmp((const char *)buf_ptr, "wwd_ip_state", strlen( "wwd_ip_state" ) ) == 0 )
        {
            err = wiced_wlu_ip_details( (void *)buf_ptr, g_rem_ptr->msg.len );

        } else if (g_rem_ptr->msg.flags & REMOTE_GET_IOCTL)
...
#if 0   /* Temporary work around - still working on root cause
         *
         * Problem is that wl43909B0.exe running as Client on PC will interpret not getting any response
         *         as a timeout error - not the actual error, which is what we want.
         *         Also, when MFGc (HOTEL) uses the wl client on PC, it times out earlier than the wl client
         *         and causes problems with MFGc.
         *
         * Question: Why was this check put here in the first place?
         */
        if (0 == err)
#endif
		{
