 ...
 *
 * $Id: wlm.h 365537 2012-10-30 02:42:14Z jwang $
 */
...
#define WLM_FUNCTION
#endif /* WIN32 */
#define WLM_VERSION_STR "wlm version: 10.05.0 (support 4330, 4334, 43236 and more)"
...
enum {
	WLM_DUT_OS_ANY   = 0,  	/* ANY OS on DUT*/
...
/* Write CIS update to SROM/OTP
 *
 * param[in] Byte offset where the cis update begin.
 * param[in] hex string in even digits which will be update to cis (SROM, OTP etc)  .
 *
 * return - Ture for success, false for failure.
 */
WLM_FUNCTION
int wlmCisUpdate(int off, char *bufp);
...
/* Get HW Access control register
 * param[in] register address
 * param[in] length
 * param[out] returned register value
 */
WLM_FUNCTION
int wlmHwAccGet(unsigned int addr, unsigned int len, unsigned int *data);

/* Set HW Access control register
 * param[in] register address
 * param[in] length
 * param[in] Desired register value
 */
WLM_FUNCTION
int wlmHwAccSet(unsigned int addr, unsigned int len, unsigned int data);
