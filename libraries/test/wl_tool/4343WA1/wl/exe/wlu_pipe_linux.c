 ...
 *
 * $Id: wlu_pipe_linux.c 431968 2013-10-25 05:14:17Z jwang $
 */
...
#endif /*  defined(RWL_DONGLE) || defined(RWL_SERIAL) */
...
rwl_open_serial(int remote_type, char *port)
{
	...
	*bsdPath = '\0';
	if (!g_rwl_device_name_serial) {
		...
		while ((serialService = IOIteratorNext(serialPortIterator)) && !found) {
		...
		g_rwl_device_name_serial = malloc(strlen(bsdPath) * sizeof(char *));
		if (!g_rwl_device_name_serial) {
			fprintf(stderr, "Failed to allocate memory.\n");
			return -1;
		}
		strncpy(g_rwl_device_name_serial, bsdPath, strlen(bsdPath));
	} else {
		bzero(bsdPath, sizeof(bsdPath));
		strncpy(bsdPath, g_rwl_device_name_serial, strlen(g_rwl_device_name_serial));
	}

	fileDescriptor = open(bsdPath, O_RDWR | O_NONBLOCK | O_NDELAY | O_NOCTTY);
	...
	tio.c_cc[VMIN] = 0;
	tio.c_cc[VTIME] = 0;
	/* WAR: 10.18.2013 for Kong cable issue  */
	/*
	...
	*/
	...
	if (tcsetattr(fileDescriptor, TCSANOW, &tio) < 0) {
		DPRINT_ERR(ERR, "Error setting tty attributes %s - %s(%d).\n",
			bsdPath, strerror(errno), errno);
	}

	/* WAR: 10.23.2013 for Kong cable issue */
	usleep(500000);

	/* WAR: 10.18.2013 for Kong cable issue  */
	/*
	  if (tcflush(fileDescriptor, TCIOFLUSH))
		DPRINT_ERR(ERR, "error doing flush");
	*/
	...
	while (read(fileDescriptor, &c, 1)) {
		i++;
		if (i > 1000) {
			DPRINT_ERR(ERR, "too many reads, breaking");
			break;
		}
	}
...
int rwl_set_baud_rate(void* hndle, int speed)
{
	...
	if (speed <= B230400) {
		...
		if (cfsetspeed (&term, speed) < 0) {
			DPRINT_ERR(ERR, "Failed to set port speed to %d", speed);
			return -1;
		}
	}
	else {
...
#endif
	  /* set nonstandard speed on AppleOnboardSerial */
		ret = ioctl((*(int*)hndle), IOSSIOSPEED, &speed);
		if (ret) {
			DPRINT_ERR(ERR, "Failed to set port speed to %d", speed);
			return -1;
		}
...
#endif /* #if defined (RWL_DONGLE) || defined (RWL_SOCKET) || defined(RWL_SERIAL) */
