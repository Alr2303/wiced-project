...
wlu_srwrite(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (erase) {
		...
		if (len == SROM4_WORDS * 2) {
		    uint16* tmp = (uint16*)srt->buf;
			if (( *(tmp + SROM4_SIGN) != SROM4_SIGNATURE) &&
			    ( *(tmp + SROM8_SIGN) != SROM4_SIGNATURE)) {
...
dhd_init(void *dhd, cmd_t *cmd, char **argv)
{
	...
	if (ret == 0) {
		if (!strcmp(*argv, "4325"))  {
		    ...
		    chip_select = "4330a1";
		} else if (!strcmp(*argv, "43340"))  {
            fprintf(stdout, "43340 is the selected chip id\n");
            chip_select = "43340";
...
dhd_download(void *dhd, cmd_t *cmd, char **argv)
{
	...
	if (!strcmp(chip_select, "43340")) {
        fprintf(stdout, "using 43340 ram_info\n");
        ram_size = RAM_SIZE_43340;
...
dhd_upload(void *dhd, cmd_t *cmd, char **argv)
{
	...
	if (!strcmp(chip_select, "4325")) {
		...
		ram_size = RAM_SIZE_4330_a1;
	} else if (!strcmp(chip_select, "43340")) {
        fprintf(stdout, "using 43340  ram_info\n");
        ram_size = RAM_SIZE_43340;
	} else {
		fprintf(stderr, "Error: unknown chip\n");
