...
#define TEST_TX_AUDIO_DEVICE         PLATFORM_DEFAULT_AUDIO_OUTPUT  /* To specify, change to a valid device for the platform
                                                                     ...
                                                                     */
#if (WICED_AUDIO_LOOPBACK_ENABLE_SINE_WAVE_OUTPUT == 0 || WICED_AUDIO_LOOPBACK_NO_DATA_VALIDATION == 0)
...
static void print_buffer( uint8_t* buffer, uint16_t buffer_length );
#endif
...
static wiced_result_t validate_rx_data( uint16_t available_bytes )
{
    ...
    do_data_validation = (is_tx_started != 0 &&
                          WICED_AUDIO_LOOPBACK_NO_DATA_VALIDATION == 0) ? 1 : 0;
...
static void print_buffer( uint8_t* buffer, uint16_t buffer_length )
{
    ...
    for (i = 0; i < buffer_length/2; i++)
    {
        if (j == 0)
        {
            PRINT_INFO("0x%04x: ", 2*i);
        }
        PRINT_INFO("0x%04x ", data[i]);
        if (j++ == 7)
        {
            PRINT_INFO("\n");
            j = 0;
...
static wiced_result_t validate_data( uint8_t* buffer, uint16_t buffer_length )
{
    ...
    while ( i < buffer_length / 2 )
    {
        ...
        {
            char ch = ((i % 2)?'R':'L');
            PRINT_INFO("iter %d: invalid %c data, offset: 0x%x. Expected 0x%04x, received 0x%04x",
                iter, ch, 2*i, (uint16_t)data[last_pos], (uint16_t)buf16[i]);
...
static wiced_result_t loop_iteration( void )
{
    ...
    while ( 0 != remaining && result == WICED_SUCCESS )
    {
        ...
        if ( result != WICED_SUCCESS )
        {
            PRINT_INFO("wiced_audio_release_buffer TX failed");
            wiced_assert("wiced_audio_release_buffer", result == WICED_SUCCESS);
            return result;
        }
