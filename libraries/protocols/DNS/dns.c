...
wiced_result_t dns_client_hostname_lookup( const char* hostname, wiced_ip_address_t* address, uint32_t timeout_ms, wiced_interface_t interface )
{
    uint32_t               a;
...
#define RETRY_INTERVAL	2000
    remaining_time = (timeout_ms > RETRY_INTERVAL) ? RETRY_INTERVAL : timeout_ms;
