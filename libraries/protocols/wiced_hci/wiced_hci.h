 ...
 * Mesh Commands
 */
#define HCI_CONTROL_MESH_COMMAND_GENERIC_ON_OFF             ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x01 )      /* PBC Connect */
#define HCI_CONTROL_MESH_COMMAND_GENERIC_LEVEL              ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x02 )      /* PBC Disconnect */
#define HCI_CONTROL_MESH_COMMAND_START                      ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x03 )      /*register for provisioing status and proxy data callbacks */
#define HCI_CONTROL_MESH_COMMAND_CONNECT_PROXY              ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x04 )      /* Command to send proxy connection */
#define HCI_CONTROL_MESH_COMMAND_SEND_PROXY_DATA            ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x05 )      /*  Command to send proxy data from application */
#define HCI_CONTROL_MESH_COMMAND_PUSH_NVRAM_DATA            ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x06 )      /*  Application pushes the nvram data to the mesh core */
#define HCI_CONTROL_MESH_COMMAND_STACK_INIT                 ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x07 )      /*  Application pushes the nvram data to the mesh core */
 ...
 * Mesh events
 */
#define HCI_CONTROL_MESH_EVENT_ON_OFF                       ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x01 )      /* ON/OFF */
#define HCI_CONTROL_MESH_EVENT_LEVEL                        ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x02 )     /* Level */
#define HCI_CONTROL_MESH_EVENT_PROVISIONING_STATUS          ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x03 )     /*  Provisioing status */
#define HCI_CONTROL_MESH_EVENT_PROXY_CONNECTION_STATUS      ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x04 )     /*  Proxy connection status */
#define HCI_CONTROL_MESH_EVENT_PROXT_DATA                   ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x05 )     /*  Proxy Data*/
#define HCI_CONTROL_MESH_EVENT_NVRAM_WRITE                  ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x06 )     /*  Recieve NVRAM data   */
#define HCI_CONTROL_MESH_COMMAND_CONNECTION_STATE           ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x07 )     /*  Application use this interface to send connection state , before it sends proxy data */
#define HCI_CONTROL_MESH_EVENT_MESH_STATUS                  ( ( HCI_CONTROL_GROUP_MESH << 8 ) | 0x08 )     /*  Application use this interface to send connection state , before it sends proxy data */
