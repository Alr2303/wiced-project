...
/* WICED HTTP2 FLAGS */
#define HTTP_REQUEST_FLAGS_FINISH_REQUEST   0x01    /* This is the last to hear from this request */
#define HTTP_REQUEST_FLAGS_FINISH_HEADERS   0x04    /* Last header part in the request. */
#define HTTP_REQUEST_FLAGS_PUSH_PROMISE     0x08    /* The request received is a push promise request from the server. */
#define HTTP_REQUEST_FLAGS_UPGRADE_HTTP2    0x10    /* While doing request, try upgrading to HTTP2 */

#define HTTP_REQUEST_FLAGS_PING_REQUEST     0x0     /* Flag used to send PING request frame */
#define HTTP_REQUEST_FLAGS_PING_RESPONSE    0x1     /* Flag used to send PING response i.e., ACK for the PING request */

/* WICED HTTP2 default settings. */
#define HTTP2_SETTINGS_HEADER_TABLE_SIZE       (0)  /* ??TODO?? Why the HPACK size is zero. Does it mean, this only represent the table used for dynamic entries */
...
#define HTTP2_PING_DATA_LENGTH                 (8)

#define HTTP2_CONNECTION_TIMEOUT_IN_MSEC       (2 * 1000)

/* Use this flag to build the library for a server (Non tested ) */
/*
...
typedef struct http_connection_callbacks_s
{
     ...
     * @param[in] flags          A bit masked parameter indicate request options.
     * @param[in] user_data      Pointer to the user data which is passed during connection init.
     *
     * @return @ref wiced_result_t
     */
    wiced_result_t (*http_request_recv_header_callback) ( http_connection_ptr_t connect, http_request_id_t request, http_header_info_t* headers, http_frame_type_t type, uint8_t flags, void* user_data );
     ...
     * @return @ref wiced_result_t
     */
    wiced_result_t (*http_request_recv_data_callback) ( http_connection_ptr_t connect, http_request_id_t request, uint8_t* data, uint32_t length, uint8_t flags, void* user_data );
...
typedef struct http_connection_state_s
{
    ...
    uint32_t goaway_error;       /* The error code recieved in a go away */
...
typedef struct http_connection_s
{
    ...
    wiced_semaphore_t            semaphore;                     /* Semaphore used reentrancy */
 ...
 *
 * Allocates a new request. At this point requests are empty.
 *
 * Usually called by client to create a request. Requests can also be created
 * dynamically as a result of receiving a request (from client if you are a server)
 * or a promise push ( from the server if you are a client ).
 *
 * @param[in] connection     Structure workspace that will be used for this HTTP connection.
 * @param[in] request        A pointer to request object which contains the stream ID on successful return
 *
 * @return @ref wiced_result_t
 */
...
/**
 * Adds/sends header fields on a created request
 *
 * A Client should fill this with a request header ( usually inclues method,
 * host, authority, etc. ).
 * A server responding to a received request should include a response header
 * ( status, etc. )
 * A server creating a request field for a promised push should fill a request
 * header and set the HTTP_REQUEST_FLAGS_PROMISE_PUSH in the flags field. It is
 * important to note that a pushed promise header request goes on a different
 * request than the actual response (header and data).
 *
 * Clients are not allowed to set the HTTP_REQUEST_FLAGS_PROMISE_PUSH flag.
 *
 * The HTTP_REQUEST_FLAGS_FINISH_REQUEST should be set if the sent header is
 * the last part of the request.
 *
 * @param[in] connection     Structure workspace that will be used for this HTTP connection.
 ...
 *                           - HTTP_REQUEST_FLAGS_PROMISE_PUSH: Set by server when sending a
 *                             promise push request (valid only for HTTP2).
 *                           - HTTP_REQUEST_FLAGS_FINISH_REQUEST: Set by server or client when
 *                             the header sent is the last part of the request/response
 *                           - HTTP_REQUEST_FLAGS_UPGRADE_HTTP2: Set by client to request to
 *                             upgrade the connection to HTTP2. Once upgraded there is not way
 *                             to revert.
 *
 * @return @ref wiced_result_t
 */
 ...
 * @param[in] flags          A bit masked parameter indicates request options.
 *                           - HTTP_REQUEST_FLAGS_PING_REQUEST: Set by the end point sending PING request
 *                           - HTTP_REQUEST_FLAGS_PING_RESPONSE: Set by the end point sending PING response
 *
 * @return @ref wiced_result_t
 */
