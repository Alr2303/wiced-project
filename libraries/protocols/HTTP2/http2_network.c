 ...
 ******************************************************/
static const ssl_extension application_layer_protocol_negotiation_extension =
{
    .id   = 0x0010,
    .used = 1,
    .sz   = 41,
    .data = { 0x00, 0x27, 0x05, 0x68, 0x32, 0x2d, 0x31, 0x36, 0x05, 0x68, 0x32, 0x2d, 0x31, 0x35, 0x05, 0x68, 0x32, 0x2d, 0x31, 0x34, 0x02, 0x68, 0x32, 0x08, 0x73, 0x70, 0x64, 0x79, 0x2f, 0x33, 0x2e, 0x31, 0x08, 0x68, 0x74, 0x74, 0x70, 0x2f, 0x31, 0x2e, 0x31 },
};
...
wiced_result_t http_network_tls_init( http_socket_t *socket, const http_security_info_t *security )
{
    wiced_result_t result = WICED_SUCCESS;
    ...
    if ( security != NULL )
    {
        ...
        wiced_tls_add_extension( &socket->tls_context, &application_layer_protocol_negotiation_extension );
        HTTP_DEBUG(( "[%s()] : [%d] : Extension Count = [%d]\n", __FUNCTION__, __LINE__, socket->tls_context.context.extension_count ));
    }
    return result;
...
wiced_result_t http2_network_init( http_connection_t* connect )
{
    ...
    result = wiced_rtos_create_thread(&connect->http2_thread, HTTP2_WORKER_THREAD_PRIORITY, HTTP2_WORKER_THREAD_NAME, http_network_thread, HTTP2_WORKER_THREAD_STACK_SIZE, connect);
    if (result != WICED_SUCCESS)
    {
