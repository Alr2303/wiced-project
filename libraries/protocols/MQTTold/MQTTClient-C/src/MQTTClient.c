...
static int readPacket(MQTTClient* c, Timer* timer)
{
    ...
    len = c->ipstack->mqttread(c->ipstack, c->readbuf, 1, TimerLeftMS(timer));
    if (len < 0) {
        goto exit;
    } else if (len == 0) {
	rc = 0;
    ...
    /* overflow, then skip message */
    if (len + rem_len >= (int)c->readbuf_size) {
	    int i;
	    printf("Overflow Recv MQTT Message: %d bytes\n", len + rem_len);
	    for (i = 0; i < rem_len; i++) {
...
int MQTTSubscribe(MQTTClient* c, const char* topicFilter, enum QoS qos, messageHandler handler)
{ 
    int rc = MQTT_FAILURE;  
    Timer timer;
    int len = 0;
