...
wiced_result_t wiced_mqtt_connect( wiced_mqtt_object_t mqtt_obj, wiced_ip_address_t *address, wiced_interface_t interface, wiced_mqtt_callback_t callback, wiced_mqtt_security_t *security, wiced_mqtt_pkt_connect_t *conninfo )
{
    ...
    if ( conninfo->will_topic && conninfo->will_message )
    {
        args.will_flag = 1;
        args.will_topic.str = (uint8_t*) conninfo->will_topic;
        args.will_topic.len = (uint16_t) strlen( (char*) conninfo->will_topic );
        args.will_message.str = (uint8_t*) conninfo->will_message;
        args.will_message.len = (uint16_t) strlen( (char*) conninfo->will_message );
        args.will_qos = conninfo->will_qos;
        args.will_retain = conninfo->will_retain;
    }
    args.will_flag = 0;
...
wiced_mqtt_msgid_t wiced_mqtt_publish( wiced_mqtt_object_t mqtt_obj, uint8_t *topic, uint8_t *data, uint32_t data_len, uint8_t qos, uint8_t retain )
{
