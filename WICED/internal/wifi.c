...
wiced_result_t wiced_resume_after_deep_sleep( void )
{
    ...
    } else if ( !WICED_DEEP_SLEEP_IS_WARMBOOT_HANDLE( ))
    {
...
wiced_result_t wiced_wifi_enter_ds1_debug( wiced_interface_t interface, wiced_offload_t offload_type, wiced_offload_value_t *offload_value, uint32_t ulp_wait_milliseconds, wiced_ds1_debug_t *debug_overrides )
{
    ...
    {
        WPRINT_APP_DEBUG(("Invalid ulp value %lu; min is %d\n", ulp_wait_milliseconds, WICED_WIFI_ULP_MIN_MILLISECONDS));
        return WICED_ERROR;
    }

    /* powersave needs to be on to enter; make it so here */
    if ( WWD_SUCCESS != wwd_wifi_enable_powersave( ) )
    {
        WPRINT_APP_DEBUG(("Unable to enable power save\n"));
        return WICED_ERROR;
    }

    wiced_wifi_wowl_get_values( offload_type, &wowl, &wowl_os, debug_overrides );

    switch ( offload_type )
    {
        case WICED_OFFLOAD_MAGIC:
        {
            result = (wiced_result_t)wwd_wifi_wowl_enable( WICED_TO_WWD_INTERFACE( interface ), wowl, wowl_os, NULL, NULL, 0, NULL );
        }
        break;
        case WICED_OFFLOAD_PATTERN:
        {
            ...
            strlcpy( pattern_cmd, "add", pattern_data_size );
            result = wiced_xlate_pattern_to_wl_pattern( &offload_value->pattern, pattern_list );

            if ( WICED_SUCCESS == result )
            {
                /* call into driver to set up wowl */
                result = (wiced_result_t)wwd_wifi_wowl_enable( WICED_TO_WWD_INTERFACE( interface ), wowl, wowl_os, NULL, (uint8_t*)pattern_cmd, pattern_data_size, NULL );
            }
        ...
        case WICED_OFFLOAD_KEEP_ALIVE:
        {
            ...
            {
                /* call into driver to set up wowl */
                result = (wiced_result_t)wwd_wifi_wowl_enable( WICED_TO_WWD_INTERFACE( interface ), wowl, wowl_os, wowl_keepalive_data, NULL, 0, NULL );
            }
        ...
        case WICED_OFFLOAD_ARP_HOSTIP:
        {
            ...
            {
                WPRINT_APP_DEBUG(("IpV6 not applicable for ARP offload\n"));
            ...
            CHECK_RETURN( wwd_wifi_wowl_enable( WICED_TO_WWD_INTERFACE( interface ), wowl, wowl_os, NULL, NULL, 0, &arp_host_ip_v4_address ) );
        }
        break;
        default:
            result = WICED_BADARG;
...
wiced_result_t wiced_wifi_exit_ds1( wiced_interface_t interface )
{
    ...
    CHECK_RETURN ( wwd_wifi_ds1_finish_wake( ) );
