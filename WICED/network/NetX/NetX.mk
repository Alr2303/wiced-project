...
GLOBAL_INCLUDES := ver$(NETX_VERSION) \
                   ver$(NETX_VERSION)/netx_bsd_layer \
                   WICED


ifdef WICED_ENABLE_TRACEX
# Precompiled library with TraceX
NETX_LIBRARY_NAME :=NetX.TraceX.$(RTOS).$(HOST_ARCH)$(DOT_TOOLCHAIN_TYPE).$(BUILD_TYPE).a
else
# Precompiled library
NETX_LIBRARY_NAME :=NetX.$(RTOS).$(HOST_ARCH)$(DOT_TOOLCHAIN_TYPE).$(BUILD_TYPE).a
endif
