...
#define RETRANSMISSION_DEFAULT_START_INTERVAL          (1000) /* in ms */
...
typedef enum
{
    ...
    DTLS_ALERT_UNSUPPORTED_EXTENSION    = 110,    /* Extension not supported */
    DTLS_ALERT_UNKNOWN_PSK_IDENTITY     = 115     /* PSK identity from client is unknown */
...
dtls_result_t verify_mac( dtls_context_t* dtls, dtls_peer_t* peer, dtls_record_t* record, size_t length );
...
dtls_result_t dtls_write_alert_msg( dtls_context_t* context, dtls_session_t* session, dtls_alert_level_t alert_level, dtls_alert_t alert );
...
dtls_result_t dtls_check_epoch ( dtls_peer_t* peer, uint16_t received_epoch );
void          dtls_flush_peer_retransmission_list (dtls_context_t* context, dtls_peer_t* peer );
void          dtls_free_retransmission_buffer ( dtls_peer_t* peer, dtls_retransmission_node_t* retransmission_node );
