...
#define DTLS_PSK_MAX_CLIENT_IDENTITY_LEN   64
#define DTLS_COOKIE_LENGTH_MAX             32
#define DTLS_MAX_RETRANSMISSION_COUNT      5
...
typedef enum
{
   DTLS_CIPHER_NONE               = 0x0000,
   DTLS_CIPHER_WITH_AES_128_CCM_8 = 0xC0AE,
} dtls_cipher_t;
...
{
        uint16_t        id_length;
        unsigned char   identity[ DTLS_PSK_MAX_CLIENT_IDENTITY_LEN + 1 ];
...
typedef struct dtls_peer_t
{
        ...
        linked_list_t                retransmission_list;  /* retransmission list */

} dtls_peer_t;

typedef struct dtls_retransmission_s
{
        linked_list_node_t  this_node;
        unsigned int        retransmit_timestamp;
        ...
        wiced_bool_t        retransmit;

        size_t              length;
        uint8_t*            packet;
} dtls_retransmission_node_t;
...
typedef struct context_t
{
        ...
        void*                     retransmission_timer_event;
