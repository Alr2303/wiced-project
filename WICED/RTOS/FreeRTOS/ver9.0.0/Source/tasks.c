...
/*-----------------------------------------------------------*/
	...
	TCB_t *pxTCB;
		...
		taskENTER_CRITICAL();
		{
			...
			/* Check if the task exists before continuing. It is possible the
			task is being deleted more than once. */
			#if ( INCLUDE_eTaskGetState == 1 )
			{
				if( xTaskToDelete && eTaskGetState( xTaskToDelete ) == eDeleted )
				{
					taskEXIT_CRITICAL();
					return;
				}
			...
			#else
			{
				#error eTaskGetState muse be enabled to use vTaskDelete in WICED
			}
			#endif
...
BaseType_t xReturn;
	...
	if( xReturn == pdPASS )
	{
		...
		#if ( configUSE_NEWLIB_MALLOC_LOCK == 1 )
		{
			...
			configASSERT( newlib_malloc_mutex );
		}
