...
typedef struct wwd_nan_config_status
{
  uint8_t enabled;
  uint8_t inited;
  uint8_t joined;
  ...
  uint32_t cnt_bcn_tx;            /* TX disc/sync beacon count */
  uint32_t cnt_bcn_rx;            /* RX disc/sync beacon count */
  uint32_t cnt_svc_disc_tx;       /* TX service disc frame count */
  uint32_t cnt_svc_disc_rx;       /* RX service disc frame count */
  uint32_t ambtt;                 /* Anchor master beacon target time */
  struct ether_addr cid;          /* Cluster id */
  uint8_t hop_count;              /* Hop count */
} wwd_nan_config_status_t;
