...
platform_result_t platform_uart_init( platform_uart_driver_t* driver, const platform_uart_t* peripheral, const platform_uart_config_t* config, wiced_ring_buffer_t* optional_ring_buffer )
{
    ...
    /* Attempt to acquire the specified MFS channel for UART operation mode */
    if ( platform_mfs_manager_acquire_channel( peripheral->port ) != PLATFORM_SUCCESS )
    {
        return PLATFORM_ERROR;
    }
    ...
    {
        platform_mfs_manager_release_channel( peripheral->port );
        return PLATFORM_ERROR;
    }
...
platform_result_t platform_uart_deinit( platform_uart_driver_t* driver )
{
    ...
    if ( platform_mfs_manager_release_channel( driver->peripheral->port ) != PLATFORM_SUCCESS )
    {
        return PLATFORM_ERROR;
    }
