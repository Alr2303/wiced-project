 ...
 *                    Constants
 ******************************************************/
#define NO_OF_INTERRUPTS_SUPPORTED      ( 128 ) // For PDL_FM4_TYPE5
/******************************************************
 *                   Enumerations
 ******************************************************/
...
void platform_init_mcu_infrastructure( void )
{
    ...
    SCB->VTOR = 0x1FFE0000; /* Change the vector table to point to start of SRAM */
#endif /* ifdef INTERRUPT_VECTORS_IN_RAM */

#if !defined ( BOOTLOADER )
    /* Initialize reset-cause-result global variable.
     * This API can only be called once because the underlying
     * register is read/reset.  The PDL API will cache the
     * result in a global variable.  Since the bootloader
     * and app will go through the same start-up, call the API
     * only for the app.
    */
    {
        stc_reset_result_t stc_reset_result_dummy_variable;
        (void)Reset_GetCause( &stc_reset_result_dummy_variable );
    }
#endif /* !defined ( WICED_DISABLE_BOOTLOADER ) */
