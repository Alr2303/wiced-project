...
restart:
    ...
    else
    {
        ...
        if( Sdcmd_SendCmd( &cmd ) != Ok )
        {
            /* Make sure the command-line is clear before retrying.
             * The commmand inhibit (CMD) bit may be stuck from powering WLAN.
             */
            (void)Sdif_SoftwareReset( m_astcSdifInstanceDataLut[0].pstcInstance, SdifResetCmdLine );
            goto restart;
        }
    }
