 ...
 ******************************************************/
static unsigned int spi_get_actioncode( sflash_command_t command, uint32_t data_length );
...
int bcm4390x_sflash_semaphore_init( void )
{
    ...
    if ( sflash_semaphore_flag == WICED_FALSE )
    {
        ...
        {
            WPRINT_PLATFORM_ERROR( ( "semaphore init failed. status=%d\n", result ) );
            return result;
        }
...
int spi_sflash_send_command( sflash_handle_t* sflash_handle, sflash_command_t command, uint32_t device_address, void* data, uint32_t* data_length )
{
    ...
    if ( control_register.bits.action_code == SFLASH_ACTIONCODE_MAX_ENUM )
    {
    ...
    if ( ( ( control_register.bits.action_code == SFLASH_ACTIONCODE_3ADDRESS_4DATA ) ||
           ( control_register.bits.action_code == SFLASH_ACTIONCODE_3ADDRESS_1DATA ) ||
           ( control_register.bits.action_code == SFLASH_ACTIONCODE_3ADDRESS ) ) &&
    ...
    switch ( purpose )
    {
        case COMMAND_TXDATA :
            ...
            if ( !CHIP_SUPPORT_QUAD_ADDR_MODE( ccrev ) )
            {
                ...
                command = ( ( command == SFLASH_QUAD_WRITE4B ) ? SFLASH_WRITE4B : SFLASH_WRITE );
...
static unsigned int spi_get_actioncode( sflash_command_t command, uint32_t data_length )
{
    if ( ( command == SFLASH_WRITE_STATUS_REGISTER ) ||( command == SFLASH_READ_STATUS_REGISTER) || ( command == SFLASH_READ_STATUS_REGISTER2 ) ||
         ...
         ( command == SFLASH_EN4B ) || ( command == SFLASH_EX4B ) || ( command == SFLASH_WRITE_ENABLE_VOLATILE_REGISTER ) )
    {
        ...
        else if ( data_length == 0 )
        {
         ...
         ( command == SFLASH_READ4B ) ||( command == SFLASH_FAST_READ4B ) || ( command == SFLASH_QUAD_READ4B ) ||
         ( command == SFLASH_X4IO_READ4B ) ||( command == SFLASH_WRITE4B ) || ( command == SFLASH_QUAD_WRITE4B ) ||
         ( command == SFLASH_4K_ERASE4B ) || ( command == SFLASH_64K_ERASE4B ) )
    {
         /* - SFLASH_READ
          * - SFLASH_FAST_READ
          * - SFLASH_QUAD_READ
    ...
    WPRINT_PLATFORM_ERROR( ( "Not found properly action code for command (0x%x)\n", command ) );
    return SFLASH_ACTIONCODE_MAX_ENUM;
}

static command_purpose_t spi_get_command_purpose( sflash_command_t command )
{
    if ( ( command == SFLASH_WRITE ) || ( command == SFLASH_QUAD_WRITE ) || ( command == SFLASH_X4IO_WRITE) ||
         ( command == SFLASH_WRITE4B ) || ( command == SFLASH_QUAD_WRITE4B ) ||
         ( command == SFLASH_WRITE_ENH_VOLATILE_REGISTER ) || ( command == SFLASH_WRITE_STATUS_REGISTER ) )
    {
        return COMMAND_TXDATA;
         ...
         ( command == SFLASH_READ4B ) || ( command == SFLASH_FAST_READ4B ) || ( command == SFLASH_QUAD_READ4B ) ||
         ( command == SFLASH_X4IO_READ ) || ( command == SFLASH_X4IO_READ4B ) )
    {
    ...
    WPRINT_PLATFORM_ERROR( ( "Not found properly command purpose for command (0x%x)\n", command ) );
