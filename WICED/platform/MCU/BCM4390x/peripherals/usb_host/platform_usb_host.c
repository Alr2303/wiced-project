...
static void bcm4390x_usb_host_enable( void )
{
    ...
    if ((bcm4390x_usb_host_driver->overcurrent_pin == WICED_GPIO_NONE) || (bcm4390x_usb_host_driver->power_enable_pin == WICED_GPIO_NONE))
    {
        WPRINT_PLATFORM_ERROR( ("Necessary USB Host mode GPIOs not available!\n") );
        return;
    }
...
{
    return bcm4390x_is_board_in_usb_phy_mode();
}

wiced_bool_t platform_is_board_in_usb_host_mode( void )
{
    return bcm4390x_is_board_in_usb_host_mode();
}
...
platform_result_t platform_usb_host_init( void )
{
    ...
    if (platform_is_board_in_usb_host_mode() == WICED_FALSE)
    {
