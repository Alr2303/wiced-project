...
#endif
{
    ...
    /*
     * Disable the clock source select of localized div_by_2 which generated app_backplane_clk from armcr4_clk.
     * The clocks should be synchronous and in phase for the usb20d and DMA operation (or enumeration) to be successful!
     */
    gci_chip_ctl = platform_gci_chipcontrol( GCI_CHIPCONTROL_BP_CLK_FROM_ARMCR4_CLK_REG,
                                  GCI_CHIPCONTROL_BP_CLK_FROM_ARMCR4_CLK_MASK,
                                  0x0 );
    dbg_printf("@@@ch_reset: gci_chip_ctl=0x%x\n", gci_chip_ctl);
...
#ifdef BCMDBG
        err("ep%d: tx unaligned by: %d; mem 0x%x",
            ep, offset, ((uint32) LBP(p)->head) - LBUFSZ);
#endif
