...
static platform_result_t uart_seci_setup_internal( uart_seci_t* uart_seci_base, uint32_t clock, const platform_uart_config_t* config )
{
    ...
    /* DO not enable AUTO_RTS, this asserts RTS Low
         * When waking up from Deep-Sleep this causes problems as
         * UART is not fully initialized at this time and interrupts are disabled
         */
        //uart_seci_base->mcr |= UART_SECI_MCR_AUTO_RTS;
         uart_seci_base->mcr |= UART_SECI_MCR_AUTO_TX_DIS;
    }
    ...
    //uart_seci_base->lcr |= UART_SECI_LCR_RX_EN; //Dont enable RX now, interrupts are still disabled
...
WICED_DEEP_SLEEP_EVENT_HANDLER( platform_uart_deep_sleep_handler )
{
    switch ( event )
    {
