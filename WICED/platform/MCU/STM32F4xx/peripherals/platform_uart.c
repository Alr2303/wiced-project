...
platform_result_t platform_uart_transmit_bytes( platform_uart_driver_t* driver, const uint8_t* data_out, uint32_t size )
{
    ...
    while ( ( driver->peripheral->port->SR & USART_SR_TXE ) == 0 )
    {
    }
...
platform_result_t platform_uart_receive_bytes( platform_uart_driver_t* driver, uint8_t* data_in, uint32_t* expected_data_size, uint32_t timeout_ms )
{
    ...
    /* Invoke the APIs to enable/disable MCU powersave function only if UART does not have the wakeup function defined */
    if (driver->peripheral->wakeup_func == NULL)
    {
        platform_mcu_powersave_disable();
    }
    ...
    if ( driver->rx_buffer != NULL )
    {
		...
		if ( transfer_size > ring_buffer_used_space( driver->rx_buffer ) ) {
			/* exception state, recheck semaphore */
			continue;
		}
            }
...
{
    if (peripheral->wakeup_func != NULL)
    {
        peripheral->wakeup_func( );
    }
    return PLATFORM_SUCCESS;
}
...
{
    if ( peripheral->sleep_func != NULL )
    {
        peripheral->sleep_func( );
    }
    return PLATFORM_SUCCESS;
}
