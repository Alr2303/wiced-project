...
/** @file
 * Defines board support package for BCM943907WCD2 board
 */
#include "stdio.h"
#include "string.h"
#include "platform.h"
...
/* TODO : Move this to 90x specific instead of board-specific file */
...
#define CHIPCOMMON_SECI_FIFO_LVL_REG    *((volatile uint32_t*)(PLATFORM_CHIPCOMMON_REGBASE(0x18C)))
...
#define CHIPCOMMON_SECI_UART_LSR_REG    *((volatile uint32_t*)(PLATFORM_CHIPCOMMON_REGBASE(0x1D4)))
#define CHIPCOMMON_SECI_UART_MSR_REG    *((volatile uint32_t*)(PLATFORM_CHIPCOMMON_REGBASE(0x1D8)))
#define CHIPCOMMON_SECI_UART_BAUDADJ_REG    *((volatile uint32_t*)(PLATFORM_CHIPCOMMON_REGBASE(0x1DC)))
...
const platform_gpio_t platform_gpio_pins[] =
{
    ...
    [WICED_GPIO_30         ]    = { PIN_I2S_MCLK0 },
    [WICED_GPIO_31         ]    = { PIN_I2S_SCLK0 },
    [WICED_GPIO_32         ]    = { PIN_I2S_LRCLK0 },
    [WICED_GPIO_33         ]    = { PIN_I2S_SDATAI0 },
    [WICED_GPIO_34         ]    = { PIN_I2S_SDATAO0 },
    [WICED_GPIO_35         ]    = { PIN_I2S_MCLK1 },
    [WICED_GPIO_36         ]    = { PIN_I2S_SCLK1 },
    [WICED_GPIO_37         ]    = { PIN_I2S_LRCLK1 },
    [WICED_GPIO_38         ]    = { PIN_I2S_SDATAI1 },
    [WICED_GPIO_39         ]    = { PIN_I2S_SDATAO1 },
    ...
    [WICED_PERIPHERAL_PIN_9]    = { PIN_I2C1_SDATA },
    [WICED_PERIPHERAL_PIN_10]    = { PIN_I2C1_CLK },
    [WICED_PERIPHERAL_PIN_11]    = { PIN_SPI_1_MISO },
    [WICED_PERIPHERAL_PIN_12]    = { PIN_SPI_1_CLK },
    [WICED_PERIPHERAL_PIN_13]    = { PIN_SPI_1_MOSI },
    [WICED_PERIPHERAL_PIN_14]    = { PIN_SPI_1_CS },
    [WICED_PERIPHERAL_PIN_15]    = { PIN_SDIO_CLK },
    [WICED_PERIPHERAL_PIN_16]    = { PIN_SDIO_CMD },
    [WICED_PERIPHERAL_PIN_17]    = { PIN_SDIO_DATA_0 },
    [WICED_PERIPHERAL_PIN_18]    = { PIN_SDIO_DATA_1 },
    [WICED_PERIPHERAL_PIN_19]    = { PIN_SDIO_DATA_2 },
    [WICED_PERIPHERAL_PIN_20]    = { PIN_SDIO_DATA_3 },
};
...
{
    .baud_rate    = PLATFORM_UART_BAUDRATE_DEFAULT,
...
const platform_gpio_t* wiced_bt_control_pins[WICED_BT_PIN_MAX] =
{
    ...
    [WICED_BT_PIN_HOST_WAKE]   = &platform_gpio_pins[WICED_GPIO_8],
    [WICED_BT_PIN_DEVICE_WAKE] = &platform_gpio_pins[WICED_GPIO_10]
};
...
{
    printf(" platform wakeup handler\n");

    /* Acknowledge WAKEUP , so IRQ is not triggered again */
    platform_mcu_powersave_gci_gpio_wakeup_ack( PIN_GPIO_12 );

    /* Disable WAKEUP from BT_HOST_WAKE (PIN_GPIO_14 is BT_HOST_WAKE) */
    platform_mcu_powersave_gci_gpio_wakeup_disable( PIN_GPIO_12 );
...
/* This is a temp function to assert the DEV_WAKE informing the BT chip to go to low power mode */
...
#if 0
    /* if only sleep mode */
    platform_gpio_irq_enable( &platform_gpio_pins[WICED_GPIO_8], IRQ_TRIGGER_BOTH_EDGES, platform_powersave_wakeup_handler, NULL );

    platform_gpio_output_high(&platform_gpio_pins[WICED_GPIO_10]);
#endif

    /* if deep sleep mode*/
    /* Use PIN_GPIO_12 is configured as BT_HOST_WAKE, configure it as GCI gpio to wake up 4390x from deep-sleep */
    result = platform_mcu_powersave_gci_gpio_wakeup_enable( PIN_GPIO_12, PLATFORM_MCU_POWERSAVE_GPIO_WAKEUP_CONFIG_HIGH_IMPEDANCE,
                    PLATFORM_GCI_GPIO_IRQ_TRIGGER_LEVEL_HIGH  );

     if ( result != PLATFORM_SUCCESS )
     {
          printf("Error in configuring GPIO to wake up MCU from deep sleep");
      ...
      platform_gpio_output_high( wiced_bt_control_pins[ WICED_BT_PIN_DEVICE_WAKE ] );
     ...
     CHIPCOMMON_SECI_UART_FCR_REG |= 0x3 ;
      ...
      host_rtos_delay_milliseconds(120000);

      /* In case of deep-sleepm the below code would not execute, after waking up from deep-sleep
       * BCM4390x will go to the reset vector -> ROM bootloader -> 2nd stage bootloader -> application_start()
       */
      platform_mcu_powersave_disable();
      printf("Out of sleep : platform sleep handler, if you are seeing this print, 4390x did not enter deep sleep\n");
