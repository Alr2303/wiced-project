...
/** @file
 * Defines board support package for BCM943907WAE2_1 board
 */
#include "stdio.h"
#include "string.h"
#include "platform.h"
...
/* This used after waking from Sleep */
void platform_bt_host_wake_irq_handler( void* arg )
{
    return;
}

/* This used after waking from Deep-Sleep */
void platform_powersave_wakeup_handler( void* arg )
{
    platform_mcu_powersave_bluetooth_wakeup_handler( wiced_bt_control_pins[ WICED_BT_PIN_DEVICE_WAKE ], wiced_bt_control_pins[ WICED_BT_PIN_HOST_WAKE ] );
}

#define SLEEP_DURATION_MS ( 20000 )
platform_result_t platform_powersave_sleep_handler( void )
{
    char sleep_string[ ]     = "Sleep";
    char deepsleep_string[ ] = "Deepsleep";
    char *str;
    wiced_result_t result;

    result = platform_gpio_irq_enable( wiced_bt_control_pins[ WICED_BT_PIN_HOST_WAKE ], IRQ_TRIGGER_RISING_EDGE, platform_bt_host_wake_irq_handler, NULL );
    if ( result != WICED_SUCCESS )
    {
        printf("Failed to request irq for GPIO pin\n");
    }

    result = platform_gpio_deepsleep_wakeup_enable( wiced_bt_control_pins[ WICED_BT_PIN_HOST_WAKE ], IRQ_TRIGGER_LEVEL_HIGH );
    if ( result != WICED_SUCCESS )
    {
        printf("Failed to request deepsleep wakeup for GPIO pin\n");
    }

    ( platform_mcu_powersave_get_mode( ) == PLATFORM_MCU_POWERSAVE_MODE_DEEP_SLEEP ) ? ( str = deepsleep_string ) : ( str = sleep_string );

    platform_mcu_powersave_bluetooth_sleep_handler( wiced_bt_control_pins[ WICED_BT_PIN_HOST_WAKE ] );

    platform_mcu_powersave_enable( );

    printf( "\nGoing to %s\n", str );
    host_rtos_delay_milliseconds( SLEEP_DURATION_MS );

    /* In case of deep-sleep, the below code would not execute, after waking up from deep-sleep
     * BCM4390x will go to the reset vector -> ROM bootloader -> 2nd stage bootloader -> application_start()
     */
    printf( "Out of sleep : 4390x did not enter deep sleep\n" );
    return PLATFORM_SUCCESS;
}
