...
static void http_test_indexed_chunked_header            ( http_connection_t* connect );
static wiced_result_t http_recv_header_callback         ( http_connection_ptr_t connect, http_request_id_t request, http_header_info_t* headers, http_frame_type_t type, uint8_t flags, void* user_data );
static wiced_result_t http_recv_data_callback           ( http_connection_ptr_t connect, http_request_id_t request, uint8_t* data, uint32_t lenght, uint8_t flags, void* user_data );
...
void application_start( void )
{
    ...
    ret = http_connection_init(&connect, security_p, &callbacks, scratch, sizeof(scratch), &app_semaphore);
    ...
    while (i-- > 0)
    {
        int request_counts = 2;
        RUN_COMMAND_PRINT_STATUS_AND_BREAK_ON_ERROR( http_connect( &connect, &http_server_address, port, WICED_STA_INTERFACE ), "HTTP Connect.", "HTTP connect. Did you check your HTTP server IP address?" );
        wiced_rtos_delay_milliseconds(1000);
        while ( request_counts-- )
        {
            //wiced_rtos_get_semaphore( &app_semaphore, WICED_NEVER_TIMEOUT );
            WPRINT_APP_INFO(("\r\n SENDING REQUEST - %d\r\n", request_counts));
            WPRINT_APP_INFO(("\r\n -------------------- \r\n"));
            if (request_counts % 2 == 0)
            {
                http_test_indexed_header( &connect );
            }
            else
            {
                http_test_indexed_chunked_header( &connect );
            }
        ...
        wiced_rtos_delay_milliseconds(1000);
        //wiced_rtos_get_semaphore( &app_semaphore, NEVER_TIMEOUT );
    }
...
static void http_test_indexed_header ( http_connection_t* connect )
{
    ...
    http_request_create( connect, &request );

    flag  = HTTP_REQUEST_FLAGS_FINISH_HEADERS;
    flag |= HTTP_REQUEST_FLAGS_FINISH_REQUEST;
    http_request_put_headers( connect, request, source_headers, flag );
}
...
static void http_test_indexed_chunked_header ( http_connection_t* connect )
{
    ...
    WPRINT_APP_INFO((" ==== Sending Chunked Frame (Header) Request\n"));
    /* Create a request */
    http_request_create( connect, &request );

    /* Send part-1 of the header */
    WPRINT_APP_INFO(("Sending Part-1 of the Header\n"));
    flag  = 0;
    http_request_put_headers( connect, request, &source_headers[0], flag );

    /* Send part-2 of the header */
    WPRINT_APP_INFO(("Sending Part-2 of the Header\n"));
    flag  = 0;
    http_request_put_headers( connect, request, &source_headers[2], flag );

    /* Send part-2 of the header */
    WPRINT_APP_INFO(("Sending Final part of the Header\n"));
    flag  = HTTP_REQUEST_FLAGS_FINISH_HEADERS;
    flag |= HTTP_REQUEST_FLAGS_FINISH_REQUEST;
    http_request_put_headers( connect, request, &source_headers[3], flag );
}
...
{
    char buffer[200];
    if ( flags & HTTP_REQUEST_FLAGS_PUSH_PROMISE )
    {
 ...
 */
static wiced_result_t http_recv_data_callback   ( http_connection_ptr_t connect, http_request_id_t request, uint8_t* data, uint32_t length, uint8_t flags, void* user_data )
{
    uint32_t i;
    wiced_semaphore_t* semaphore = (wiced_semaphore_t*)user_data;
