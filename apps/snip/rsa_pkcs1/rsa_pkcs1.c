...
void application_start(void)
{
    ...
    rsa_init( &rsa_priv, RSA_PKCS_V15, 0, NULL, NULL );

    if( ( ret = mpi_read_string( &rsa_priv.N , 16, RSA_PRIVATE_N ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.E , 16, RAS_PRIVATE_E ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.D , 16, RSA_PRIVATE_D ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.P , 16, RSA_PRIVATE_P ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.Q , 16, RSA_PRIVATE_Q ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.DP, 16, RSA_PRIVATE_DP ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.DQ, 16, RSA_PRIVATE_DQ ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_priv.QP, 16, RSA_PRIVATE_QP ) ) != 0 )
    {
        WPRINT_APP_INFO(("RSA Private Key Init Failed: mpi_read_file returned %d\n", ( int ) ret));
        return;
    }

    rsa_priv.length = ( mpi_msb( &rsa_priv.N ) + 7 ) >> 3;

    WPRINT_APP_INFO(("RSA Private Key Init Done. key size = %u bits\n", ( unsigned ) rsa_priv.length * 8));
    ...
    rsa_init( &rsa_pub, RSA_PKCS_V15, 0, NULL, NULL );

    if( ( ret = mpi_read_string( &rsa_pub.N , 16, RSA_PUBLIC_N ) ) != 0 ||
        ( ret = mpi_read_string( &rsa_pub.E , 16, RSA_PUBLIC_E ) ) != 0 )
    {
        WPRINT_APP_INFO(("RSA Public Key Init Failed: mpi_read_file returned %d\n", ( int ) ret));
        return;
    }

    rsa_pub.length = ( mpi_msb( &rsa_priv.N ) + 7 ) >> 3;

    WPRINT_APP_INFO(("RSA Public Key Init Done. key size = %u bits\n", ( unsigned ) rsa_pub.length * 8));
    ...
    sha2( ( const unsigned char * ) MESSAGE, n, hash, 0);

    WPRINT_APP_INFO(("PKCS#1 Signing with Private Key...\n"));
    if( ( ret = rsa_pkcs1_sign( &rsa_priv, RSA_PRIVATE, RSA_SHA256,
                                0, hash, signature ) ) != 0 )
    {
    ...
    if( ( ret = rsa_pkcs1_verify( &rsa_priv, RSA_PUBLIC, RSA_SHA256,
                                  0, hash, signature ) ) != 0 )
    {
    ...
    if( ( ret = rsa_pkcs1_sign( &rsa_priv, RSA_PUBLIC, RSA_SHA256,
                                0, hash, signature ) ) != 0 )
    {
    ...
    if( ( ret = rsa_pkcs1_verify( &rsa_priv, RSA_PRIVATE, RSA_SHA256,
                                  0, hash, signature ) ) != 0 )
    {
