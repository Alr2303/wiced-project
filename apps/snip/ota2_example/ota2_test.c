...
int ota2_console_command(int argc, char *argv[])
{
    ...
    switch (i)
    {
        ...
        case OTA2_CONSOLE_CMD_GET_UPDATE:
            ...
            {
                OTA2_APP_PRINT(OTA2_LOG_INFO, (" Use app_dct->ota2_update_uri\r\n"));
...
wiced_result_t my_ota2_callback(void* session_id, wiced_ota2_service_status_t status, uint32_t value, void* opaque )
{
                                         ...
                                         *                           the download, the OTA Service will do nothing.  */
    {
        ...
        {
            OTA2_APP_PRINT(OTA2_LOG_NOTIFY, ("OTA2 Update Version Fail - return ERROR, do not update!\r\n"));
            return WICED_ERROR;
        }
#endif
    ...
    case OTA2_SERVICE_UPDATE_ENDED:     /* All update actions for this check are complete.
        ...
        {
            OTA2_APP_PRINT(OTA2_LOG_ERROR, ("        REBOOTING !!!\r\n"));
...
wiced_result_t ota2_test_get_update(ota2_data_t* player)
{
    ...
    if (player->ota2_bg_service != NULL)
    {
        ...
        if (player->ota2_bg_service != NULL)
        {
            ...
            wiced_ota2_service_set_debug_log_level(g_player->ota2_bg_service, player->log_level);
...
wiced_result_t ota2_test_start_timed_update(ota2_data_t* player)
{
    ...
    {
        player->ota2_bg_service = wiced_ota2_service_init(&player->ota2_bg_params, player);
        OTA2_APP_PRINT(OTA2_LOG_ERROR, ("ota2_test_start_timed_update() wiced_ota2_service_init() bg_service:%p \r\n", player->ota2_bg_service));
    }
    if (player->ota2_bg_service != NULL)
    {
        /* add a callback */
        ...
        if (result != WICED_SUCCESS)
        {
                OTA2_APP_PRINT(OTA2_LOG_ERROR, ("ota2_test_start_timed_update register callback failed! %d \r\n", result));
                wiced_ota2_service_deinit(player->ota2_bg_service);
                player->ota2_bg_service = NULL;
        }
        ...
        if (player->ota2_bg_service != NULL)
        {
            ...
            if (result != WICED_SUCCESS)
            {
                    OTA2_APP_PRINT(OTA2_LOG_ERROR, ("ota2_test_start_timed_update wiced_ota2_service_start() failed! %d \r\n", result));
            }
...
static void ota2_test_mainloop(ota2_data_t *player)
{
    ...
    while (player->tag == PLAYER_TAG_VALID)
    {
        ...
        if (events & PLAYER_EVENT_DISCONNECT_AP)
        {
            ...
            player->ota2_bg_params.ota2_interface = player->dct_network->interface;
            result = wiced_ota2_network_down( player->ota2_bg_params.ota2_interface, &player->saved_network_parameters);
            if (result != WICED_SUCCESS)
            {
        ...
        if (events & PLAYER_EVENT_CONNECT_OTA2_AP)
        {
            ...
            if(player->dct_wifi->stored_ap_list[1].details.SSID.length == 0)
            {
                OTA2_APP_PRINT(OTA2_LOG_ERROR, ("PLAYER_EVENT_CONNECT_OTA2_AP: No secondary AP defined!\r\n"));
            }
            else
            {
                /* disconnect from current AP and connect to OTA2 update AP */
                player->ota2_bg_params.ota2_interface = player->dct_network->interface;
                result = wiced_ota2_network_up(player->ota2_bg_params.ota2_interface, &player->dct_wifi->stored_ap_list[1], &player->saved_network_parameters);
                if (result != WICED_SUCCESS)
                {
                    OTA2_APP_PRINT(OTA2_LOG_ERROR, ("PLAYER_EVENT_CONNECT_OTA2_AP: Bringing up network interface failed!\r\n"));
                }
            }
        }
        ...
        if (events & PLAYER_EVENT_CONNECT_DEFAULT_AP)
        {
            ...
            player->ota2_bg_params.ota2_interface = player->dct_network->interface;
            result = wiced_ota2_network_up(player->ota2_bg_params.ota2_interface, &player->dct_wifi->stored_ap_list[0], &player->saved_network_parameters);
            if (result != WICED_SUCCESS)
            {
        ...
        if (events & PLAYER_EVENT_FACTORY_RESET_NOW)
        {
            ...
            OTA2_APP_PRINT(OTA2_LOG_NOTIFY, ("Extract the OTA2 Image in the Factory Reset Area NOW (will FAIL on Internal Flash systems)\r\n"));
        ...
        if (events & PLAYER_EVENT_UPDATE_NOW)
        {
            ...
            OTA2_APP_PRINT(OTA2_LOG_NOTIFY, ("Extract the OTA2 Image in the Staged Area NOW (will FAIL on Internal Flash systems)\r\n"));
...
static ota2_data_t* init_player(void)
{
   ...
   /*
    ...
    OTA2_APP_PRINT(OTA2_LOG_INFO, ("OTA2_IMAGE_FAILSAFE_APP_AREA_BASE    0x%08lx\n", (uint32_t)OTA2_IMAGE_FAILSAFE_APP_AREA_BASE));
    ...
    OTA2_APP_PRINT(OTA2_LOG_INFO, ("OTA2_IMAGE_CURR_DCT_2_AREA_BASE      0x%08lx\n", (uint32_t)OTA2_IMAGE_CURR_DCT_2_AREA_BASE));
    OTA2_APP_PRINT(OTA2_LOG_INFO, ("OTA2_IMAGE_CURR_OTA_APP_AREA_BASE    0x%08lx\n", (uint32_t)OTA2_IMAGE_CURR_OTA_APP_AREA_BASE));
    ...
    switch( boot_type )
    {
        ...
        default:
            ...
            OTA2_APP_PRINT(OTA2_LOG_INFO, ("Unexpected boot_type %d!\r\n", boot_type));
            /* FALL THROUGH */
