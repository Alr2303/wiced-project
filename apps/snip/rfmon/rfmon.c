...
typedef struct {
    ...
    float last_rtp_drop_rate;
    float last_aud_drop_rate;
    float rtp_ma;
    float aud_ma;
    int rssi_ma;
...
/* Histogram styles */
#define RF_DRAW_CCA             (1 << 0)    /* Histo based on CCA results */
#define RF_DRAW_RTP             (1 << 1)    /* Histo based on audio/RTP reports collected from sinks */
struct {
...
void application_start( )
{
    ...
    while (1)
    {
        ...
        /* Measure load across all channels.  Skip if SKIP_CONGEST flag is on */
        if (!SKIP_CONGEST()) {
...
draw(u8g_t *u8g, const int *channels, int *scores)
{
    ...
    do {
        ...
        if (user_pref.draw_style & RF_DRAW_CCA)
            make_page_CCA(u8g, channels, g_scores);

        if (user_pref.draw_style & RF_DRAW_RTP)
            make_page_RTP(u8g);
...
print_stats_header()
{
    WPRINT_APP_INFO(("                 Last/Avg    RTP   Last/%d Pt MA   Last/%d Pt MA   Spkr  Num Reports/\n",
        MA_WIN_SZ, MA_WIN_SZ));
    WPRINT_APP_INFO(("   MAC Address     RSSI    Drop/Rx   RTP PER       Audio PER     Name  Secs Since Last  Dups\n"));
}
...
print_stats()
{
    ...
    for (i = 0; i < MAX_DEVICES; i++) {
        stats = &apollo_stats[i];
        if (memcmp(stats->mac_addr, empty, MACADDR_SZ) != 0) {
            WPRINT_APP_INFO((""MACDBG" %d/%d  %lu/%lu   %1.3f%%/%1.3f%%  %1.3f%%/%1.3f%%",
...
rfmon_console_command(int argc, char *argv[])
{
    ...
    switch (i)
    {
        ...
        case RFMON_CONSOLE_HISTO:
            ...
            if (argc >= 2) {
                /* Accept "CCA" or "cca" */
                WPRINT_APP_INFO(("rf_draw [\"CCA\" | \"RTP\"], Default: RTP\n"));
                if (!(strncmp(argv[1], "CCA", strlen("CCA"))) || !(strncmp(argv[1], "cca", strlen("cca")))) {
                    ...
                    /* Anything else gets RTP reports */
...
WPRINT_APP_INFO(("    ---------        -----         --------------------------   --------------------------\n"));
    ...
    WPRINT_APP_INFO(("   RTP: %lu%-7lu/", rf_hi(stats->rtp_packets_dropped), rf_lo(stats->rtp_packets_dropped)));
    WPRINT_APP_INFO(("%lu%-8lu", rf_hi(stats->rtp_packets_received), rf_lo(stats->rtp_packets_received)));
    if (delta)
        WPRINT_APP_INFO(("/%lu", delta->rtp_drop));

    WPRINT_APP_INFO(("\t"));
    WPRINT_APP_INFO(("Aud: %lu%-6lu/", rf_hi(stats->audio_frames_dropped), rf_lo(stats->audio_frames_dropped)));
    WPRINT_APP_INFO(("%lu%-10lu", rf_hi(stats->audio_frames_played), rf_lo(stats->audio_frames_played)));
    if (delta)
...
**/
    ...
    if (stats->num_rpts_rx > MIN_REPORTS) {
        ...
        stats->delta.rtp_drop = incoming->rtp_packets_dropped  - stats->rtp_packets_dropped;
        ...
        if (!SKIP_CONGEST()) {
            ...
            if (stats->last_rtp_drop_rate >= rf_thresh.instant_rtp_thresh) {
                ...
                if (!cause->reason) {
                    WPRINT_APP_INFO(("Momentary RTP for "MACDBG"[%1.3f%%] >= thresh[%d%%], num_rpts %d\n",
            ...
            if (stats->last_aud_drop_rate >= rf_thresh.instant_aud_thresh) {
                ...
                if (!cause->reason) {
                    WPRINT_APP_INFO(("Momentary AUD for "MACDBG"[%1.3f%%] >= thresh[%d%%]\n",
        ...
        if (user_pref.report_raw == 1)  {
            dump_raw_report(macaddr, incoming, &stats->delta, 0);
        }
        if (user_pref.report_raw == 2)  {
...
sniff_report(csa_cause_t *cause, int action)
{
    ...
    while ((result = wiced_udp_receive(&rfg.stats_socket, &report_packet, WICED_NO_WAIT)) == WICED_SUCCESS) {
        ...
        if (report->msg_type == APOLLO_REPORT_MSG_STATS) {
            if (report->msg_length == sizeof(apollo_report_stats_t)) {
                ...
                WPRINT_APP_INFO(("%s: Stats rpt: bad msg len: 0x%x\n", __FUNCTION__, (unsigned int)report->msg_length));
            }
        }
        wiced_packet_delete(report_packet);
    }
