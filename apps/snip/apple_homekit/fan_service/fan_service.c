...
/** @file
 *
 * HomeKit Fan Accessory Snippet
 *
 * This snippet demonstrates how to setup a fan accessory
 *
 ...
 *
 *     MFi Mode
 *        This uses WAC to start a soft AP and bring the Wiced HomeKit Fan Control Accessory onto the network
 ...
 *
 *        snip.apple_homekit.fan_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 *        This uses the wifi credentials stored in wifi_config_dct.h to bring up the Wiced HomeKit Fan Control Accessory
 *        This advertises pin only support, and does not use MFi certificate during pairing
 *
 *        Build string in this mode:
 *
 *        snip.apple_homekit.fan_service-BCM9WCDPLUS114
 *
 * Once the Wiced HomeKit Fan Control Accessory is on the network, it can be discovered by the HAT application or iPhone/iPad app.
 ...
 *
 *      If running MFi mode, use the AirPort utility on the MAC (or  iPhone/iPad ), to bring up the HomeKit Fan Accessory
 *
 * HomeKit Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Fan Control Accessory, start the HAT application once the Wiced HomeKit Fan Control Accessory has been added to
 *   your local network.
 ...
 *     2. For this controller, press "Start" to start listening for the Wiced HomeKit Fan Control Accessory
 ...
 *   Please keep these points in mind when exploring the Wiced HomeKit Fan Control Accessory snippet.
 *
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Fan Control Accessory-"
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-fan-test",
...
apple_homekit_accessory_config_t apple_homekit_discovery_settings =
{
    ...
    .accessory_category_identifier  = FAN
};


/* Accessory definitions */
wiced_homekit_accessories_t fan_control_accessory;

/* Accessory Information Service and Characteristics */
wiced_homekit_services_t        fan_accessory_information_service;
wiced_homekit_characteristics_t fan_accessory_identify_characteristic;
wiced_homekit_characteristics_t fan_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t fan_accessory_model_characteristic;
wiced_homekit_characteristics_t fan_accessory_name_characteristic;
wiced_homekit_characteristics_t fan_accessory_serial_number_characteristic;
...
/* Fan accessory information service characteristic values */
char fan_accessory_name_value[MAX_NAME_LENGTH] = "\""MAIN_ACCESSORY_NAME"\"";
char fan_accessory_manufacturer_value[]        = "\"Fan Co.\"";
char fan_accessory_model_value[]               = "\"Fan Controller Model 1.0\"";
char fan_accessory_serial_number_value[]       = "\"12345678\"";
...
void application_start( )
{
    ...
    *(accessory_name + offset) = '\0';
    ...
    if ( result != WICED_SUCCESS )
    {
       WPRINT_APP_INFO(("Invalid Entry in DCT (or) the configured WiFi network is not UP at present. Starting WAC\n"));
        ...
        if ( apple_wac_info.out_new_name != NULL )
        {
            ...
            snprintf( fan_accessory_name_value, sizeof(fan_accessory_name_value), "\"%s\"", apple_wac_info.out_new_name );
            free( apple_wac_info.out_new_name );
        }
    }
    ...
    /* Initialise fan accessory information service */
    wiced_homekit_initialise_accessory_information_service( &fan_accessory_information_service, iid++ );
    ...
    /* Information Fan Service Characteristics */
    wiced_homekit_initialise_name_characteristic         ( &fan_accessory_name_characteristic, fan_accessory_name_value, strlen( fan_accessory_name_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_identify_characteristic     ( &fan_accessory_identify_characteristic, identify_action, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &fan_accessory_manufacturer_characteristic, fan_accessory_manufacturer_value, strlen( fan_accessory_manufacturer_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &fan_accessory_model_characteristic, fan_accessory_model_value, strlen( fan_accessory_model_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &fan_accessory_serial_number_characteristic, fan_accessory_serial_number_value, strlen( fan_accessory_serial_number_value ), NULL, NULL, iid++ );
    ...
    /******************************
     * Create Fan Control Accessory
     ******************************/
    wiced_homekit_add_accessory( &fan_control_accessory, 1 );

    /* Add information service */
    wiced_homekit_add_service( &fan_control_accessory, &fan_accessory_information_service );

    wiced_homekit_add_characteristic( &fan_accessory_information_service, &fan_accessory_identify_characteristic      );
    wiced_homekit_add_characteristic( &fan_accessory_information_service, &fan_accessory_manufacturer_characteristic  );
    wiced_homekit_add_characteristic( &fan_accessory_information_service, &fan_accessory_model_characteristic         );
    wiced_homekit_add_characteristic( &fan_accessory_information_service, &fan_accessory_name_characteristic          );
    wiced_homekit_add_characteristic( &fan_accessory_information_service, &fan_accessory_serial_number_characteristic );

    /* Add fan service */
    wiced_homekit_add_service( &fan_control_accessory, &fan_service );
    ...
    while( 1 )
    {
        value           = fan_on_characteristic.value.current.value;
        ...
        if ( button1_pressed )
        {
            ...
            {
                fan_on_value[0]= '0';
                WPRINT_APP_INFO(("\n Received fan-service OFF request \n"));
                wiced_gpio_output_low( WICED_LED1 );
            }
            else
            {
                fan_on_value[0]= '1';
                WPRINT_APP_INFO(("\n Received fan-service ON request \n"));
             ...
             **/
            wiced_homekit_register_characteristic_value_update( &fan_control_accessory, &fan_on_characteristic, fan_on_value, 1);
        }
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    ...
    for ( i = 0; i < update_list->number_of_updates; i++ )
    {
        ...
        if ( fan_control_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            ...
            if ( fan_on_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                wiced_homekit_accept_controller_value( &fan_on_characteristic );

                /**
                 * Update the current value length( value.current.value_length), and place this characteristic value
                 * into a list of events to be sent to the controller (if controller enabled events for this characteristic)
                 **/
                wiced_homekit_register_characteristic_value_update( &fan_control_accessory, &fan_on_characteristic, fan_on_characteristic.value.new.value, fan_on_characteristic.value.new.value_length );
           }
        }
        else
        {
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    /* Check if this is the Fan Accessory */
    if ( fan_control_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n FAN CONTROL ACCESSORY IDENTIFY \r\n"));
