...
/** @file
 *
 * HomeKit Thermostat Accessory Snippet
 *
 * This snippet demonstrates how to setup a thermostat service
 *
 ...
 *
 *     MFi Mode
 *        This uses WAC to start a soft AP and bring the Wiced HomeKit Thermostat Accessory onto the network
 ...
 *
 *        snip.apple_homekit.thermostat_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 *        This uses the wifi credentials stored in wifi_config_dct.h to bring up the Wiced HomeKit Thermostat Accessory
 *        This advertises pin only support, and does not use MFi certificate during pairing
 *
 *        Build string in this mode:
 *
 *        snip.apple_homekit.thermostat_service-BCM9WCDPLUS114
 *
 *
 * Once the Wiced HomeKit Thermostat Accessory is on the network, it can be discovered by the HAT application or iPhone/iPad app.
 ...
 *
 *      If running MFi mode, use the AirPort utility on the MAC (or  iPhone/iPad ), to bring up the Wiced HomeKit Thermostat Accessory
 *
 * HomeKit Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Thermostat Accessory, start the HAT application once the Wiced HomeKit Thermostat Accessory has been added to
 *   your local network.
 ...
 *     2. For this controller, press "Start" to start listening for the Wiced HomeKit Thermostat Accessory
 ...
 *   Please keep these points in mind when exploring the Wiced HomeKit Thermostat Accessory snippet.
 *
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Thermostat-"
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-thermostat-test",
...
apple_homekit_accessory_config_t apple_homekit_discovery_settings =
{
    ...
    .accessory_category_identifier  = THERMOSTAT
};


/* Accessory definitions */
wiced_homekit_accessories_t thermostat_accessory;

/* Accessory Information Service and Characteristics */
wiced_homekit_services_t        thermostat_accessory_information_service;
wiced_homekit_characteristics_t thermostat_accessory_identify_characteristic;
wiced_homekit_characteristics_t thermostat_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t thermostat_accessory_model_characteristic;
wiced_homekit_characteristics_t thermostat_accessory_name_characteristic;
wiced_homekit_characteristics_t thermostat_accessory_serial_number_characteristic;

/* Thermostat services and characteristics */
wiced_homekit_services_t        thermostat_service;
wiced_homekit_characteristics_t heating_cooling_current_characteristic;
wiced_homekit_characteristics_t heating_cooling_target_characteristic;
...
char thermostat_accessory_name_value[MAX_NAME_LENGTH] = "\""MAIN_ACCESSORY_NAME"\"";
    ...
    *(accessory_name + offset) = '\0';
    ...
    if ( result != WICED_SUCCESS )
    {
        ...
        if ( apple_wac_info.out_new_name != NULL )
        {
            ...
            snprintf( thermostat_accessory_name_value, sizeof(thermostat_accessory_name_value), "\"%s\"", apple_wac_info.out_new_name );
            free( apple_wac_info.out_new_name );
        }
    }
    ...
    /* Initialise Thermostat Accessory Information Service */
    wiced_homekit_initialise_accessory_information_service( &thermostat_accessory_information_service, iid++ );
    ...
    wiced_homekit_initialise_name_characteristic         ( &thermostat_accessory_name_characteristic, thermostat_accessory_name_value, strlen( thermostat_accessory_name_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_identify_characteristic     ( &thermostat_accessory_identify_characteristic, identify_action, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &thermostat_accessory_manufacturer_characteristic, thermostat_accessory_manufacturer_value, strlen( thermostat_accessory_manufacturer_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &thermostat_accessory_model_characteristic, thermostat_accessory_model_value, strlen( thermostat_accessory_model_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &thermostat_accessory_serial_number_characteristic, thermostat_accessory_serial_number_value, strlen( thermostat_accessory_serial_number_value ), NULL, NULL, iid++ );

    /* Initialise Thermostat characteristics */
    wiced_homekit_initialise_heating_cooling_current_characteristic ( &heating_cooling_current_characteristic, heating_cooling_current_value, strlen( heating_cooling_current_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_heating_cooling_target_characteristic  ( &heating_cooling_target_characteristic, heating_cooling_target_value, strlen( heating_cooling_target_value ), NULL, NULL, iid++ );
    ...
    wiced_homekit_add_accessory( &thermostat_accessory, 1 );

    /* Add Thermostat Accessory Information Service */
    wiced_homekit_add_service( &thermostat_accessory, &thermostat_accessory_information_service );

    wiced_homekit_add_characteristic( &thermostat_accessory_information_service, &thermostat_accessory_identify_characteristic      );
    wiced_homekit_add_characteristic( &thermostat_accessory_information_service, &thermostat_accessory_manufacturer_characteristic  );
    wiced_homekit_add_characteristic( &thermostat_accessory_information_service, &thermostat_accessory_model_characteristic         );
    wiced_homekit_add_characteristic( &thermostat_accessory_information_service, &thermostat_accessory_name_characteristic          );
    wiced_homekit_add_characteristic( &thermostat_accessory_information_service, &thermostat_accessory_serial_number_characteristic );

    /* Add Thermostat Service to Accessory */
    wiced_homekit_add_service( &thermostat_accessory, &thermostat_service );

    /* Add the required characteristics to the Thermostat Service */
    wiced_homekit_add_characteristic( &thermostat_service, &heating_cooling_current_characteristic );
    wiced_homekit_add_characteristic( &thermostat_service, &heating_cooling_target_characteristic );
    ...
    while( 1 )
    {
        ...
        if ( button1_pressed )
        {
            if ( ( heating_cooling_current_characteristic.value.current.value[0] == '1' ) ||
                 ( heating_cooling_current_characteristic.value.current.value[0] == 't' ) )
            {
             ...
             **/
            wiced_homekit_register_characteristic_value_update( &thermostat_accessory, &heating_cooling_current_characteristic, heating_cooling_current_value, 1);
        }
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    ...
    for ( i = 0; i < update_list->number_of_updates; i++ )
    {
        ...
        if ( thermostat_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            WPRINT_APP_INFO(("\r\n THERMOSTAT ACCESSORY \r\n"));

            if ( heating_cooling_target_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                updated_characteristic = &heating_cooling_target_characteristic;

                switch ( heating_cooling_target_characteristic.value.new.value[ 0 ] )
                {
                    case '0':
            ...
            if ( updated_characteristic != NULL )
            {
                ...
                if( memcmp( updated_characteristic->value.new.value, updated_characteristic->value.current.value, updated_characteristic->value.current.value_length ) != COMPARE_MATCH )
                {
                     ...
                     * into a list of events to be sent to the controller (if controller enabled events for this characteristic)
                     **/
                    wiced_homekit_register_characteristic_value_update( &thermostat_accessory, updated_characteristic, updated_characteristic->value.current.value, updated_characteristic->value.current.value_length );
                }
            }
        }
        else
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    if( thermostat_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n THERMOSTAT ACCESSORY IDENTIFY \r\n"));
