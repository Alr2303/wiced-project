...
/** @file
 *
 * HomeKit Dynamic Accessory Snippet
 *
 * This snippet demonstrates how to setup an accessory and remove services and characteristics from it dynamically
 *
 ...
 *
 *        snip.apple_homekit.dynamic_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 ...
 *        Build string in this mode:
 *
 *        snip.apple_homekit.dynamic_service-BCM9WCDPLUS114
 *
 ...
 *  - Adding of multiple services and characteristics dynamically
 ...
 *
 *      If running MFi mode, use the AirPort utility on the MAC (or  iPhone/iPad ), to bring up the HomeKit Dynamic Accessory
 *
 * HomeKit Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Accessory, start the HAT application once the Wiced HomeKit Dynamic Accessory has been added to
 *   your local network.
 ...
 *
 *
 *      To switch between adding/removing service and charactersitic state, toggle SW1. This wil cycle thought the add/remove
 *      service and characteristic state machine in the example snippet.
 *
 *      HAT takes some time to issue a re-discover. To quicken the process quit HAT and restart, then press 'Discover'.
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Dynamic Accessory-"
...
typedef enum
{
     ADD_LIGHT_SERVICE,
     REMOVE_FAN_SERVICE,
     REMOVE_LIGHT_SERVICE,
     ADD_OPTIONAL_LIGHT_CHARACTERITIC,
     REMOVE_OPTIONAL_LIGHT_CHARACTERITIC,
     ADD_FAN_SERVICE
} dynamic_states_t;
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-dynamic-test",
...
wiced_homekit_accessories_t dynamic_accessory;

/* Accessory Information Service and Characteristics */
wiced_homekit_services_t        dynamic_accessory_information_service;
wiced_homekit_characteristics_t dynamic_accessory_identify_characteristic;
wiced_homekit_characteristics_t dynamic_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t dynamic_accessory_model_characteristic;
wiced_homekit_characteristics_t dynamic_accessory_name_characteristic;
wiced_homekit_characteristics_t dynamic_accessory_serial_number_characteristic;
...
/* Dynamic accessory information service characteristic values */
char dynamic_accessory_name_value[MAX_NAME_LENGTH]  = "\""MAIN_ACCESSORY_NAME"\"";
char dynamic_accessory_manufacturer_value[]         = "\"Changes\"";
char dynamic_accessory_model_value[]                = "\"Changes Model 1.0\"";
char dynamic_accessory_serial_number_value[]        = "\"12345678\"";

/* fan characteristics values */
char fan_on_value[]    = "false";
char fan_name_value[]  = "\"Fan \"";
...
void application_start( )
{
    ...
    dynamic_states_t switch_services = ADD_LIGHT_SERVICE;
    wiced_mac_t      mac;
    ...
    *(accessory_name + offset) = '\0';
    ...
    if ( result != WICED_SUCCESS )
    {
       ...
       result = apple_wac_configure( &apple_wac_info );
        ...
        if ( apple_wac_info.out_new_name != NULL )
        {
            ...
            snprintf( dynamic_accessory_name_value, sizeof(dynamic_accessory_name_value), "\"%s\"", apple_wac_info.out_new_name );
            free( apple_wac_info.out_new_name );
        }
    }
    ...
    /* Initialise dynamic accessory information service */
    wiced_homekit_initialise_accessory_information_service( &dynamic_accessory_information_service, iid++ );
    ...
    wiced_homekit_initialise_identify_characteristic     ( &dynamic_accessory_identify_characteristic, identify_action, iid++ );
    wiced_homekit_initialise_name_characteristic         ( &dynamic_accessory_name_characteristic,          dynamic_accessory_name_value,          strlen( dynamic_accessory_name_value ),          NULL, NULL, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &dynamic_accessory_manufacturer_characteristic,  dynamic_accessory_manufacturer_value,  strlen( dynamic_accessory_manufacturer_value ),  NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &dynamic_accessory_model_characteristic,         dynamic_accessory_model_value,         strlen( dynamic_accessory_model_value ),         NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &dynamic_accessory_serial_number_characteristic, dynamic_accessory_serial_number_value, strlen( dynamic_accessory_serial_number_value ), NULL, NULL, iid++ );
    ...
    /******************************
     * Create dynamic Control Accessory
     ******************************/
    wiced_homekit_add_accessory( &dynamic_accessory, 1 );

    /* Add information service */
    wiced_homekit_add_service( &dynamic_accessory, &dynamic_accessory_information_service );

    wiced_homekit_add_characteristic( &dynamic_accessory_information_service, &dynamic_accessory_identify_characteristic      );
    wiced_homekit_add_characteristic( &dynamic_accessory_information_service, &dynamic_accessory_manufacturer_characteristic  );
    wiced_homekit_add_characteristic( &dynamic_accessory_information_service, &dynamic_accessory_model_characteristic         );
    wiced_homekit_add_characteristic( &dynamic_accessory_information_service, &dynamic_accessory_name_characteristic          );
    wiced_homekit_add_characteristic( &dynamic_accessory_information_service, &dynamic_accessory_serial_number_characteristic );

    /* Add fan service */
    wiced_homekit_add_service( &dynamic_accessory, &fan_service );
    ...
    /* Add the required characteristics to the lightbulb service, but dont add the lightbulb service to accessory */
    ...
    while( 1 )
    {
        ...
        if ( button1_pressed )
        {
            switch ( switch_services )
            {
                case ADD_LIGHT_SERVICE:
                    /* Add lightbulb service to accessory */
                    wiced_homekit_add_service( &dynamic_accessory, &lightbulb_service );

                    WPRINT_APP_INFO(("\r\nAdded Lightbulb service.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = REMOVE_FAN_SERVICE;
                    break;

                case REMOVE_FAN_SERVICE:
                    /* Remove Fan service */
                    wiced_homekit_remove_service( &dynamic_accessory, &fan_service );

                    WPRINT_APP_INFO(("\r\nRemoved Fan service.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = REMOVE_OPTIONAL_LIGHT_CHARACTERITIC;
                    break;

                case REMOVE_OPTIONAL_LIGHT_CHARACTERITIC:
                    /* Remove optional lightbulb characteristics to service */
                    wiced_homekit_remove_characteristic( &lightbulb_service, &lightbulb_hue_characteristic );
                    wiced_homekit_remove_characteristic( &lightbulb_service, &lightbulb_brightness_characteristic );
                    wiced_homekit_remove_characteristic( &lightbulb_service, &lightbulb_saturation_characteristic );
                    wiced_homekit_remove_characteristic( &lightbulb_service, &lightbulb_name_characteristic );

                    WPRINT_APP_INFO(("\r\nRemoved all optional lightbulb service characteristics.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = ADD_OPTIONAL_LIGHT_CHARACTERITIC;
                    break;

                case ADD_OPTIONAL_LIGHT_CHARACTERITIC:
                    /* Add optional lightbulb characteristics to service */
                    wiced_homekit_add_characteristic( &lightbulb_service, &lightbulb_hue_characteristic );
                    wiced_homekit_add_characteristic( &lightbulb_service, &lightbulb_brightness_characteristic );
                    wiced_homekit_add_characteristic( &lightbulb_service, &lightbulb_saturation_characteristic );
                    wiced_homekit_add_characteristic( &lightbulb_service, &lightbulb_name_characteristic );

                    WPRINT_APP_INFO(("\r\nAdded all optional lightbulb service characteristics.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = ADD_FAN_SERVICE;
                    break;

                case ADD_FAN_SERVICE:
                    /* Add the fan service back */
                    wiced_homekit_add_service( &dynamic_accessory, &fan_service );

                    WPRINT_APP_INFO(("\r\nAdded fan service.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = REMOVE_LIGHT_SERVICE;
                    break;

                case REMOVE_LIGHT_SERVICE:
                    /* Remove lightbulb service */
                    wiced_homekit_remove_service( &dynamic_accessory, &lightbulb_service );

                    WPRINT_APP_INFO(("\r\nRemoved Lightbulb service.\r\n New database size = %lu\r\n", wiced_homekit_get_current_accessory_database_size() ));

                    switch_services = ADD_LIGHT_SERVICE;
                    break;
            }
        }
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    ...
    {
        /* If dynamic accessory, then check to see what services are being activated */
        if ( dynamic_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            WPRINT_APP_INFO(("\r\n DYNAMIC ACCESSORY \r\n"));

            if ( fan_on_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
            ...
            if ( updated_characteristic != NULL )
            {
               /* If the requested value is different to the current value/state update to new value */
                if ( memcmp( updated_characteristic->value.new.value, updated_characteristic->value.current.value, updated_characteristic->value.current.value_length ) != COMPARE_MATCH )
                {
                    ...
                    **/
                   wiced_homekit_register_characteristic_value_update( &dynamic_accessory, updated_characteristic, updated_characteristic->value.new.value, updated_characteristic->value.new.value_length );
               }
           }
        }
        else
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    /* Check if this is the dynamic accessory */
    if( dynamic_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n DYNAMIC SERVICES EXAMPLE IDENTIFY \r\n"));
