...
/** @file
 *
 * HomeKit Lightbulb Accessory with Shared DCT Snippet
 *
 * This snippet demonstrates how to setup a lightbulb service, which shares the
 * Application DCT with HomeKit to store its on/off value, hue and brightness
 *
...
char lightbulb_hue_value[]         = "xxx"; /* The initial value for this will be read from Application DCT */
char lightbulb_brightness_value[]  = "xxx"; /* The initial value for this will be read from Application DCT */
char lightbulb_saturation_value[]  = "xxx"; /* The initial value for this will be read from Application DCT */
...
void application_start( )
{
    ...
    *(accessory_name + offset) = '\0';
    ...
    snprintf(lightbulb_on_value, sizeof(lightbulb_on_value), "%s", ( ( app_dct->on == 1 ) ? "true" : "false" ) );
    isOn =  app_dct->on;
    snprintf(lightbulb_hue_value, sizeof(lightbulb_hue_value), "%d", app_dct->hue );
    snprintf(lightbulb_brightness_value, sizeof(lightbulb_brightness_value), "%d", app_dct->brightness );
    snprintf(lightbulb_saturation_value, sizeof(lightbulb_saturation_value), "%d", app_dct->saturation );

    WPRINT_APP_INFO(("+-------------------------------------------+\r\n"));
    WPRINT_APP_INFO(("+ Preserved Lightbulb Characteristic Values +\r\n"));
    WPRINT_APP_INFO(("+-------------------------------------------+\r\n"));
    WPRINT_APP_INFO(("ON         = [%s] \r\n", lightbulb_on_value));
    WPRINT_APP_INFO(("HUE        = [%s] \r\n", lightbulb_hue_value));
    WPRINT_APP_INFO(("SATURATION = [%s] \r\n", lightbulb_saturation_value));
    WPRINT_APP_INFO(("BRIGHTNESS = [%s] \r\n", lightbulb_brightness_value));
    WPRINT_APP_INFO(("+-------------------------------------------+\r\n"));

    wiced_dct_read_unlock( (void*)app_dct, WICED_FALSE );

    /* Initialise Lightbulb characteristics based on the last preserved value  */
    ...
    /* Set accessory bulb status based on recently preserved values */
    if ( isOn )
    {
        WPRINT_APP_INFO(("\r\n\t Light ON\r\n" ));
    ...
    while( 1 )
    {
        value           =  lightbulb_on_characteristic.value.current.value;
    ...
    for ( i = 0; i < update_list->number_of_updates; i++ )
    {
        ...
        if ( lightbulb_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            ...
            if ( lightbulb_on_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                offset = HOMEKIT_APP_DCT_OFFSET(on);
            }
            else if ( lightbulb_hue_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                val = (uint16_t)atoi((const char*)light_characteristic);
                offset = HOMEKIT_APP_DCT_OFFSET(hue);
            }
            else if ( lightbulb_brightness_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                val = (uint16_t)atoi((const char*)light_characteristic);
                offset = HOMEKIT_APP_DCT_OFFSET(brightness);
            }
            else if ( lightbulb_saturation_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                val = (uint16_t)atoi((const char*)light_characteristic);
                offset = HOMEKIT_APP_DCT_OFFSET(saturation);
            }
            ...
            if ( updated_characteristic != NULL )
            {
                ...
                if ( memcmp( updated_characteristic->value.new.value, updated_characteristic->value.current.value, updated_characteristic->value.current.value_length ) != COMPARE_MATCH )
                {
                    ...
                    wiced_dct_read_lock( (void**)&pval, WICED_TRUE, DCT_HOMEKIT_APP_SECTION, offset, sizeof( *pval ) );

                    *pval = val;
                    wiced_dct_write( (const void *)pval, DCT_HOMEKIT_APP_SECTION, offset, sizeof( *pval )  );
