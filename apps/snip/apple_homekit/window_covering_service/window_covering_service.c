...
/** @file
 *
 * HomeKit Window Covering Accessory Snippet
 *
 * This snippet demonstrates how to setup a window_covering accessory, with a garage controller
 *
 ...
 *
 *     MFi Mode
 *        This uses WAC to start a soft AP and bring the Wiced HomeKit Window Covering Accessory onto the network
 ...
 *
 *        make snip.apple_homekit.window_covering_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 *        This uses the wifi credentials stored in wifi_config_dct.h to bring up the Wiced HomeKit Window Covering Accessory
 *        This advertises pin only support, and does not use MFi certificate during pairing
 *
 *        Build string in this mode:
 *
 *        make snip.apple_homekit.window_covering_service-BCM9WCDPLUS114
 *
 *
 * Once the Wiced HomeKit Window Covering Accessory is on the network, it can be discovered by the HAT application or iPhone/iPad app.
 ...
 *
 * Wiced HomeKit Window Covering Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Window Covering Accessory, start the HAT application once the Wiced HomeKit Window Covering Accessory has been added to
 *   your local network.
 ...
 *     2. For this controller, press "Start" to start listening for the Wiced HomeKit Window Covering Accessory
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Window Covering-"
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-window_covering-test",
...
apple_homekit_accessory_config_t apple_homekit_discovery_settings =
{
    ...
    .accessory_category_identifier  = WINDOW_COVERING
};

/* Accessory definitions */
wiced_homekit_accessories_t window_covering_accessory;

/* Window Covering Accessory Information Service and Characteristics */
wiced_homekit_services_t        window_covering_accessory_information_service;
wiced_homekit_characteristics_t window_covering_accessory_identify_characteristic;
wiced_homekit_characteristics_t window_covering_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t window_covering_accessory_model_characteristic;
wiced_homekit_characteristics_t window_covering_accessory_name_characteristic;
wiced_homekit_characteristics_t window_covering_accessory_serial_number_characteristic;
...
/* Window Covering accessory information service characteristic values */
char window_covering_controller_name_value[]           = "\""MAIN_ACCESSORY_NAME"\"";
char window_covering_controller_manufacturer_value[]   = "\"Window Covering Co.\"";
char window_covering_controller_model_value[]          = "\"Controller 1.0\"";
char window_covering_controller_serial_number_value[]  = "\"12345678\"";

/* Window Covering characteristics values */
char window_covering_current_position[]   = "100";
char window_covering_target_position[]    = "100";
char window_covering_position_state[]     = "2";
...
void application_start( )
{
    ...
    *(accessory_name + offset) = '\0';
    ...
    /* Initialise window_covering accessory information service */
    wiced_homekit_initialise_accessory_information_service( &window_covering_accessory_information_service, iid++ );

    /* Initialise window_covering service */
    ...
    /* Information Window Covering Service Characteristics */
    wiced_homekit_initialise_name_characteristic         ( &window_covering_accessory_name_characteristic, window_covering_controller_name_value, strlen( window_covering_controller_name_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_identify_characteristic     ( &window_covering_accessory_identify_characteristic, identify_action, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &window_covering_accessory_manufacturer_characteristic, window_covering_controller_manufacturer_value, strlen( window_covering_controller_manufacturer_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &window_covering_accessory_model_characteristic, window_covering_controller_model_value, strlen( window_covering_controller_model_value ), NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &window_covering_accessory_serial_number_characteristic, window_covering_controller_serial_number_value, strlen( window_covering_controller_serial_number_value ), NULL, NULL, iid++ );

    /* Initialise window_covering characteristics */
    wiced_homekit_initialise_current_position_characteristic ( &current_position_characteristic, window_covering_current_position, strlen( window_covering_current_position ), NULL, NULL, iid++ );
    wiced_homekit_initialise_target_position_characteristic  ( &target_position_characteristic, window_covering_target_position, strlen( window_covering_target_position ), NULL, NULL, iid++ );
    wiced_homekit_initialise_position_state_characteristic   ( &position_state_characteristic, window_covering_position_state, strlen( window_covering_position_state ), NULL, NULL, iid++ );


    /******************************
     * Create Window Covering Accessory
     ******************************/
    wiced_homekit_add_accessory( &window_covering_accessory, 1 );

    /* Add accessory information service to accessory */
    wiced_homekit_add_service( &window_covering_accessory, &window_covering_accessory_information_service );

    /* Add the required characteristics to the window_covering accessory information service */
    wiced_homekit_add_characteristic( &window_covering_accessory_information_service, &window_covering_accessory_name_characteristic );
    wiced_homekit_add_characteristic( &window_covering_accessory_information_service, &window_covering_accessory_identify_characteristic );
    wiced_homekit_add_characteristic( &window_covering_accessory_information_service, &window_covering_accessory_manufacturer_characteristic );
    wiced_homekit_add_characteristic( &window_covering_accessory_information_service, &window_covering_accessory_model_characteristic );
    wiced_homekit_add_characteristic( &window_covering_accessory_information_service, &window_covering_accessory_serial_number_characteristic );

    /* Add window_covering service to accessory */
    wiced_homekit_add_service( &window_covering_accessory, &window_covering_service );

    /* Add the required characteristics to the window_covering service */
    ...
    while( 1 )
    {
        ...
        if( button1_pressed )
        {
             ...
             **/
            wiced_homekit_register_characteristic_value_update( &window_covering_accessory, &position_state_characteristic, &current_position_value, sizeof(current_position_value));
        }
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    ...
    for ( i = 0; i < update_list->number_of_updates; i++ )
    {
        ...
        /* If window_covering door accessory, then check to see what services are being activated */
        if ( window_covering_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            WPRINT_APP_INFO(("\r\n WINDOW COVERING ACCESSORY \r\n"));

            /* Receive target window_covering state from controller and place it into local buffer */
            if ( target_position_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {

                WPRINT_APP_INFO(("\r\n\t Received Window Covering Target Position: %.*s \r\n", target_position_characteristic.value.new.value_length, target_position_characteristic.value.new.value ));
                ...
                if ( memcmp( target_position_characteristic.value.new.value, target_position_characteristic.value.current.value, target_position_characteristic.value.current.value_length ) != COMPARE_MATCH )
                {
                    ...
                    if( memcmp(current_position_characteristic.value.current.value,target_position_characteristic.value.current.value,current_position_characteristic.value.current.value_length) != COMPARE_MATCH)
                    {
                         ...
                         * into a list of events to be sent to the controller (if controller enabled events for this characteristic)
                         **/
                        wiced_homekit_register_characteristic_value_update( &window_covering_accessory, &target_position_characteristic, target_position_characteristic.value.new.value, target_position_characteristic.value.new.value_length );
                    }
                }
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    /* Check if this is the Window Covering Control accessory */
    if ( accessory_id == 0)
    {
        WPRINT_APP_INFO(("\r\n WINDOW COVERING ACCESSORY UNPAIRED IDENTIFY \r\n"));
    ...
    else if( window_covering_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n WINDOW COVERING ACCESSORY PAIRED IDENTIFY \r\n"));
