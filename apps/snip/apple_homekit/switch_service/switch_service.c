...
/** @file
 *
 * HomeKit Switch Accessory Snippet
 *
 * This snippet demonstrates how to setup a switch service
 *
 ...
 *
 *     MFi Mode
 *        This uses WAC to start a soft AP and bring the Wiced HomeKit Switch Accessory onto the network
 ...
 *
 *        snip.apple_homekit.switch_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 *        This uses the wifi credentials stored in wifi_config_dct.h to bring up the Wiced HomeKit Switch Accessory
 *        This advertises pin only support, and does not use MFi certificate during pairing
 *
 *        Build string in this mode:
 *
 *        snip.apple_homekit.switch_service-BCM9WCDPLUS114
 *
 *
 * Once the Wiced HomeKit Switch Accessory is on the network, it can be discovered by the HAT application or iPhone/iPad app.
 ...
 *
 *      If running MFi mode, use the AirPort utility on the MAC (or  iPhone/iPad ), to bring up the Wiced HomeKit Switch Accessory
 *
 * HomeKit Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Switch Accessory, start the HAT application once the Wiced HomeKit Switch Accessory has been added to
 *   your local network.
 ...
 *     2. For this controller, press "Start" to start listening for the Wiced HomeKit Switch Accessory
 ...
 *   Please keep these points in mind when exploring the Wiced HomeKit Switch Accessory snippet.
 *
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Switch Accessory-"
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-switch-test",
...
apple_homekit_accessory_config_t apple_homekit_discovery_settings =
{
    ...
    .accessory_category_identifier  = SWITCH
};


/* Accessory definitions */
wiced_homekit_accessories_t switch_accessory;

/* Accessory Information Service and Characteristics */
wiced_homekit_services_t        switch_accessory_information_service;
wiced_homekit_characteristics_t switch_accessory_identify_characteristic;
wiced_homekit_characteristics_t switch_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t switch_accessory_model_characteristic;
wiced_homekit_characteristics_t switch_accessory_name_characteristic;
wiced_homekit_characteristics_t switch_accessory_serial_number_characteristic;
...
char switch_accessory_name_value[MAX_NAME_LENGTH] = "\""MAIN_ACCESSORY_NAME"\"";
char switch_accessory_manufacturer_value[]        = "\"Switch Co.\"";
char switch_accessory_model_value[]               = "\"Controller 1.0\"";
char switch_accessory_serial_number_value[]       = "\"12345678\"";

/* switch characteristics values */
char switch_name_value[]  = "\"Main Switch \"";
char switch_on_value[]    = "false";
...
void application_start( )
{
    ...
    *(accessory_name + offset) = '\0';
    ...
    if ( result != WICED_SUCCESS )
    {
       WPRINT_APP_INFO(("Invalid Entry in DCT (or) the configured WiFi network is not UP at present. Starting WAC\n"));
        ...
        if ( apple_wac_info.out_new_name != NULL )
        {
            ...
            snprintf( switch_accessory_name_value, sizeof(switch_accessory_name_value), "\"%s\"", apple_wac_info.out_new_name );
            free( apple_wac_info.out_new_name );
        }
    }
    ...
    /* Initialise switch accessory information service */
    wiced_homekit_initialise_accessory_information_service( &switch_accessory_information_service, iid++ );
    ...
    /* Switch Information Service Characteristics */
    wiced_homekit_initialise_identify_characteristic     ( &switch_accessory_identify_characteristic,      identify_action, iid++ );
    wiced_homekit_initialise_name_characteristic         ( &switch_accessory_name_characteristic,          switch_accessory_name_value,          strlen( switch_accessory_name_value ),          NULL, NULL, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &switch_accessory_manufacturer_characteristic,  switch_accessory_manufacturer_value,  strlen( switch_accessory_manufacturer_value ),  NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &switch_accessory_model_characteristic,         switch_accessory_model_value,         strlen( switch_accessory_model_value ),         NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &switch_accessory_serial_number_characteristic, switch_accessory_serial_number_value, strlen( switch_accessory_serial_number_value ), NULL, NULL, iid++ );
    ...
    wiced_homekit_add_accessory( &switch_accessory, 1 );

    /* Add Switch Accessory Information Service */
    wiced_homekit_add_service( &switch_accessory, &switch_accessory_information_service );

    wiced_homekit_add_characteristic( &switch_accessory_information_service, &switch_accessory_identify_characteristic      );
    wiced_homekit_add_characteristic( &switch_accessory_information_service, &switch_accessory_manufacturer_characteristic  );
    wiced_homekit_add_characteristic( &switch_accessory_information_service, &switch_accessory_model_characteristic         );
    wiced_homekit_add_characteristic( &switch_accessory_information_service, &switch_accessory_name_characteristic          );
    wiced_homekit_add_characteristic( &switch_accessory_information_service, &switch_accessory_serial_number_characteristic );

    /* Add Switch Outlet Service to Accessory */
    wiced_homekit_add_service( &switch_accessory, &switch_service );
    ...
    while( 1 )
    {
        ...
        if ( button1_pressed )
        {
            if ( ( switch_on_characteristic.value.current.value[0] == '1' ) ||
                 ( switch_on_characteristic.value.current.value[0] == 't' ) )
            {
                switch_on_value[0]= '0';
                WPRINT_APP_INFO(("\n Received switch service OFF request \n"));
                wiced_gpio_output_low( WICED_LED1 );
            }
            else
            {
                switch_on_value[0]= '1';
                WPRINT_APP_INFO(("\n Received switch service ON request \n"));
             ...
             **/
            wiced_homekit_register_characteristic_value_update( &switch_accessory, &switch_on_characteristic, switch_on_value, 1);
        }
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    ...
    {
        /* If switch accessory, then check to see what services are being activated */
        if ( switch_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            WPRINT_APP_INFO(("\r\n SWITCH ACCESSORY \r\n"));

            if ( switch_on_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                if ( memcmp( switch_on_characteristic.value.new.value, switch_on_characteristic.value.current.value, switch_on_characteristic.value.current.value_length ) != COMPARE_MATCH )
                {
                    /* Copies the new value requested by controller (value.new) to the current value (memory pointed to by value.current) */
                    wiced_homekit_accept_controller_value( &switch_on_characteristic );

                    /**
                     * Update the current value length( value.current.value_length), and place this characteristic value
                     * into a list of events to be sent to the controller (if controller enabled events for this characteristic)
                     **/
                    wiced_homekit_register_characteristic_value_update( &switch_accessory, &switch_on_characteristic, switch_on_characteristic.value.new.value, switch_on_characteristic.value.new.value_length );
                }
            }
        }
        else
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    /* Check if this is the Switch Outlet Accessory */
    if ( switch_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n SWITCH ACCESSORY IDENTIFY \r\n"));

        /* Flash the LED representing the switch */
        for ( i = 0; i < 5; i++ )
        {
