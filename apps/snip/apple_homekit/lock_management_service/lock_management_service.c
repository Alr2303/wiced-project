...
/** @file
 *
 * HomeKit Lock Management Accessory Snippet
 *
 * This snippet demonstrates how to setup a Lock Management Service
 * Note that Lock Management is a secondary service, which means it requires a primary service in order to exist.
 * In this case we added Lock Mechanism as the primary service
 * It can not exist alone
 *
 ...
 *
 *     MFi Mode
 *        This uses WAC to start a soft AP and bring the Wiced HomeKit Lock Management Accessory onto the network
 ...
 *
 *        snip.apple_homekit.lock_management_service-BCM9WCDPLUS114 USE_MFI=1
 *
 *     Non-MFi Mode
 *        This uses the wifi credentials stored in wifi_config_dct.h to bring up the Wiced HomeKit Lock Control Accessory
 *        This advertises pin only support, and does not use MFi certificate during pairing
 *
 *        Build string in this mode:
 *
 *        snip.apple_homekit.lock_management_service-BCM9WCDPLUS114
 *
 ...
 *
 *      If running MFi mode, use the AirPort utility on the MAC (or  iPhone/iPad ), to bring up the HomeKit Lock Management Accessory
 *
 * HomeKit Accessory Demonstration
 *   To demonstrate the Wiced HomeKit Lock Control Accessory, start the HAT application once the Wiced HomeKit Lock Management Control Accessory has been added to
 *   your local network.
 ...
 *     2. For this controller, press "Start" to start listening for the Wiced HomeKit Lock Control Accessory
 ...
 *   Please keep these points in mind when exploring the Wiced HomeKit Lock Control Accessory snippet.
 *
...
#define MAIN_ACCESSORY_NAME     "Wiced IP Lock Control Accessory-"
...
apple_wac_info_t apple_wac_info =
{
    ...
    .mdns_desired_hostname  = "homekit-lock_management-test",
...
wiced_homekit_accessories_t lock_control_accessory;

/* Accessory Information Service and Characteristics */
wiced_homekit_services_t        lock_control_accessory_information_service;
wiced_homekit_characteristics_t lock_control_accessory_identify_characteristic;
wiced_homekit_characteristics_t lock_control_accessory_manufacturer_characteristic;
wiced_homekit_characteristics_t lock_control_accessory_model_characteristic;
wiced_homekit_characteristics_t lock_control_accessory_name_characteristic;
wiced_homekit_characteristics_t lock_control_accessory_serial_number_characteristic;
...
/* Lock Management service and characteristics */
...
/* Lock management accessory information service characteristic values */
char lock_control_accessory_name_value[MAX_NAME_LENGTH] = "\""MAIN_ACCESSORY_NAME"\"";
char lock_control_accessory_manufacturer_value[]        = "\"Lock Co.\"";
char lock_control_accessory_model_value[]               = "\"Lock Controller Model 1.0\"";
char lock_control_accessory_serial_number_value[]       = "\"12345678\"";

/* Lock Management characteristics values */
char lock_management_control_point_value[20] = "\"AAEA\"";
char lock_management_version_value[]  = "\"1.0\"";
...
void application_start( )
{
    ...
    *(accessory_name + offset) = '\0';
    ...
    if ( result != WICED_SUCCESS )
    {
       WPRINT_APP_INFO(("Invalid Entry in DCT (or) the configured WiFi network is not UP at present. Starting WAC\n"));
        ...
        if ( apple_wac_info.out_new_name != NULL )
        {
            ...
            snprintf( lock_control_accessory_name_value, sizeof(lock_control_accessory_name_value), "\"%s\"", apple_wac_info.out_new_name );
            free( apple_wac_info.out_new_name );
        }
    }
    ...
    /* Initialise lock management accessory information service */
    wiced_homekit_initialise_accessory_information_service( &lock_control_accessory_information_service, iid++ );

    /* Initialise lock management service */
    ...
    /* Initialise Lock Controller Information Service Characteristics */
    wiced_homekit_initialise_identify_characteristic     ( &lock_control_accessory_identify_characteristic, identify_action, iid++ );
    wiced_homekit_initialise_name_characteristic         ( &lock_control_accessory_name_characteristic,          lock_control_accessory_name_value,          strlen( lock_control_accessory_name_value ),          NULL, NULL, iid++ );
    wiced_homekit_initialise_manufacturer_characteristic ( &lock_control_accessory_manufacturer_characteristic,  lock_control_accessory_manufacturer_value,  strlen( lock_control_accessory_manufacturer_value ),  NULL, NULL, iid++ );
    wiced_homekit_initialise_model_characteristic        ( &lock_control_accessory_model_characteristic,         lock_control_accessory_model_value,         strlen( lock_control_accessory_model_value ),         NULL, NULL, iid++ );
    wiced_homekit_initialise_serial_number_characteristic( &lock_control_accessory_serial_number_characteristic, lock_control_accessory_serial_number_value, strlen( lock_control_accessory_serial_number_value ), NULL, NULL, iid++ );
    ...
    /* Initialise Lock Management characteristics */
    ...
    /* Create Lock Management Accessory */
    wiced_homekit_add_accessory( &lock_control_accessory, 1 );

    /* Add information service */
    wiced_homekit_add_service( &lock_control_accessory, &lock_control_accessory_information_service );

    wiced_homekit_add_characteristic( &lock_control_accessory_information_service, &lock_control_accessory_identify_characteristic      );
    wiced_homekit_add_characteristic( &lock_control_accessory_information_service, &lock_control_accessory_manufacturer_characteristic  );
    wiced_homekit_add_characteristic( &lock_control_accessory_information_service, &lock_control_accessory_model_characteristic         );
    wiced_homekit_add_characteristic( &lock_control_accessory_information_service, &lock_control_accessory_name_characteristic          );
    wiced_homekit_add_characteristic( &lock_control_accessory_information_service, &lock_control_accessory_serial_number_characteristic );

    /* Add lock mechanism service */
    wiced_homekit_add_service( &lock_control_accessory, &lock_mechanism_service );
    ...
    wiced_homekit_add_service( &lock_control_accessory, &lock_management_service );
...
wiced_result_t wiced_homekit_actions_on_remote_value_update( wiced_homekit_characteristic_value_update_list_t* update_list )
{
    int     i, j;
    int     len;
    uint8_t tlv8_frame[100];
    lock_control_point_type_t type;

    /* Cycle through list of all updated characteristics */
    for ( i = 0; i < update_list->number_of_updates; i++ )
    {
        /* If Lock Management accessory, then check to see what services are being activated */
        if ( lock_control_accessory.instance_id == update_list->characteristic_descriptor[ i ].accessory_instance_id )
        {
            WPRINT_APP_INFO(("\r\n LOCK MANAGEMENT ACCESSORY \r\n"));

            if ( lock_management_control_point_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                len = MIN( sizeof(tlv8_frame) - 1, lock_management_control_point_characteristic.value.new.value_length );
                ...
                for( j = 0; j < len; j++)
                {
                    WPRINT_APP_INFO(("%X", tlv8_frame[j] ));
                }
                WPRINT_APP_INFO(("]\r\n"));
            ...
            else if ( lock_mechanism_target_state_characteristic.instance_id == update_list->characteristic_descriptor[i].characteristic_instance_id )
            {
                ...
                if ( memcmp( lock_mechanism_target_state_characteristic.value.new.value, lock_mechanism_target_state_characteristic.value.current.value, lock_mechanism_target_state_characteristic.value.current.value_length ) != COMPARE_MATCH )
                {
                     ...
                     **/
                    wiced_homekit_register_characteristic_value_update( &lock_control_accessory, &lock_mechanism_target_state_characteristic, lock_mechanism_target_state_characteristic.value.current.value, lock_mechanism_target_state_characteristic.value.current.value_length );
                }
            }
        }
        else
...
wiced_result_t identify_action( uint16_t accessory_id, uint16_t characteristic_id )
{
    ...
    /* Check if this is the Lock Management accessory */
    if ( lock_control_accessory.instance_id == accessory_id )
    {
        WPRINT_APP_INFO(("\r\n LOCK CONTROL ACCESSORY IDENTIFY \r\n"));

        /* Flash the LED representing the lock management */
        for ( i = 0; i < 5; i++ )
        {
