...
extern platform_result_t platform_powersave_sleep_handler( void );
extern platform_result_t platform_powersave_wakeup_handler( void );

void app_powersave_sleep( void )
{
    /* For 4390x platforms(Embedded Mode) the below function is dummy.
    ...
    if ( wiced_bt_dev_allow_host_sleep( ) == WICED_TRUE )
    {
        ...
        platform_powersave_sleep_handler( );
    }
}
...
static int hello_sensor_set_adv_tx_power( int argc, char *argv[] )
{
    ...
    if ( !is_connected )
    {
        ...
        {
            WPRINT_APP_INFO(("TX power can be set only between range -16db to +12db \n"));
        }
        return ERR_CMD_OK;
    }
    else
...
static void application_deep_sleep_event_handler( wiced_deep_sleep_event_type_t event, int handler )
{
    ...
    switch ( event )
    {
        case WICED_DEEP_SLEEP_EVENT_ENTER:
            WPRINT_BT_APP_INFO( ( "ble_hello_sensor : Entering  Low power mode" ) );
            break;

        case WICED_DEEP_SLEEP_EVENT_CANCEL:
            WPRINT_BT_APP_INFO( ( "ble_hello_sensor : Cancelling Low power mode \n") );
            platform_powersave_wakeup_handler();
            break;

        case WICED_DEEP_SLEEP_EVENT_LEAVE:
            WPRINT_BT_APP_INFO( ( "ble_hello_sensor : Leaving Low power mode \n") );
            platform_powersave_wakeup_handler();
            break;
