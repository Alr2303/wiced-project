...
void application_start( )
{
    ...
    wiced_time_get_time(&time_before_writing);
    wiced_dct_write( (const void*) app_dct, DCT_APP_SECTION, OFFSETOF(dct_read_write_app_dct_t, uint8_var), sizeof(uint8_t) );
    wiced_time_get_time(&time_after_writing);
    WPRINT_APP_INFO( ( "  > Write data : %ldms\r\n", time_after_writing - time_before_writing) );
    ...
    wiced_time_get_time(&time_before_writing);
    wiced_dct_write( (const void*) app_dct, DCT_APP_SECTION, OFFSETOF(dct_read_write_app_dct_t, uint32_var), sizeof(uint32_t) );
    wiced_time_get_time(&time_after_writing);
    WPRINT_APP_INFO( ( "  > Write data : %ldms\r\n", time_after_writing - time_before_writing) );
