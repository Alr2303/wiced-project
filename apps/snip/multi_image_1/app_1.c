...
/** @file
 *
 *  Multi Image Application 1
 *
 * This application snippet, along with multi_image_0 illustrate how to
 * use the Deepsleep / tiny_bootloader functionality to switch applications.
 *
 * Please see  snip/multi_image_0/app_0.c and snip/multi_image_0/README.txt for more details.
 *
 */
...
#include "app_1.h"                  /* This application header */
...
#define APP_CONSOLE_COMMANDS \
    ...
    { (char*) "str_app",        app_console_command,    0, NULL, NULL, (char *)"", (char *)"Change App 1 DCT string" }, \
...
static app_1_context_t*  g_app_ctx;
/******************************************************
...
void application_start(void)
{
    ...
    WPRINT_APP_INFO(( "Starting Application 1\n" ));
    ...
    {
        WPRINT_APP_INFO(( "App_1 application_initialize() failed !\r\n\r\n"));
        exit(1);
    }
    ...
    WPRINT_APP_INFO(( "\r\n      Application 1 started! wakeup_count:%ld \r\n\r\n", wakeup_count));
...
{
    wiced_result_t      result;
    app_1_context_t*    app_ctx = NULL;
    wiced_time_t        get_up_time;


    /* allocate our Application's context */
    app_ctx = (app_1_context_t*)calloc_named("App 2", 1, sizeof(app_1_context_t));
    if (app_ctx == NULL)
    {
...
static void application_print_info( app_1_context_t* app_ctx )
{
    ...
    WPRINT_APP_INFO(("  App 1 DCT:\n"));
    WPRINT_APP_INFO(("            app_1_string:%s\n", app_dct->app_1_data.app_1_string));
...
{
    if ((g_app_ctx != NULL) && (g_app_ctx->tag == APP_1_VALID_TAG))
    {
        if (value != NULL)
        {
...
static void application_change_string_app_1( char * string )
{
    if ((g_app_ctx != NULL) && (g_app_ctx->tag == APP_1_VALID_TAG))
    {
        if (string != NULL)
        {
            strlcpy(g_app_ctx->common_dct.app_1_data.app_1_string, string, sizeof(g_app_ctx->common_dct.app_1_data.app_1_string));
            wiced_dct_write(&g_app_ctx->common_dct, DCT_APP_SECTION, 0, sizeof( multi_image_dct_t ) );
        }
        WPRINT_APP_INFO(( "app_1_data.app_1_string: %s\n", g_app_ctx->common_dct.app_1_data.app_1_string ));
    }
}
...
int app_console_command(int argc, char *argv[])
{
    ...
    switch (i)
    {
        ...
        case APP_CONSOLE_CMD_STRING_APP_1:
            application_change_string_app_1( argv[1] );
            break;
        ...
        case APP_CONSOLE_CMD_SWITCH:
            ...
            platform_deepsleep_set_boot(DCT_APP0_INDEX);
