 ...
 *                      Macros
 ******************************************************/
#define GSPI_DEVICE_SEND_BUFFER_SIZE ( 1024 )
#define GSPI_DEVICE_RECEIVE_BUFFER_SIZE ( 1024 )
...
static wiced_result_t process_command_callback( gspi_sw_tx_header_t* gspi_sw_tx_hdr )
{
    ...
    data_address = gspi_sw_tx_hdr->gspi_sw_hdr.address;
    data_length = gspi_sw_tx_hdr->gspi_sw_hdr.data_length;

    if (gspi_sw_tx_hdr->gspi_sw_hdr.direction == GSPI_MASTER_TRANSFER_WRITE )
    {
        WPRINT_APP_INFO( ("%s: WRITE, datat_address : 0x%lx, data length: %ld\n", __FUNCTION__, data_address,
                            data_length) );
        memcpy(receive_buffer, data_ptr, data_length);
    }
    else
    {
        WPRINT_APP_INFO( ("%s: READ, datat_address : 0x%lx, data length: %ld\n", __FUNCTION__, data_address, data_length) );
