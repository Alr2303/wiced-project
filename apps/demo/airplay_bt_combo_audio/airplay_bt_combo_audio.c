...
/** @file airplay_bt_combo_audio.c
 *
 * Airplay BT Combo Audio Application
 *
 * This application demonstrates how we can stream airplay, BT Audio (A2DP) and also use the
 * device as a handsfree device for receiving phone calls on the device.
 *
 * Features Supported:
 *   - Playback of airplay audio
 *   - Playback of Bluetooth Audio
 *   - Remote control using AVRCP
 *   - PLAY/PAUSE/FWD/BACKWARD etc. from the DUT.
 *   - Receiving phone calls through the HFP profile
 *     - Inband Ring tone supported
 *     - Out of Range supported
 *     - Accept/End a call (MFB Short press)
 *     - Reject a call (MFB Long press)
 *     - Last number redialling by means of a button press (MFB button)
 *     - 3-way call handling
 *              - Accept waiting call and end the active call (MFB Short press)
 *              - Accept waiting call and put the active call on hold (MFB long press)
 *              - Swap between active and held calls (MFB Long press)
 *   - Displays the song information on devices that support display (e.g. 43907WAE_1)
 *
 * Application Instructions
 *
 *   1. This application needs WiFi to connect to the access point, bluetooth for A2DP/AVRCP/HFP
 *      and hardware that supports I2S and a speaker.
 *
 *   2. The network to be used can be changed by the #define WICED_NETWORK_INTERFACE in wifi_config_dct.h
 *       In the case of using AP or STA mode, change the AP_SSID and AP_PASSPHRASE accordingly.
 *
 *   3. The Bluetooth BD ADDRESS can be changed by the #define WICED_BLUETOOTH_DEVICE_ADDRESS in bt_config_dct.h.
 *      The device name can be changed by the #define WICED_BLUETOOTH_DEVICE_NAME in bt_config_dct.h.
 *
 *   4.  To enable display, ensure that GLOBAL_DEFINES     += USE_AUDIO_DISPLAY is uncommented in
 *   airplay_bt_combo_audio.mk
 *
 */
...
static void app_worker_thread( uint32_t args )
{
    ...
    WPRINT_APP_INFO( ("[App] Airplay-BT Combo Audio thread started...\n") );
...
void wiced_app_reset_for_wac(void)
{
    wiced_airplay_cleanup();
    wiced_rtos_delay_milliseconds(WICED_APP_REBOOT_DELAY_MSECS);
    wiced_airplay_do_wac_on_reboot();
    wiced_rtos_delay_milliseconds(WICED_APP_REBOOT_DELAY_MSECS);
...
{
    wiced_init();
    WPRINT_APP_INFO( ("[App] Starting Airplay-BT Combo Audio App\n") );
...
#ifdef USE_AUDIO_DISPLAY
    audio_display_create_management_thread(&display_thread);
    audio_display_header_update_options(BATTERY_ICON_IS_VISIBLE | BATTERY_ICON_SHOW_PERCENT);
#endif /*USE_AUDIO_DISPLAY */

    /* Start Airplay Audio service */
    //TODO - run this concurrently
    wiced_add_airplay_audio_service();
