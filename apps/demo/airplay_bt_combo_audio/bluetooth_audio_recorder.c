...
******************************************************/
/*
** the below functions in comment would be needed if we have to transfer recorder data to SCO asynchronously.
Currently, the data is sent to stack as soon as there is enough data to send from recorder loop itself.. need to test and see if this scales.
Otherwise we may have to offload this work to a new task!

uint16_t bt_audio_get_recorder_buffer_count( void )
{
    uint32_t count;
    if ( WICED_SUCCESS == wiced_rtos_get_queue_occupancy(&recorder.queue, &count) )
        return count;
    else
        return 0;
}

void bt_audio_send_recorder_data_ready_event( void )
{

}


bt_audio_codec_data_t* bt_audio_get_recorder_data_buffer( void )
{
    bt_audio_codec_data_t* pcm;
    wiced_result_t result;

    result = wiced_rtos_pop_from_queue(&recorder.queue, &pcm, 0);
...
/*void bt_audio_tx_recorder_data_to_sco( void )
{
    bt_audio_codec_data_t* pcm;
    wiced_result_t result;
    ...
    while(1)
    {
        pcm = bt_audio_get_recorder_data_buffer();
        if(!pcm)
        {
            break;
        }
