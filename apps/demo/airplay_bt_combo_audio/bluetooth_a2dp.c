...
void bt_audio_a2dp_disconnect_service(void)
{
    ...
    result = wiced_bt_a2dp_sink_disconnect(a2dp_context.peer_address);
    if( WICED_SUCCESS != result )
    {
...
void bt_audio_avrcp_disconnect_service(void)
{
    ...
    result = wiced_bt_remote_control_disconnect(a2dp_context.peer_address);
    if( WICED_SUCCESS != result )
    {
...
static void bt_audio_app_sink_avrc_connect(void)
{
    if(a2dp_context.rc_connected == WICED_FALSE && a2dp_context.sink_connected == WICED_TRUE )
    {
        wiced_bt_remote_control_connect(a2dp_context.peer_address);
    }
    return;
}
...
static void bt_audio_sink_control_cb( wiced_bt_a2dp_sink_event_t event, wiced_bt_a2dp_sink_event_data_t* p_data)
{
    ...
    wiced_app_service_t*  service_check = NULL;


    switch(event)
    {
        case WICED_BT_A2DP_SINK_CONNECT_EVT:
            WPRINT_APP_INFO( ("%s:CONNECT EVENT \nstatus = %d\n", __func__,p_data->connect.result) );

            if(p_data->connect.result == WICED_SUCCESS)
            {
                memcpy( a2dp_context.peer_address, p_data->connect.bd_addr, sizeof(wiced_bt_device_address_t) );
                WPRINT_APP_INFO( ("%s: Remote Bluetooth Address: [%02X:%02X:%02X:%02X:%02X:%02X]\n",__func__,
                                                a2dp_context.peer_address[0], a2dp_context.peer_address[1], a2dp_context.peer_address[2],
                                                a2dp_context.peer_address[3], a2dp_context.peer_address[4], a2dp_context.peer_address[5]) );
                service = wiced_get_entry(SERVICE_BT_A2DP);
                service->state = SERVICE_ENABLED;
                a2dp_context.sink_connected = WICED_TRUE;

                WPRINT_APP_INFO(("[%s %d] rc_connected = %d\n",__func__,__LINE__,a2dp_context.rc_connected));
                if(a2dp_context.rc_connected == WICED_FALSE)
                {
            ...
            {
                a2dp_context.sink_connected = WICED_FALSE;
                signal_for_event_semaphore( WICED_BT_CONNECT_A2DP, WICED_FALSE );
            }
        ...
        audio_display_header_update_options(BATTERY_ICON_IS_VISIBLE | BATTERY_ICON_SHOW_PERCENT
                | SIGNAL_STRENGTH_IS_VISIBLE | INVERT_BLUETOOTH_ICON_COLORS);
        ...
        {
            WPRINT_APP_INFO( ("%s:STREAM START EVENT \nstatus = %d \n",__func__, p_data->start.result) );
            ...
            service_check = get_app_current_service();
            ...
            /* When Peer disconnects Bluetooth, bluetooth-a2dp service looses it's state. Adding an extra check so that, when a remote device re-connects on BT
             * bluetooth-a2dp service make sure that airplay or other higher priority service is not currently playing. */

            if( (service_check != NULL) && (service_check->state == SERVICE_PLAYING_AUDIO) && (service_check->priority >= SERVICE_BT_A2DP_PRIORITY) )
            {
                app_set_service_state(SERVICE_BT_A2DP, SERVICE_PENDING);
                return;
            }

            bt_audio_configure_decoder(&a2dp_context.decoder_config);
            bt_audio_configure_device( &a2dp_context.audio_config, BT_AUDIO_DEVICE_PLAYER, SERVICE_BT_A2DP );
...
#ifdef USE_WICED_HCI
            wiced_bt_a2dp_sink_start_rsp( a2dp_context.peer_address, A2D_SUCCESS);
#endif
        }
        break;

        case WICED_BT_A2DP_SINK_SUSPEND_EVT:
            WPRINT_APP_INFO( ("%s:STREAM SUSPEND EVENT \nstatus = %d \n",__func__, p_data->start.result) );
            bt_audio_end_loop(BT_AUDIO_DEVICE_PLAYER);
        ...
        case WICED_BT_A2DP_SINK_CODEC_CONFIG_EVT:
        {
            ...
            memcpy(&a2dp_context.decoder_config, &p_data->codec_config, sizeof(wiced_bt_a2dp_codec_info_t));
            bt_audio_get_config_from_cie(&a2dp_context.decoder_config, &a2dp_context.audio_config);
        }
        break;
    }
}
...
static void bt_audio_remote_control_rsp_cback(wiced_bt_device_address_t remote_addr, wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    WPRINT_APP_INFO( ( "[REMOTE-CONTROL]: %s pdu [%d] status [%d]\n", __func__, avrc_rsp->pdu, avrc_rsp->rsp.status ) );
            switch( avrc_rsp->pdu )
            {
            ...
            if( ((display_play_status_ptr->song_pos) == -1) || ((display_play_status_ptr->song_len) == -1) )
            {
                audio_display_footer_update_time_info( 0, 0 );
                audio_display_footer_update_options(FOOTER_IS_VISIBLE | FOOTER_CENTER_ALIGN);
            }
            else
            {
...
{
    wiced_result_t result;
    WPRINT_APP_INFO(("[%s %d] %x:%x:%x:%x:%x:%x\n",__func__,__LINE__,
            a2dp_context.peer_address[0],
            a2dp_context.peer_address[1],
            a2dp_context.peer_address[2],
            a2dp_context.peer_address[3],
            a2dp_context.peer_address[4],
            a2dp_context.peer_address[5]));
...
static wiced_result_t bt_audio_a2dp_skip_backward( void )
{
    ...
    return bt_audio_a2dp_send_passthru_command( AVRC_ID_BACKWARD );
}
...
static wiced_result_t bt_audio_a2dp_stop( void )
{
    return bt_audio_a2dp_send_passthru_command(AVRC_ID_STOP);
}
...
static wiced_result_t bt_audio_a2dp_skip_forward( void )
{
    ...
    return bt_audio_a2dp_send_passthru_command(AVRC_ID_FORWARD);
}
...
static void bt_audio_a2dp_allow_service(wiced_app_service_t *service)
{
    ...
    if( service->state == SERVICE_PENDING )
    {
        bt_audio_configure_device( &a2dp_context.audio_config, BT_AUDIO_DEVICE_PLAYER, SERVICE_BT_A2DP );
        app_set_current_service(SERVICE_BT_A2DP);
        app_set_service_state(SERVICE_BT_A2DP, SERVICE_PLAYING_AUDIO);
...
static void bt_audio_a2dp_prevent_service( wiced_app_service_t *service )
{
    ...
    if( service->state == SERVICE_PLAYING_AUDIO )
    {
        bt_audio_a2dp_send_passthru_command(AVRC_ID_PAUSE);
        wiced_bt_a2dp_sink_suspend( a2dp_context.peer_address );
