...
bluetooth_hfp_context_t hfp_ctxt_data;
...
void bt_audio_add_hfp_service(void)
{
    ...
    hfp_ctxt_data.call_active       = 0;
    hfp_ctxt_data.call_setup        = WICED_BT_HFP_HF_CALLSETUP_STATE_IDLE;
    hfp_ctxt_data.mic_volume        = 8;
    hfp_ctxt_data.spkr_volume       = 8;
    hfp_ctxt_data.sco_index         = BT_AUDIO_INVALID_SCO_INDEX;
    hfp_ctxt_data.connection_status = WICED_BT_HFP_HF_STATE_DISCONNECTED;
...
{
    if(hfp_ctxt_data.call_active == 0)
    {
...
static void bt_audio_hfp_callsetup_evt_handler(wiced_bt_hfp_hf_call_data_t* call_data)
{
    switch (call_data->setup_state)
    {
        ...
        case WICED_BT_HFP_HF_CALLSETUP_STATE_IDLE:
            ...
            {
                if(hfp_ctxt_data.call_setup == WICED_BT_HFP_HF_CALLSETUP_STATE_INCOMING || hfp_ctxt_data.call_setup == WICED_BT_HFP_HF_CALLSETUP_STATE_DIALING
                || hfp_ctxt_data.call_setup == WICED_BT_HFP_HF_CALLSETUP_STATE_ALERTING)
                {
                ...
                if(hfp_ctxt_data.call_setup == WICED_BT_HFP_HF_CALLSETUP_STATE_INCOMING)
                {
                    if(hfp_ctxt_data.inband_ring_status == WICED_BT_HFP_HF_INBAND_RING_ENABLED)
                        wiced_tone_stop(); //stop tone if being played out, if not, does nothing.
                }

                if( hfp_ctxt_data.call_active == 1 )
                {
    ...
    hfp_ctxt_data.call_active = call_data->active_call_present;
    hfp_ctxt_data.call_setup = call_data->setup_state;
    hfp_ctxt_data.call_held = call_data->held_call_present;
}
...
static void bt_audio_hfp_event_cb( wiced_bt_hfp_hf_event_t event, wiced_bt_hfp_hf_event_data_t* p_data)
{
    ...
    switch(event)
    {
        case WICED_BT_HFP_HF_CONNECTION_STATE_EVT:
            ...
            hfp_ctxt_data.connection_status = p_data->conn_state;
            if(p_data->conn_state == WICED_BT_HFP_HF_STATE_CONNECTED)
            {
                status = wiced_bt_sco_create_as_acceptor(&hfp_ctxt_data.sco_index);
                WPRINT_APP_INFO( ("%s: status [%d] SCO INDEX [%d] \n", __func__, status, hfp_ctxt_data.sco_index));
            }
            else if(p_data->conn_state == WICED_BT_HFP_HF_STATE_SLC_CONNECTED)
            {
                WPRINT_APP_INFO( ("%s: Peer BD Addr [0x%2x0x%2x0x%2x0x%2x0x%2x0x%2x]\n", __func__, p_data->remote_address[0], p_data->remote_address[1],
                                              p_data->remote_address[2], p_data->remote_address[3], p_data->remote_address[4], p_data->remote_address[5]) );
                memcpy( hfp_ctxt_data.peer_bd_addr, p_data->remote_address, sizeof(wiced_bt_device_address_t));
            ...
            {
                memset(hfp_ctxt_data.peer_bd_addr, 0, sizeof(wiced_bt_device_address_t));
                if(hfp_ctxt_data.sco_index != BT_AUDIO_INVALID_SCO_INDEX)
                {
                    status = wiced_bt_sco_remove(hfp_ctxt_data.sco_index);
                    hfp_ctxt_data.sco_index = BT_AUDIO_INVALID_SCO_INDEX;
        ...
        case WICED_BT_HFP_HF_AG_FEATURE_SUPPORT_EVT:
            ...
            {
                hfp_ctxt_data.inband_ring_status = WICED_BT_HFP_HF_INBAND_RING_ENABLED;
            }
            else
            {
                hfp_ctxt_data.inband_ring_status = WICED_BT_HFP_HF_INBAND_RING_DISABLED;
            }
        break;
        ...
        case WICED_BT_HFP_HF_RING_EVT:
        {
            ...
            WPRINT_APP_INFO( ( "%s: RING \n", __func__) );
            if((hfp_ctxt_data.inband_ring_status == WICED_BT_HFP_HF_INBAND_RING_DISABLED) &&
        ...
        case WICED_BT_HFP_HF_INBAND_RING_STATE_EVT:
            hfp_ctxt_data.inband_ring_status = p_data->inband_ring;
        break;
        ...
        case WICED_BT_HFP_HF_VOLUME_CHANGE_EVT:
        {
            ...
            if(result == WICED_SUCCESS)
            {
                hfp_ctxt_data.spkr_volume = p_data->volume.level;
            }
        }
        break;
...
static void bt_audio_sco_data_cback( uint16_t sco_index, BT_HDR *sco_data, uint8_t status )
{
    ...
    //WPRINT_APP_INFO( ("[%s] sco data length %d, offset %d\n",__func__, sco_data->len, sco_data->offset) );
...
/*    if(bt_audio_get_recorder_buffer_count())
    {
        bt_audio_tx_recorder_data_to_sco();
    }*/
}
...
void bt_audio_hfp_send_sco_data(bt_audio_codec_data_t* pcm)
{
    ...
    wiced_bt_sco_write_data(hfp_ctxt_data.sco_index, data, pcm->length);
}
...
static void bt_audio_hfp_init_service(void)
{
    ...
    p_config_data.mic_volume        = hfp_ctxt_data.mic_volume;
    p_config_data.speaker_volume    = hfp_ctxt_data.spkr_volume;
...
{
    wiced_result_t result;
    result = wiced_bt_hfp_hf_disconnect(hfp_ctxt_data.peer_bd_addr);
...
static wiced_result_t bt_audio_hfp_volume_up(void)
{
    ...
    if( hfp_ctxt_data.spkr_volume == BT_AUDIO_HFP_VOLUME_MAX )
    {
        WPRINT_APP_INFO( ("%s: Volume level already Maximum\n", __func__) );
        return WICED_SUCCESS;
    }

    hfp_ctxt_data.spkr_volume++;

    result = bt_audio_hfp_update_volume( hfp_ctxt_data.spkr_volume, WICED_BT_HFP_HF_SPEAKER );
    if (result == WICED_SUCCESS)
    {
        wiced_bt_hfp_hf_notify_volume(hfp_ctxt_data.peer_bd_addr, WICED_BT_HFP_HF_SPEAKER , hfp_ctxt_data.spkr_volume);
    }
...
static wiced_result_t bt_audio_hfp_volume_down(void)
{
    ...
    if( hfp_ctxt_data.spkr_volume == BT_AUDIO_HFP_VOLUME_MIN )
    {
...
wiced_result_t bt_audio_hfp_dial_last_number( void )
{
    ...
    return wiced_bt_hfp_hf_perform_call_action(hfp_ctxt_data.peer_bd_addr, WICED_BT_HFP_HF_CALL_ACTION_DIAL, NULL);
}
...
static wiced_result_t bt_audio_hfp_multi_func_long_release(void)
{
    ...
    switch(hfp_ctxt_data.call_setup)
    {
        ...
        else if(hfp_ctxt_data.call_active == 1 || hfp_ctxt_data.call_held == 1)
        {
        ...
        default: //WICED_BT_HFP_HF_CALLSETUP_STATE_IDLE
            if(hfp_ctxt_data.call_active == 1 || hfp_ctxt_data.call_held == 1)
            {
    ...
    if(action != -1)
        ret = wiced_bt_hfp_hf_perform_call_action(hfp_ctxt_data.peer_bd_addr, (wiced_bt_hfp_hf_call_action_t)action, NULL);
    ...
    switch(hfp_ctxt_data.call_setup)
    {
        ...
        default: //WICED_BT_HFP_HF_CALLSETUP_STATE_IDLE
            if(hfp_ctxt_data.call_active == 1)
            {
                ...
                action = (hfp_ctxt_data.call_held == 0)?WICED_BT_HFP_HF_CALL_ACTION_HANGUP:WICED_BT_HFP_HF_CALL_ACTION_HOLD_1;
            }
        break;
    }
...
void bt_audio_hfp_sco_disconnection_evt_handler(wiced_bt_sco_disconnected_t* disconnected)
{
    ...
    if(hfp_ctxt_data.connection_status != WICED_BT_HFP_HF_STATE_DISCONNECTED)
    {
        status = wiced_bt_sco_create_as_acceptor(&hfp_ctxt_data.sco_index);
        WPRINT_APP_INFO( ("%s: status [%d] SCO INDEX [%d] \n", __func__, status, hfp_ctxt_data.sco_index));
    }
...
{
    return hfp_ctxt_data.connection_status;
}
