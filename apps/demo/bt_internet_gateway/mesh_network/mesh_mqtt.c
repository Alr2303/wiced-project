...
wiced_result_t mqtt_app_publish( wiced_mqtt_object_t mqtt_obj, uint8_t qos, uint8_t *topic, uint8_t *data, uint32_t data_len );
static wiced_result_t parse_json_shadow_status(wiced_json_object_t * json_object );

wiced_result_t mesh_init_mqtt_transport( void );
...
//            WPRINT_APP_INFO( ( "[MQTT] Received %.*s  for TOPIC : %.*s\n\n", (int) msg.data_len, msg.data, (int) msg.topic_len, msg.topic ) );
            ...
            if ( state == 0 )
            {
                mqtt_app_publish( mqtt_object, WICED_MQTT_QOS_DELIVER_AT_MOST_ONCE, (uint8_t*) WICED_TOPIC_PUBLISH, (uint8_t*) msg.data, (int) msg.data_len );
