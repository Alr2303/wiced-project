 ...
 *
 * Note : BLUEMIX and AWS setup will be added soon
 *  BLUEMIX SETUP
 ...
 *  3. Make sure AWS Root Certificate 'resources/apps/aws_iot/rootca.cer' is up to date while building the app.
...
extern wiced_result_t mesh_init_mqtt_transport( void );
...
//    int i;
    ...
    for(i = 0 ; i < packet_len; i++) {
         WPRINT_APP_INFO(("mesh_write_node_info data :%02X\n", packet[i]));
     }
#endif
...
static wiced_result_t mesh_management_cback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        WPRINT_BT_APP_INFO(("mesh_management_cback: BTM_ENABLED_EVT \n"));
        mesh_read_dct();
        if(mesh_dct_info.node_authenticated == MESH_NODE_PROVISIONED)
        {
            WPRINT_BT_APP_INFO(("Device is already provisioned push NVM chunks " ));
            for( i = 0; i < MESH_NV_DATA_MAX_ENTRIES ; i++){
                ...
                wiced_bt_mesh_push_nvram_data(mesh_dct_info.mesh_nv_data[i].data,mesh_dct_info.mesh_nv_data[i].len,i);
...
static wiced_result_t mesh_read_dct()
{
 ...
 static wiced_result_t mesh_write_dct( mesh_dct_t* dct_to_write, int id, uint8_t *packet, uint32_t packet_len )
 {
     WPRINT_APP_DEBUG(("mesh_write_dct , recieved packet_len = %lu:\n",packet_len));
     memcpy((uint8_t*)mesh_dct_info.mesh_nv_data[id].data, packet ,packet_len);
     mesh_dct_info.mesh_nv_data[id].len = (uint8_t)packet_len;
     return WICED_SUCCESS;
 }
