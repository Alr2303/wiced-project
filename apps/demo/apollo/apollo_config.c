...
#define APP_DCT_BURST_LENGTH_DIRTY      (1 << 8)
#define APP_DCT_SHUFFLE_LENGTH_DIRTY    (1 << 9)
...
#define APP_DCT_PLL_PPM_MAX_DIRTY       (1 << 20)
#define APP_DCT_PLL_PPM_MIN_DIRTY       (1 << 21)

#define APP_DCT_RMC_SSID_DIRTY          (1 << 22)
#define APP_DCT_RMC_SECURITY_KEY_DIRTY  (1 << 23)
#define APP_DCT_RMC_BSS_TYPE_DIRTY      (1 << 24)
#define APP_DCT_RMC_CHANNEL_DIRTY       (1 << 25)
#define APP_DCT_RMC_SECURITY_TYPE_DIRTY (1 << 26)
#define APP_DCT_RMC_EXTERNAL_AP_DIRTY   (1 << 27)

#if defined(OTA2_SUPPORT)
#define APP_DCT_OTA2_STOP_PLAYBACK_DIRTY (1 << 28)
#define APP_DCT_OTA2_REBOOT_DIRTY        (1 << 29)
#define APP_DCT_OTA2_URI_DIRTY           (1 << 30)
#endif
    ...
    switch(cmd)
    {
        ...
        case CONFIG_CMD_THRESHOLD_MS:
            if (argc > 2)
            {
                ...
                {
                    wiced_log_msg(WLF_AUDIO, WICED_LOG_DEBUG0, "CONFIG_CMD_THRESHOLD_MS: %d -> %d\r\n", dct_tables->dct_app->threshold_ms, ms);
                    dct_tables->dct_app->threshold_ms = ms;
                    app_dct_dirty |= APP_DCT_THRESH_MS_DIRTY;
                }
            }
            wiced_log_msg(WLF_AUDIO, WICED_LOG_NOTICE, "Threshold: %d ms\r\n", dct_tables->dct_app->threshold_ms);
            break;
