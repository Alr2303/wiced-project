...
#endif
            wiced_log_msg(WLF_AUDIO, WICED_LOG_DEBUG1, "%s track: %lu / %lu secs\n",
...
static void apollo_mainloop(apollo_app_t* apollo)
{
...
#if defined(OTA2_SUPPORT)
    if (apollo->perform_ota2_update_at_start == WICED_TRUE)
    {
        ...
        apollo->perform_ota2_update_at_start = WICED_FALSE;
    ...
    while (apollo->tag == APOLLO_TAG_VALID)
    {
        ...
        if (events & APOLLO_EVENT_GET_UPDATE)
        {
            ...
            /* If we are already running the background service,
             * do not kill it when we are done downloading.
             */
            apollo->ota2_stop_bg_service = WICED_FALSE;
            if (apollo->ota2_info.bg_service == NULL)
            {
                /*
                 * The timed service was not running.
                 * Instruct the callback to trigger the STOP_UPDATE
                 * event to stop the ota2 timed service when done.
                 *
                 */
                apollo->ota2_stop_bg_service = WICED_TRUE;
            }

            /* stop playback and Apollo network services */
            apollo_service_stop(apollo);
            result = apollo_stop_network_services(apollo);
            if (result != WICED_SUCCESS)
            {
                wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "GET_UPDATE apollo_stop_network_services() failed! %d \r\n", result);
            }
            else
            {
        ...
        if (events & APOLLO_EVENT_START_TIMED_UPDATE)
        {
            ...
            apollo->ota2_info.bg_params.auto_update               = 1;                      /* auto update on reboot (callback overrides)  */
            apollo->ota2_info.bg_params.initial_check_interval    = SECONDS_PER_MINUTE * 10; /* initial update time from now                */
        ...
        if (events & APOLLO_EVENT_OTA2_UPDATE_PERCENT)
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_DEBUG0, "download progress: %d \r\n", apollo->ota2_download_percentage);
        }
        ...
        case APOLLO_CMD_EVENT_SET_LOG_SERVER:
            ...
            if (ip_addr.ip.v4 != 0 && !apollo->network_logging_active)
            {
                if (apollo_debug_create_tcp_data_socket(&ip_addr, 0) == WICED_SUCCESS)
                {
                    ...
                    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Apollo IP address: %d.%d.%d.%d   MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
static apollo_app_t* apollo_init(void)
{
    ...
    {
        apollo->perform_ota2_update_at_start = WICED_TRUE;
...

    {
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Chip version is %s\n",
...
wiced_result_t apollo_ota2_callback(void* session_id, wiced_ota2_service_status_t status, uint32_t value, void* opaque )
{
    ...
    case OTA2_SERVICE_STARTED:      /* Background service has started
                                     * return - None
                                     */
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : SERVER STARTED -----------------------------\r\n");
        break;
    case OTA2_SERVICE_AP_CONNECT_ERROR:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : AP CONNECT_ERROR -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");
        /* restart Apollo when OTA2_SERVICE_UPDATE_ENDED occurs */        break;

    case OTA2_SERVICE_SERVER_CONNECT_ERROR:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : SERVER_CONNECT_ERROR -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");
        /* restart Apollo when OTA2_SERVICE_UPDATE_ENDED occurs */        break;

    case OTA2_SERVICE_AP_CONNECTED:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : AP_CONNECTED -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");
        break;

    case OTA2_SERVICE_SERVER_CONNECTED:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : SERVER_CONNECTED -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");
        break;
    ...
    case OTA2_SERVICE_CHECK_FOR_UPDATE: /* Time to check for updates.\r 1641
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : CHECK_FOR_UPDATE -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS, let Service do the checking.\r\n");

        /* Stopping playback  */
        if ((apollo_is_service_active(apollo) == WICED_TRUE) && (apollo->dct_tables.dct_app->ota2_stop_playback_to_update == 0))
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "apollo->dct_tables.dct_app->ota2_stop_playback_to_update == 0 (DO NOT stop playback for updates).\r\n");
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Bring down RMC network. (stops playback).\r\n");
                                         ...
                                         *                           the download, the OTA Service will do nothing.  */
    {
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : UPDATE_AVAILABLE -----------------------------\r\n");
        ...
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "OTA2 Update Version Fail - return ERROR, do not update!\r\n");
            return WICED_ERROR;
        }
#endif

        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS, let Service perform the download.\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_NOTICE, "starting download:");

        apollo->ota2_download_percentage            = 0;
        break;
    }
    ...
    case OTA2_SERVICE_DOWNLOAD_STATUS:  /* Download status - value has % complete (0-100)
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_NOTICE, "OTA2_SERVICE_DOWNLOAD_STATUS %ld %%!\r\n", value);
        apollo->ota2_download_percentage = value;
        wiced_rtos_set_event_flags(&apollo->events, APOLLO_EVENT_OTA2_UPDATE_PERCENT);
        break;
    ...
    case OTA2_SERVICE_PERFORM_UPDATE:   /* Download is complete
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : PERFORM_UPDATE -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS, let Service extract update on next reboot.\r\n");
        break;
    ...
    case OTA2_SERVICE_UPDATE_ERROR:     /* There was an error in transmission
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : UPDATE_ERROR -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS, let Service use retry check interval.\r\n");
    ...
    case OTA2_SERVICE_UPDATE_ENDED:     /* All update actions for this check are complete.
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : UPDATE_ENDED value: %ld-----------------------------\r\n", value);
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");

        if ((value == WICED_SUCCESS) && (apollo->dct_tables.dct_app->ota2_reboot_after_download != 0 ))
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, " Download is complete - reboot! \r\n");
            wiced_framework_reboot();
        }
        ...
        /* If the console command "get_update" was used,
         * and the OTA2 background service was not running at the time,
         * stop the OTA2 background service.
         */
        if (apollo->ota2_stop_bg_service == WICED_TRUE)
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "APOLLO_EVENT_STOP_TIMED_UPDATE\r\n");
            wiced_rtos_set_event_flags(&apollo->events, APOLLO_EVENT_STOP_TIMED_UPDATE);
        }

        break;

    case OTA2_SERVICE_STOPPED:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "----------------------------- OTA2 Service Called : SERVICEENDED -----------------------------\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "        return SUCCESS (not used by service). This is informational \r\n");
        /* restart Apollo when OTA2_SERVICE_UPDATE_ENDED occurs */
        break;

    default:
        wiced_log_msg(WLF_AUDIO, WICED_LOG_INFO, "apollo_ota2_callback() UNKNOWN STATUS %d!\r\n", status);
        break;
    }
