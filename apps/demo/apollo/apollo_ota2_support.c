...
wiced_result_t apollo_ota2_get_update(apollo_ota2_service_info_t* ota2_info)
{
    ...
    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "ota2_update_uri:%s\r\n", ota2_info->update_uri);
    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "ota2_host_name :%s \r\n", ota2_info->bg_params.host_name);
    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "ota2_file_path :%s \r\n", ota2_info->bg_params.file_path);
    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "ota2_port      :%d \r\n", ota2_info->bg_params.port);
    ...
    if (ota2_info->bg_service == NULL)
    {
        ...
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "apollo_get_update() wiced_ota2_service_init() bg_service:%p \r\n", ota2_info->bg_service);
    }
    else
    {
    ...
    if (ota2_info->bg_service != NULL)
    {
        ...
        {
                wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "ota2_test_get_update register callback failed! %d \r\n", result);
                wiced_ota2_service_deinit(ota2_info->bg_service);
                ota2_info->bg_service = NULL;
        }
        else
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Download the OTA Image file - get it NOW!\r\n");
            /* NOTE: This is a blocking call! */
            result = wiced_ota2_service_check_for_updates(ota2_info->bg_service);
            if (result != WICED_SUCCESS)
            {
                    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "apollo_get_update wiced_ota2_service_check_for_updates() failed! %d \r\n", result);
            }
        }
    }
...
wiced_result_t apollo_ota2_start_bg_update(apollo_ota2_service_info_t* ota2_info)
{
    ...
    wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "apollo_start_bg_update() ota2_info->bg_service %p \r\n", ota2_info->bg_service);
    if (ota2_info->bg_service == NULL)
    {
        /* start a new instance of the ota2 bg service */
        ota2_info->bg_service = wiced_ota2_service_init(&ota2_info->bg_params, ota2_info->cb_opaque);
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "apollo_start_bg_update() wiced_ota2_service_init() bg_service:%p \r\n", ota2_info->bg_service);
    }
    ...
    if (ota2_info->bg_service != NULL)
    {
        ...
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "Download the OTA Image file - start background task!\r\n");
...
wiced_result_t apollo_ota2_status(apollo_ota2_service_info_t* ota2_info)
{
    ...
    {
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "OTA2 Service info:\r\n");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "                update_uri: %s\r\n", (strlen(ota2_info->update_uri) > 0) ? ota2_info->update_uri : "NO URI SET");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "                   cb_func: %s\r\n", (ota2_info->cb_func!= NULL) ? "Callback Set" : "NO CALLBACK SET");
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "                 cb_opaque: %p\r\n", ota2_info->cb_opaque);
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "              ota2_ap_info: %p\r\n", ota2_info->bg_params.ota2_ap_info);
        if (ota2_info->bg_params.ota2_ap_info != NULL)
        {
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "              SSID: %.*s\r\n",
                    ota2_info->bg_params.ota2_ap_info->details.SSID.length, ota2_info->bg_params.ota2_ap_info->details.SSID.value);
        }
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "        ota2_ap_list_count: %ld\r\n", ota2_info->bg_params.ota2_ap_list_count);
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "              ota2_ap_list: %p\r\n", ota2_info->bg_params.ota2_ap_list);
    ...
    {
        /* Staging Area status */
        wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "OTA2 Staged Area Status:\n");
        switch(status)
        {
        case WICED_OTA2_IMAGE_INVALID:
        default:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Invalid\n");
            break;
        case WICED_OTA2_IMAGE_DOWNLOAD_IN_PROGRESS:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    In Progress\n");
            break;
        case WICED_OTA2_IMAGE_DOWNLOAD_FAILED:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Failed\n");
            break;
        case WICED_OTA2_IMAGE_DOWNLOAD_UNSUPPORTED:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Unsupported\n");
            break;
        case WICED_OTA2_IMAGE_DOWNLOAD_COMPLETE:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Complete\n");
            break;
        case WICED_OTA2_IMAGE_VALID:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Valid\n");
            break;
        case WICED_OTA2_IMAGE_EXTRACT_ON_NEXT_BOOT:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Extract on Boot\n");
            break;
        case WICED_OTA2_IMAGE_DOWNLOAD_EXTRACTED:
            wiced_log_msg(WLF_AUDIO, WICED_LOG_ERR, "    Extracted\n");
            break;
        }
    }
