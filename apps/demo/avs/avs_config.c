...
static wiced_result_t avs_save_app_dct(avs_dct_collection_t* dct_tables);
static void avs_print_app_info(avs_dct_collection_t* dct_tables);
static void avs_print_network_info(avs_dct_collection_t* dct_tables);
static void avs_print_wifi_info(avs_dct_collection_t* dct_tables);
...
static void avs_config_command_help(void)
{
...
static security_lookup_t avs_security_name_table[] =
{
...
static wiced_result_t avs_save_app_dct(avs_dct_collection_t* dct_tables)
{
    return wiced_dct_write((void*)dct_tables->dct_app, DCT_APP_SECTION, 0, sizeof(avs_dct_t));
}

static char* avs_get_security_type_name(wiced_security_t type)
{
    int table_index;

    for (table_index = 0; table_index < sizeof(avs_security_name_table)/sizeof(security_lookup_t); table_index++)
    {
        if (avs_security_name_table[table_index].sec_type == type)
        {
            return avs_security_name_table[table_index].name;
        }
    }
...
static wiced_bool_t avs_get_mac_addr_text(wiced_mac_t *mac, char* out, int out_len)
{
    ...
    sprintf(out, "%02x:%02x:%02x:%02x:%02x:%02x", mac->octet[0], mac->octet[1], mac->octet[2], mac->octet[3], mac->octet[4], mac->octet[5]);

    return WICED_TRUE;
}

void avs_set_config(avs_dct_collection_t* dct_tables, int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        avs_config_print_info(dct_tables);
        return;
    }
    ...
    switch (cmd)
    {
        ...
        case CONFIG_CMD_VOLUME:
            if (argc > 2)
            {
                arg = atoi(argv[2]);
                /* validity check */
                if (arg < AVS_VOLUME_MIN || arg > AVS_VOLUME_MAX)
                {
                    wiced_log_msg(WLF_DEF, WICED_LOG_ERR, "Error: out of range min: %d  max: %d\n",
                                  AVS_VOLUME_MIN, AVS_VOLUME_MAX);
                    break;
                }
        ...
        case CONFIG_CMD_SAVE:
            if (app_dct_dirty != 0)
            {
                ...
                avs_save_app_dct(dct_tables);
                app_dct_dirty = 0;
                avs_print_app_info(dct_tables);
            }
            break;
...
static void avs_print_wifi_info(avs_dct_collection_t* dct_tables)
{
    ...
    avs_get_mac_addr_text(&wiced_mac, wiced_mac_str, sizeof(wiced_mac_str));
    avs_get_mac_addr_text(&dct_tables->dct_wifi->mac_address, mac_str, sizeof(mac_str));
    ...
    if (dct_tables->dct_network->interface == WICED_STA_INTERFACE)
    {
        ...
        wiced_log_printf("            Security: %s\n", avs_get_security_type_name(sec));
...
static void avs_print_current_info(avs_dct_collection_t* dct_tables)
{
    uint32_t channel;
...
void avs_config_print_info(avs_dct_collection_t* dct_tables)
{
    wiced_log_printf("\nConfig Info: * = dirty\n");
    avs_print_app_info(dct_tables);
    avs_print_network_info(dct_tables);
    avs_print_wifi_info(dct_tables);
    avs_print_current_info(dct_tables);
    wiced_log_printf("\n");
}

static wiced_result_t avs_config_load_dct_wifi(avs_dct_collection_t* dct_tables)
{
...
static wiced_result_t avs_config_load_dct_network(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result;
...
wiced_result_t avs_config_init(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result;

    /* wifi */
    result = avs_config_load_dct_wifi(dct_tables);

    /* network */
    result |= avs_config_load_dct_network(dct_tables);

    /* App */
    result |= wiced_dct_read_lock( (void**)&dct_tables->dct_app, WICED_TRUE, DCT_APP_SECTION, 0, sizeof(avs_dct_t));

    return result;
}

static wiced_result_t avs_config_unload_dct_wifi(avs_dct_collection_t* dct_tables)
{
...
static wiced_result_t avs_config_unload_dct_network(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result = WICED_SUCCESS;
...
wiced_result_t avs_config_deinit(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result;

    result = wiced_dct_read_unlock(dct_tables->dct_app, WICED_TRUE);

    result |= avs_config_unload_dct_network(dct_tables);
    result |= avs_config_unload_dct_wifi(dct_tables);

    return result;
}

wiced_result_t avs_config_reload_dct_wifi(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result;
...
wiced_result_t avs_config_reload_dct_network(avs_dct_collection_t* dct_tables)
{
    wiced_result_t result;
