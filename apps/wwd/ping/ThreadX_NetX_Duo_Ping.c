...
static void app_main_thread( ULONG thread_input )
{
    ...
    if ( NX_SUCCESS != nx_packet_pool_create( &nx_pools[ 0 ], (char* )"", WICED_LINK_MTU_ALIGNED, PLATFORM_L1_CACHE_PTR_ROUND_UP(tx_buffer_pool_memory), APP_TX_BUFFER_POOL_SIZE ) )
    {
        WPRINT_APP_ERROR( ("Couldn't create TX packet pool\n") );
        return;
    }
    if ( NX_SUCCESS != nx_packet_pool_create( &nx_pools[ 1 ], (char* )"", WICED_LINK_MTU_ALIGNED, PLATFORM_L1_CACHE_PTR_ROUND_UP(rx_buffer_pool_memory), APP_RX_BUFFER_POOL_SIZE ) )
    {
    ...
    {
        ip_create_result = ( status = nx_ip_create( &ip_handle, (char*)"NetX IP", IP_ADDRESS(0, 0, 0, 0), 0xFFFFF000UL, &nx_pools[0], wiced_sta_netx_duo_driver_entry, ip_stack, IP_STACK_SIZE, 2 ) );
    }
    else
    {
        ip_create_result = ( status = nx_ip_create( &ip_handle, (char*)"NetX IP", IP_ADDR, NETMASK, &nx_pools[0], wiced_sta_netx_duo_driver_entry, ip_stack, IP_STACK_SIZE, 2 ) );
    }
    ...
    if ( USE_DHCP == WICED_TRUE )
    {
        ...
        do
        {
            ...
            nx_dhcp_packet_pool_set( &dhcp_handle, &nx_pools[0] );
